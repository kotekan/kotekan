 ...
 __kernel void Upchannelize(__global float2 *data, __global float *results_array){
   ...
   //  uint address_in = get_group_id(0)<<8;
   //address_in = address_in + local_address;
   uint address_in = get_global_id(1)*38432 + get_group_id(0)*384 + local_address;
   ...
   if (get_local_id(0) < 16){ //currently only 16 out of 64 has work to do. not ideal
     for (int j=0;j<3;j++){
       for (int i=0;i<8;i++){
	 outtmp += local_data[ get_local_id(0)*8 +j*128+i];
       }
     }
  
//results_array[get_group_id(0)*16+get_local_id(0)] = sqrt(outtmp.REAL*outtmp.REAL + outtmp.IMAG*outtmp.IMAG)/24.;
     results_array[get_global_id(1)*100*16 + get_group_id(0)*16+get_local_id(0)] = (outtmp.REAL*outtmp.REAL + outtmp.IMAG*outtmp.IMAG)/24.;  
  }

//results_array[get_global_id(1)*100*16 + get_group_id(0)*16+get_local_id(0)] = get_global_id(1); //get_global_id(1)*100*16 + get_group_id(0)*16+get_local_id(0);
    //results_array[get_global_id(1)*100*16] = 20; //get_global_size(1);
    //results_array[get_global_id(1)*100*16] = data[get_global_id(1)*38432].REAL ; 
    //results_array[get_global_id(1)] = data[get_global_id(1)*38432].REAL;


  //write back to memory (each write 4 values)
  //results_array[address_in     ] = local_data[get_local_id(0)    ];
  //results_array[address_in +64 ] = local_data[get_local_id(0) + 64];
  //results_array[address_in +128] = local_data[get_local_id(0) + 128];
  //results_array[address_in +192] = local_data[get_local_id(0) + 192];

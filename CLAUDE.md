...
- In main_thread, lock_range should lock at least int_frames*2 and no more than half the total number of frames - 3 frames.
...
- if the size of the input buffer frame is (samples_per_dataset, num_local_freq, num_elements), the size of the outmb_buf frame should be (samples_per_dataset * num_elements / num_beams, num_local_freq, num_beams). Let's keep the frame size same so that the metadata overhead ratio is similar.

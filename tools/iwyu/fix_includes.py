...
def _CalculateLineTypesAndKeys(file_lines, iwyu_record):
    ...
    for line_info in file_lines:
        if line_info.line is None:
            ...
            line_info.type = _COMMENT_LINE_RE
        elif in_allman_or_mixed_namespace and _NAMESPACE_CONTINUE_ALLMAN_MIXED_RE.match(
            line_info.line
        ):
            in_allman_or_mixed_namespace = False
...
def _DeleteEmptyNamespaces(file_lines):
    ...
    while start_line < len(file_lines):
        ...
        if line_info.deleted or (
            line_info.type != _NAMESPACE_START_RE
            and line_info.type != _NAMESPACE_START_ALLMAN_RE
            and line_info.type != _NAMESPACE_START_MIXED_RE
        ):
            start_line += 1
            continue
        if line_info.type in (_NAMESPACE_START_RE, _NAMESPACE_START_MIXED_RE):
            ...
            assert False, ("unknown namespace type", _LINE_TYPES.index(line_info.type))
...
def _GetToplevelReorderSpans(file_lines):
    ...
    for reorder_span in reorder_spans:
        for line_number in range(*reorder_span):
            if (
                in_ifdef[line_number]
                or in_namespace[line_number]
                or file_lines[line_number].is_nested_forward_declaration
            ):
                break
        else:  # for/else
    ...
    try:
        ...
        for line_number, line_info in enumerate(file_lines):
            ...
            # will still propagate to the top of the file.
                ...
                if len(namespaces) != 1:
                    raise FixIncludesError(
                        "Allman namespace found containing multiple " "names: %s",
                        line_info.line,
                    )
                ...
                if pending_namespace_prefix == "":
                    raise FixIncludesError(
                        "Namespace bracket found without an associated "
                        "namespace name at line: %s",
                        line_number,
                    )
                pending_namespace_prefix += " {"
        ...
        print(
            "DEBUG: Namespace detection returned prematurely because of an "
            "exception: %s" % (why)
        )
        pass
...
def _IsMainCUInclude(line_info, filename):
    ...
    canonical_include = re.sub(
        r"(-inl\.h|\.h|\.hpp)$", "", line_info.key.replace('"', ""), flags=re.I
    )
...
def _GetLineKind(file_line, filename, separate_project_includes):
    ...
    if file_line.deleted:
        ...
        if separate_project_includes and _IsSameProject(
    ...
    for reorder_span in good_reorder_spans:
        for line_number in range(*reorder_span):
            line_kind = _GetLineKind(
                file_lines[line_number], filename, flags.separate_project_includes
            )
...
def _DecoratedMoveSpanLines(iwyu_record, file_lines, move_span_lines, flags):
    ...
    kind = _GetLineKind(
        firstline, iwyu_record.filename, flags.separate_project_includes
    )
...
def _GetSymbolNameFromForwardDeclareLine(line):
    ...
    namespaces_in_line = iwyu_namespace_re.findall(
        line.replace("namespace {", "namespace (anonymous namespace) {")
    )
...
def ProcessIWYUOutput(f, files_to_process, flags, cwd):
    ...
    if files_to_process is not None:
        files_to_process = [NormalizeFilePath(cwd, fname) for fname in files_to_process]
    ...
    while True:
        ...
        if flags.only_re and not re.search(flags.only_re, filename):
            print(
                "(skipping %s: it does not match --only_re, which is %s)"
...
def main(argv):
    ...
    parser.add_argument("--noblank_lines", action="store_false", dest="blank_lines")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=("Replace 'why' comments with the ones provided by" " IWYU"),
    )
    parser.add_argument(
        "--noupdate_comments", action="store_false", dest="update_comments"
    )
    ...
    parser.add_argument("--nosafe_headers", action="store_false", dest="safe_headers")
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Re-order lines relative to other similar lines "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "%(prog)s will skip editing any file whose name"

...
if __name__ == "__main__":
    ...
    parser.add_argument("--frame0-src", choices=['fpga_master', 'kotekan', 'manual'],
                        default='manual', help="Where to obtain frame0 time")
    parser.add_argument("--frame0-ns", default=None, type=int, help="A manual frame0 time, only used if frame0-src is 'manual'")
    parser.add_argument("--current-time", default=None, help="The current time, or reference time to build EOP table for, as an Astropy-parsable string (e.g. ISOT). If unset, astropy.time.Time.now() is used.")
    parser.add_argument("-kh", "--kotekan-host", default="localhost", help="A host running Kotekan, for frame0 or a current EOP table, default 'localhost'.") 
    parser.add_argument("-kp", "--kotekan-port", default=12048, type=int, help="The port to contact Kotekan on, default 12048.")
    parser.add_argument("-kprot", "--kotekan-protocol", default="http://",
                        help="Protocol to issue GET requests to kotekan, default: 'http://'")
    parser.add_argument("-fh", "--fpga-master-host", default="localhost", help="A host running fpga_master, default: localhost") 
    parser.add_argument("-fp", "--fpga-master-port", default=54321, type=int, help="A port to contact fpga_master on, default: 54321")
    parser.add_argument("-fprot", "--fpga-master-protocol", default="http://", help="Protocol to issue GET requests to fpga_master, default: 'http://'")
    ...
    parser.add_argument("-dt", "--interval-length-days", default=1.0, type=float, help="Length of interval, e.g. time between EOP table entries in UTC days, default 1.0.")
    parser.add_argument("--force-iers-download", action='store_true', help="Download a fresh set of IERS data, otherwise use cached data (which is not more than 10 days out)")
    parser.add_argument("--enforce-continuity", choices=['yes', 'no'],
                        default='yes', help="If 'yes', read the current EOP table in Kotekan and merge it with the new table so all EOP will be interpolated continuously in time. Only new entries ahead of the current interval (plus cushion time) will be included in the new table, otherwise old entries will be used. Some old entries sufficiently in the past will be dropped.")
    parser.add_argument("--merge-cushion-dt", default="1hr", help="Cushion time to add to the current (reference) time when determining the current interval when merging eop tables to enforce continuity. An astropy-parsable TimeDelta string, default `1hr`")
    parser.add_argument("-o", "--out-json-file", default=None, help="File to write EOP table to in JSON format. File is suitable input for `broadcastEOPTable.py` and POSTing to Kotekan update endpoint.")
    ...
    if args.frame0_src == "fpga_master":
        t0_ns = eop_utils.read_fpga_master_frame0_ns(fpga_master_host, fpga_master_port, timeout)
    ...
    ts = eop_utils.build_time_array(t_ref, num_intervals_before, num_intervals_after,
    ...
    if enforce_continuity:
        ...
        current_eop_table = eop_utils.read_kotekan_eop_table(kotekan_host, kotekan_port, timeout,
                                                   kotekan_protocol)
        eop_table = eop_utils.merge_eop_tables(current_eop_table, new_eop_table, t_ref, t0_ns,
    ...
    print("The final EOP table contains {:d} entries from {} to {}"
          .format(len(eop_table),
                  eop_utils.calc_astropy_time_from_inst_ns(eop_table[0]['time_inst_ns'], t0_ns).utc.isot,
                  eop_utils.calc_astropy_time_from_inst_ns(eop_table[-1]['time_inst_ns'], t0_ns).utc.isot))

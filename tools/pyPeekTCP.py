...
# 	int samples_per_packet;	// - number of samples in packet (or dimensions,
#                           //   n_freq x n_time x n_stream?)
...
# 	char stokes_type; 		// - description of stream (e.g. V / H pol,
#                           //   Stokes-I / Q / U / V)
# 							//	-8	-7	-6	-5	-4	-3	-2	-1	1	2	3	4
# 							//	YX	XY	YY	XX	LR	RL	LL	RR	I	Q	U	V
# };
header_fmt = '=iiiidiiiId'
stokes_lookup = ['YX', 'XY', 'YY', 'XX', 'LR', 'RL', 'LL', 'RR', 'I', 'Q', 'U',
                 'V']
...
def updatefig(*args):
    ...
    for i in np.arange(pkt_elems):
        if medsub:
            p[i].set_data(
                waterfall[:, :, i] - np.nanmedian(waterfall[:, :, i], axis=0)[
                                     np.newaxis, :])
        ...
        p[pkt_elems + i].set_clim(vmin=colorscale[0] / 10,
                                  vmax=colorscale[1] / 10)
    return p,
...
info_header = receive(connection, pkt_freqs * 4 * 2 +
                      pkt_elems * 1)
freqlist = np.fromstring(info_header[:pkt_freqs * 4 * 2],
                         dtype=np.float32).reshape(-1, 2)  # .mean(axis=1)
...
if (pkt_int_len > total_integration):
    print
    "Pre-integrated to longer than desired time!"
    print
    "{} vs {}".format(pkt_int_len, total_integration)
    print
    "Resetting integration length to {}".format(pkt_int_len)
    total_integration = pkt_int_len
local_integration = total_integration / pkt_int_len

waterfall = np.zeros((plot_times, plot_freqs, pkt_elems),
    ...
    while True:
        try:
            ...
            for i in np.arange(local_integration * pkt_elems):
                ...
                d[:, data_pkt_elem_idx] += np.fromstring(data[pkt_header:],
                                                         dtype=np.uint32) * 1.0
                n[:, data_pkt_elem_idx] += data_pkt_samples_summed * 1.0
                fold_idx = np.array(((sec_per_pkt_frame * data_pkt_frame_idx
                                      + 0.5 * fold_period) % fold_period)
                                    / fold_period * plot_phase,
                                    dtype=np.int32)
                waterfold[fold_idx, :, data_pkt_elem_idx] += np.fromstring(
                    data[pkt_header:],
                    dtype=np.uint32).reshape(-1, pkt_freqs / plot_freqs).mean(
                    axis=1)
                countfold[fold_idx, :,
            ...
            times[0] = sec_per_pkt_frame * (
                        data_pkt_frame_idx - pkt_idx0) + pkt_utc0
            # print(d,n)
            waterfall = np.roll(waterfall, roll_idx, axis=0)
            waterfall[0, :, :] = 10 * np.log10(
                (d / n).reshape(-1, pkt_freqs / plot_freqs, pkt_elems).mean(
                    axis=1))
...
plt.ioff()
p = []
tmin = md.date2num(datetime.datetime.fromtimestamp(
    pkt_utc0 - plot_times * local_integration * sec_per_pkt_frame))
tmax = md.date2num(datetime.datetime.fromtimestamp(pkt_utc0))
times = pkt_utc0 - np.arange(
    plot_times) * local_integration * sec_per_pkt_frame
date_format = md.DateFormatter('%H:%M:%S')
medsub = True
colorscale = [-.5, .5]

for i in np.arange(pkt_elems):
    p.append(ax[0, i].imshow(waterfall[:, :, i], aspect='auto', animated=True,
                             origin='upper', interpolation='nearest',
                             cmap='gray', vmin=colorscale[0],
                             vmax=colorscale[1],
                             extent=[freqlist[0, 0], freqlist[-1, -1], tmin,
                                     tmax]))
...
for i in np.arange(pkt_elems):
    p.append(ax[1, i].imshow(waterfold[:, :, i], aspect='auto', animated=True,
                             origin='upper', interpolation='nearest',
                             cmap='gray', vmin=colorscale[0],
                             vmax=colorscale[1],

...
struct BitMapping {
    ...
    BitMapping(const vector<string>& names_) : names(names_), rank(names_.size()) {
        assert(rank >= 5);
        assert(rank <= 15);
...
static void test_shuffle_kernel(const Array<int>& dst_, const BitMapping& bm_phys,
                                const BitMapping& bm_in, const BitMapping& bm_out) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cout << "Mismatch found\n"
             << "    at dst_index=" << i << ", dst_phys=" << bm_phys.index_to_name(i)
             ...
             << ", src_logical=" << bm_in.index_to_name(d_expected) << endl;

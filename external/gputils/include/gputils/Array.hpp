...
struct Array {
    ...
    inline bool on_gpu() const {
        return !data || af_on_gpu(aflags);
    }
    inline bool on_host() const {
        return !data || af_on_host(aflags);
    }

    // Copies data from 'src' to 'this'. Arrays must have the same shape, but need not have the same
    // strides.
    ...
    bool is_fully_contiguous() const {
        return get_ncontig() == ndim;
    }
    ...
    inline Array<Tdst> convert_dtype() const;
    template<typename Tdst>
    inline Array<Tdst> convert_dtype(int aflags) const;
    ...
    inline bool shape_equals(const Array<T2>& a) const;
...
extern void check_array_invariants(const void* data, int ndim, const ssize_t* shape, ssize_t size,
                                   const ssize_t* strides, int aflags);
...
extern void fill_helper(void* dst, int dst_ndim, const ssize_t* dst_shape,
                        const ssize_t* dst_strides, const void* src, int src_ndim,
                        const ssize_t* src_shape, const ssize_t* src_strides, ssize_t itemsize,
                        bool noisy = false);
...
template<typename T>
Array<T>::Array() : Array(0, nullptr, 0) {}
...
template<typename T>
Array<T>::Array(int ndim_, const ssize_t* shape_, int aflags_) : ndim(ndim_), aflags(aflags_) {
    assert(ndim >= 0);
...
template<class C>
int ndim_ss(const C& shape, const C& strides) {
...
template<typename T>
Array<T>::Array(const std::vector<ssize_t>& shape_, const std::vector<ssize_t>& strides_,
                int aflags) : Array(ndim_ss(shape_, strides_), &shape_[0], &strides_[0], aflags) {}

template<typename T>
Array<T>::Array(std::initializer_list<ssize_t> shape_, std::initializer_list<ssize_t> strides_,
                int aflags) :
...
void Array<T>::fill(const Array<T>& src) {
    fill_helper(data, ndim, shape, strides, src.data, src.ndim, src.shape, src.strides, sizeof(T),
                false); // noisy=false
}
...
template<typename T>
T& Array<T>::at(int nd, const ssize_t* ix) {
    return _at(nd, ix);
}
template<typename T>
T& Array<T>::at(const std::vector<ssize_t>& ix) {
    return _at(ix.size(), &ix[0]);
}
template<typename T>
T& Array<T>::at(std::initializer_list<ssize_t> ix) {
    return _at(ix.size(), ix.begin());
}

template<typename T>
const T& Array<T>::at(int nd, const ssize_t* ix) const {
    return _at(nd, ix);
}
template<typename T>
const T& Array<T>::at(const std::vector<ssize_t>& ix) const {
    return _at(ix.size(), &ix[0]);
}
template<typename T>
const T& Array<T>::at(std::initializer_list<ssize_t> ix) const {
    return _at(ix.size(), ix.begin());
}
...
template<typename T>
std::string Array<T>::shape_str() const {
    return ::gputils::shape_str(ndim, shape);
}

template<typename T>
std::string Array<T>::stride_str() const {
    return ::gputils::shape_str(ndim, strides);
}

template<typename T>
void Array<T>::check_invariants() const {
...
struct DtypeConverter<__half, float> {
    static inline __half convert(float x) {
...
template<>
struct DtypeConverter<__half, double> {
    static inline __half convert(double x) {
...
struct DtypeConverter<float, __half> {
    static inline float convert(__half x) {
...
template<>
struct DtypeConverter<double, __half> {
    // CUDA doesn't define __half2double()
    static inline double convert(__half x) {
        return __half2float(x);
    }
};
...
template<typename Tdst, typename Tsrc>
inline void convert_dtype_helper(Tdst* dst, const Tsrc* src, int nouter_dims,
                                 const ssize_t* outer_shape, const ssize_t* outer_dst_strides,
                                 const ssize_t* outer_src_strides, ssize_t nelts_contig) {
    if (nouter_dims > 0) {
        for (int i = 0; i < outer_shape[0]; i++) {
            convert_dtype_helper(dst + i * outer_dst_strides[0], src + i * outer_src_strides[0],
                                 nouter_dims - 1, outer_shape + 1, outer_dst_strides + 1,
                                 outer_src_strides + 1, nelts_contig);
        }
    } else {

...
static constexpr int af_uhost =
    0x02; // host memory, not registered with cuda runtime or page-locked
...
// Mmap flags: if specified (with af_uhost or af_rhost), then mmap/munmap will be used instead of
// malloc/free.
static constexpr int af_mmap_small = 0x100; // 4KB standard pages
static constexpr int af_mmap_huge = 0x200;  // 2MB huge pages
static constexpr int af_mmap_try_huge =
    0x400; // try huge pages, if fails then fall back to 4K pages, and print warning.
...
static constexpr int af_all_flags =
    af_location_flags | af_initialization_flags | af_mmap_flags | af_debug_flags;
...
inline bool af_on_gpu(int flags) {
    return (flags & (af_gpu | af_unified)) != 0;
}
inline bool af_on_host(int flags) {
    return (flags & af_gpu) == 0;
}

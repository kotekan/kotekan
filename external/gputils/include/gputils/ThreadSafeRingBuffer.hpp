...
// ThreadSafeRingBuffer<T>: A simple ring buffer which allows producer threads to "put" items of
// type T (blocking if ring buffer is full), and consumer threads to "get" items (blocking if ring
// buffer is empty).
//
// Typical use cases are T=int (representing iterations of a loop), or T=shared_ptr<...>
// (representing work units defined by a more complex data structure)
...
struct ThreadSafeRingBuffer {
    ...
    ThreadSafeRingBuffer(ssize_t capacity_) : capacity(capacity_) {
        assert(capacity > 0);
    ...
    bool get(T& t) {
        ...
        //
        // To see why this is useful, consider a case where T is a shared_ptr to a "heavyweight"
        // object. Then (*) would leave a stray reference in the ring buffer, whereas std::swap()
        // allows the consumer (i.e. caller of get()) to "swap in" an empty pointer.
    ...
    // After set_done() has been called, calls to get() return 'false', and calls to put() throw an
    // exception.

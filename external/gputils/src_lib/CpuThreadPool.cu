...
CpuThreadPool::CpuThreadPool(const callback_t& callback_, int nthreads_,
                             int max_callbacks_per_thread_, const string& name_) :
    callback(callback_), nthreads(nthreads_), max_callbacks_per_thread(max_callbacks_per_thread_),
    name(name_) {
    assert(nthreads > 0);
    assert(nthreads <= 256);

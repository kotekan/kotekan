...
void launch_memcpy_kernel(void* dst, const void* src, long nbytes, cudaStream_t stream) {
    ...
    memcpy_kernel<<<nblocks, nthreads_per_block, 0, stream>>>(
        reinterpret_cast<int*>(dst), reinterpret_cast<const int*>(src), nelts, nelts_per_block);
}
...
__global__ void memcpy_2d_kernel(int* dst, long dp_nelts, const int* src, long sp_nelts, long nrows,
                                 long ncols, long nrows_per_block) {
...
void launch_memcpy_2d_kernel(void* dst, long dpitch, const void* src, long spitch, long width,
                             long height, cudaStream_t stream) {
    ...
    memcpy_2d_kernel<<<nblocks, nthreads, 0, stream>>>(reinterpret_cast<int*>(dst), dp_nelts,

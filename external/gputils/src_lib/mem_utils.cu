...
void check_aflags(int flags, const char* where) {
    ...
    if (_unlikely(non_single_bit(flags & af_location_flags)))
        throw runtime_error(string(where) + ": must specify precisely one of "
                            + aflag_str(af_location_flags));

    if (_unlikely(multiple_bits(flags & af_initialization_flags)))
        throw runtime_error(string(where) + ": can specify at most one of "
                            + aflag_str(af_initialization_flags));

    if (_unlikely(multiple_bits(flags & af_mmap_flags)))
        throw runtime_error(string(where) + ": can specify at most one of "
                            + aflag_str(af_mmap_flags));

    if (_unlikely((flags & af_mmap_flags) && !(flags & (af_uhost | af_rhost))))
        throw runtime_error(string(where)
                            + ": if af_mmap_* flag is specified, then one of {af_uhost,af_rhost} "
...
struct alloc_helper {
    ...
    string mmap_error_message(ssize_t nbytes, bool hugepage_flag) {
        ...
        ss << "mmap(" << nbytes_to_str(nbytes) << (hugepage_flag ? ", MAP_HUGETLB" : "")
    ...
    inline void _mmap(ssize_t nbytes_unpadded) {
        ...
        if (flags & (af_mmap_huge | af_mmap_try_huge)) {
            ...
            this->base = (char*)mmap(NULL, nbytes_allocated, pflags, mflags | MAP_HUGETLB, -1,
                                     0); // note MAP_HUGETLB
            ...
            if (flags & af_mmap_try_huge) {
                // Temporarily buffer in stringstream, to reduce probability of interleaved output
        ...
        if (flags & (af_mmap_small | af_mmap_try_huge)) {
            ...
            this->nbytes_allocated = (nbytes_unpadded + mask) & (~mask);
            this->base =
                (char*)mmap(NULL, nbytes_allocated, pflags, mflags, -1, 0); // note no MAP_HUGETLB
    ...
    alloc_helper(ssize_t nbytes, int flags_) : nbytes_requested(nbytes), flags(flags_) {
        ...
        if (flags & af_mmap_flags) {
            // Note: this->_mmap() initializes this->base, and updates the value of
            // this->nbytes_allocated.
            ...
            CUDA_CALL(cudaMallocManaged((void**)&this->base, this->nbytes_allocated,
                                        cudaMemAttachGlobal));
            ...
            throw std::runtime_error("gputils::alloc(): couldn't allocate "
                                     + to_string(nbytes_allocated) + " bytes");
        ...
        // Step 2: if verbose, announce that memory has been allocated (keep in sync with Step 1 by
        // hand).

        if (flags & af_verbose) {
            // Temporarily buffer in stringstream, to reduce probability of interleaved output in
    ...
    void check_guard() {
        ...
        if (flags & af_gpu) {
            ...
            CUDA_CALL_ABORT(cudaMemcpy(p + nguard, base + nguard + nbytes_requested, nguard,
    ...
    void operator()(void* ptr) {
        ...
        // Deallocate memory. Keep this part in sync with "Step 1: allocate memory..." in
        // constructor.

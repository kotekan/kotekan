...
// Maybe it's time for <gputils/asserts.hpp> which defines _unlikely(), assert macros, exception
...
void pin_thread_to_vcpus(const vector<int>& vcpu_list) {
    ...
    //
    // FIXME Calling std::thread::hardware_concurrency() doesn't seem quite right, but doing the
    // "right" thing seems nontrivial. According to 'man sched_setaffinity()':
    //

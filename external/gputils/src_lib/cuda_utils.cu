...
runtime_error make_cuda_exception(cudaError_t xerr, const char* xstr, const char* file, int line) {
    ...
    ss << "CUDA error: " << xstr << " returned " << xerr << " (" << cudaGetErrorString(xerr) << ")"
       << " [" << file << ":" << line << "]";
...
void assign_kernel_dims(dim3& nblocks, dim3& nthreads, long nx, long ny, long nz,
                        int threads_per_block, bool noisy) {
    // Reference:
    ...
    for (int x = 5; (1 << x) <= threads_per_block; x++) {
        ...
        for (int y = 0; (1 << (x + y)) <= threads_per_block; y++) {
            ...
            // A caller which sets ny=1 should always get nblocks.y == nthreads.y == 1 (and likewise
            // for z).
            if ((ny == 1) && (ty != 1))
                continue;
            if ((nz == 1) && (tz != 1))
    ...
    if (nbest == 0) {
        stringstream ss;
        ss << "assign_kernel_dims() failed: (ny,nz,nz)=(" << nx << "," << ny << "," << nz
           << ") is too large";
    ...
    if (noisy) {
        ...
        cout << "assign_kernel_dims: (nx,ny,nz,T)=(" << nx << "," << ny << "," << nz << ","
             << threads_per_block << "): nblocks=" << dim3_str(nblocks)
             << ", nthreads=" << dim3_str(nthreads) << ", overhead=" << overhead << endl;
    }
}

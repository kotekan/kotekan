...
void make_random_reshape_compatible_shapes(vector<ssize_t>& dshape, vector<ssize_t>& sshape,
                                           vector<ssize_t>& sstrides, int maxaxis,
                                           ssize_t maxsize) {
    assert(maxaxis > 0);
    assert(maxsize > 0);
...
assert_arrays_equal(const Array<T>& arr1, const Array<T>& arr2, const string& name1,
                    const string& name2, const vector<string>& axis_names, float epsabs,
                    float epsrel, ssize_t max_display, bool verbose) {
    ...
    for (auto ix = arr1.ix_start(); arr1.ix_valid(ix); arr1.ix_next(ix)) {
        ...
        if constexpr (is_complex_v<T>) {
            if (!std::isfinite(x.real()) || !std::isfinite(x.imag()) || !std::isfinite(y.real())
                || !std::isfinite(y.imag()))
                failed = true;
        ...
        cout << ": " << name1 << "=" << x << ", " << name2 << "=" << y << "  [delta=" << delta
             << "]";
...
#define INSTANTIATE_PRINT_ARRAY(T)                                                                 \
    template void print_array(const Array<T>& arr, const vector<string>& axis_names, ostream& os);

#define INSTANTIATE_ASSERT_ARRAYS_EQUAL(T)                                                         \
    template gputils::decomplexify_type<T>::type assert_arrays_equal(                              \
        const Array<T>& arr1, const Array<T>& arr2, const string& name1, const string& name2,      \
        const vector<string>& axis_names, float epsabs, float epsrel, ssize_t max_display,         \
        bool verbose);

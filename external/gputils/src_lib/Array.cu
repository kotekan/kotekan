...
void check_array_invariants(const void* data, int ndim, const ssize_t* shape, ssize_t size,
...
void reshape_ref_helper(int src_ndim, const ssize_t* src_shape, const ssize_t* src_strides,
                        int dst_ndim, const ssize_t* dst_shape, ssize_t* dst_strides) {
    int status =
        reshape_helper2(src_ndim, src_shape, src_strides, dst_ndim, dst_shape, dst_strides);
...
static void fill_helper2(char* dst, const char* src, int ndim, const fill_axis* axes) {
    ...
    if (ndim == 2) {
        ...
        CUDA_CALL(cudaMemcpy2D(dst, axes[1].dstride, src, axes[1].sstride, axes[0].length,
                               axes[1].length, cudaMemcpyDefault));
        return;
    }
    ...
    for (int i = 0; i < axes[ndim - 1].length; i++) {
        fill_helper2(dst + i * axes[ndim - 1].dstride, src + i * axes[ndim - 1].sstride, ndim - 1,
                     axes);
    }
}
...
void fill_helper(void* dst, int dst_ndim, const ssize_t* dst_shape, const ssize_t* dstride,
    ...
    if (noisy) {
        cout << "fill: input shape=" << tuple_str(ndim, shape)
             ...
             << ", sstride_nelts=" << tuple_str(ndim, sstride) << ", itemsize=" << itemsize << "\n";

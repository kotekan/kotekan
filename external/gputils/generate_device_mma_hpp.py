...
class Argument:
    ...
    def make_cuda_arglist(self):
        ...
        if self.is_immediate:
            ...
            return [
                f"{self.cuda_type} {cv}{self.name}{i}"
...
def emit_dense_mma(
    cuda_name, ptx_name, cuda_type, dbits, sbits, m, n, k, ptx_type=None, s=1
):
    # Register counts
    na = (m * k * s * sbits) // 1024
    nb = (k * n * s * sbits) // 1024
...
def emit_dense_f16_mma(m, n, k, s=1, layout=None):
    ...
    emit_dense_mma(
        cuda_name, ptx_name, "__half2", 16, 16, m, n, k, ptx_type="unsigned int", s=s
    )
...
def emit_dense_int_mma(sbits, m, n, k):
    ...
    ptx_name = (
        f"mma.sync.aligned.m{m}n{n}k{k}.row.col.satfinite.s32.s{sbits}.s{sbits}.s32"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"//   https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#warp-level-matrix-shape"
    )
    print(f"//")
    print(f"// Reference for PTX instruction syntax:")
    print(
        f"//   https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#warp-level-matrix-instructions-mma"
    )
    print()

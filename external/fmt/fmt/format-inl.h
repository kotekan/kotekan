...
digits::result grisu_gen_digits(fp value, uint64_t error, int& exp, Handler& handler) {
    ...
    do {
        ...
        result =
            handler.on_digit(static_cast<char>('0' + digit), data::powers_of_10_64[exp] << -one.e,
...
FMT_API bool grisu_format(Double value, buffer<char>& buf, int precision, unsigned options,
                          int& exp) {
...
FMT_FUNC int internal::utf16_to_utf8::convert(wstring_view s) {
    ...
    buffer_.resize(length + 1);
    length =
        WideCharToMultiByte(CP_UTF8, 0, s.data(), s_size, &buffer_[0], length, nullptr, nullptr);
    if (length == 0)
        return GetLastError();
    buffer_[length] = 0;
    return 0;

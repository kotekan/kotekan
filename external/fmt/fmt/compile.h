...
public:
    ...
    struct argument_id {
        ...
        FMT_CONSTEXPR argument_id(internal::string_view_metadata id) :
            which(which_arg_id::named_index),
            val(id) {}
    ...
    FMT_CONSTEXPR format_part(unsigned id) :
        which(kind::argument_id),
        end_of_argument_id(0u),
        val(id) {}

    FMT_CONSTEXPR format_part(named_argument_id arg_id) :
        which(kind::named_argument_id),
        end_of_argument_id(0u),
        val(arg_id) {}

    FMT_CONSTEXPR format_part(specification spec) :
        which(kind::specification),
        end_of_argument_id(0u),
        val(spec) {}
...
public:
    ...
    constexpr prepared_format(Format f) :
        format_(std::move(f)),
        parts_provider_(to_string_view(format_)) {}
                ...
                case format_part_t::kind::specification: {
                    ...
                    const auto arg =
                        value.spec.arg_id.which == format_part_t::argument_id::which_arg_id::index
                            ...
                            : ctx.arg(
                                arg_id_value.named_index.to_view(to_string_view(format_).data()));

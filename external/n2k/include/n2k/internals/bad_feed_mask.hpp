...
__device__ inline uint load_bad_feed_mask(const uint* bf_mask, uint* sp, int S) {
    ...
    for (uint t = t0; t < (S >> 2); t += nt) {
        ...
        // FIXME (low-priority) creates a bank conflict if S > 1024, but I doubt this is ever a
        // bottleneck.
    ...
    // In the loop (*) above, each warp will process 32 values of i, and (S / blockDim.x) values of
    // j. We read the mask from shared memory as: (bits <-> i), (threads <-> j) And then transpose
    // to: (bits <-> j), (threads <-> i).
    ...
    uint y = sp[shi];
    y = transpose_bit_with_lane<1>(
        y, 4); // i0 i1 i2 i3 i4 <-> b3 b4 t2 b1 b2     j0 j1 j2 j3 j4 <-> b0 t3 t4 t0 t1
    y = transpose_bit_with_lane<2>(
        y, 8); // i0 i1 i2 i3 i4 <-> b3 b4 t2 t3 b2     j0 j1 j2 j3 j4 <-> b0 b1 t4 t0 t1
    y = transpose_bit_with_lane<4>(
        y, 16); // i0 i1 i2 i3 i4 <-> b3 b4 t2 t3 t4     j0 j1 j2 j3 j4 <-> b0 b1 b2 t0 t1
    y = transpose_bit_with_lane<8>(
        y, 1); // i0 i1 i2 i3 i4 <-> t0 b4 t2 t3 t4     j0 j1 j2 j3 j4 <-> b0 b1 b2 b3 t1
    y = transpose_bit_with_lane<16>(
        y, 2); // i0 i1 i2 i3 i4 <-> t0 t1 t2 t3 t4     j0 j1 j2 j3 j4 <-> b0 b1 b2 b3 b4

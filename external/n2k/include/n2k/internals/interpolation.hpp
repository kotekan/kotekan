...
//   float shmem_bias_coeffs[bias_nx][4][2];  // length-2 axis is a spectator, i.e. both values are
//   equal float shmem_sigma_coeffs[sigma_nx][8];   // length-8 axis is a spectator, i.e. all 8
//   values are equal float shmem_tmp_coeffs[4*bias_nx + sigma_nx];
//
...
__host__ inline double interpolate_bias_cpu(double x, double y) {
    ...
    double c[4];
    for (int j = 0; j < 4; j++)
        c[j] = btab[4 * (i + j)] + btab[4 * (i + j) + 1] * y + btab[4 * (i + j) + 2] * y * y
               + btab[4 * (i + j) + 3] * y * y * y;
...
__device__ inline void load_sigma_coeffs(const float* sigma_coeffs, int i, float& c0, float& c1,
                                         float& c2, float& c3) {
    int t = threadIdx.x;
...
template<bool Debug = false>
__device__ inline float load_bias_inner(const float* coeffs, float y0, float y1, float y2,
                                        float y3) {
    int t = threadIdx.x;
...
template<bool Debug = false>
__device__ inline void load_bias_coeffs(const float* bias_coeffs, int i, float y, float& c0,
...
__device__ inline float interpolate_sigma_gpu(const float* shmem_bsigma_coeffs, float x) {
    ...
    float c0, c1, c2, c3;
    load_sigma_coeffs(shmem_bsigma_coeffs + nb, ix - 1, c0, c1, c2,
                      c3); // note (ix-1) and (... + nb) here.

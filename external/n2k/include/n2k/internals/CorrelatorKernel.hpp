...
struct CorrelatorKernel {
    ...
    // Given a 32-bit register 'x' containing eight signed 4-bit integers x[0:8], negate x[i] ->
    // -x[i]. WARNING: assumes x[i] is in the range [-7,7], i.e. fails if any x[i] is (-8)!
    ...
    // array of 4-bit integers, represented as eight 32-bit registers, and performs a logical
    // transpose.
    //
...
#pragma unroll
    ...
    static __device__ void store_prefetched_chunk(int* __restrict__ sp, int pf_in[8], uint rm) {
        const int t8 =
            threadIdx.x >> 6; // satisfies 0 <= t8 < 4, indicates 8-time chunk for this warp.
...
#pragma unroll
    ...
    // On exit, 'sp' will be filled with 128 time samples, and 'pf' will be filled with the next 32
    // time samples.
    ...
    correlate_t64(int V[8][2][2][4], const int* __restrict__ ap, const int* __restrict__ bp,
        ...
        constexpr int SA =
            2 * CorrelatorParams::shmem_s8_stride; // one A-fragment corresponds to 16 stations
...
#pragma unroll
    ...
    // trivial wrapper around two calls to correlate_t64(). The point of defining this wrapper is
    // that 128 time samples is the longest interval over which the __restrict__ keywords are valid.
    //
    // The 'P' template parameter controls the level of prefetching.
    //
    //   P=0: Caller should pass old prefetched data in 'pf'.
...
#pragma unroll
    ...
    //
    // Outer three V indices are V[k//8][i//16][ReIm], and innermost int[4] is a 16-by-8 fragment,
    // with:
    ...
    static __device__ void kernel_body(int* dst, const int8_t* src, const uint* rfimask,
                                       const int* ptable, int nt_inner, int nt_outer) {
        ...
        // The 'vp_tf' pointer has (t,f) offsets applied, but no (i,j) offsets (even per-block
        // offsets)

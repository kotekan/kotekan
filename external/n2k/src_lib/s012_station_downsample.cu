...
struct template_magic {
    ...
    static __device__ ulong load_and_sum(const ulong* Sin, uint bf, int m, int T, int Tmin,
                                         int Tsize, int F, int S) {
...
struct template_magic<N, N> {
    ...
    static __device__ ulong load_and_sum(const ulong* Sin, uint bf, int m, int T, int Tmin,
        ...
        ulong y = template_magic<(N / 2), (N / 2)>::load_and_sum(Sin, bf, m + (N / 2), T, Tmin,
                                                                 Tsize, F, S);
...
struct template_magic<1, 1> {
    // Base case: L == N == 1.
    static __device__ ulong load_and_sum(const ulong* Sin, uint bf, int m0, int T, int Tmin,
                                         int Tsize, int F, int S) {
        const int M = T * F * 3;
        m0 = (m0 < M) ? m0 : (M - 1);
...
__global__ void s012_station_downsample_kernel(ulong* Sout, const ulong* Sin, const uint* bf_mask,
    ...
    ulong x = template_magic<N, 32>::load_and_sum(Sin, bf, m, T, Tmin, Tsize, F,
                                                  S); // handles case m >= M
...
void launch_s012_station_downsample_kernel(ulong* Sout, const ulong* Sin, const uint8_t* bf_mask,
    ...
    if (Tmin < 0)
        throw runtime_error(
            "launch_s012_station_downsample_kernel(): first sample Tmin must be >= 0");
    if (Tsize <= 0)
        throw runtime_error(
            "launch_s012_station_downsample_kernel(): ringbuffer size Tsize be > 0");
    if (F <= 0)
    ...
    if (S & 127)
        throw runtime_error(
            "launch_s012_station_downsample_kernel(): expected S to be a multple of 128");
    if (S > rfi_max_stations)
        throw runtime_error(
            "launch_s012_station_downsample_kernel(): expected S to be <= max_rfi_stations");
    if ((M * S) > INT_MAX)
...
void launch_s012_station_downsample_kernel(ulong* Sout, const ulong* Sin, const uint8_t* bf_mask,
                                           long M, long S, cudaStream_t stream) {
    if (!Sout || !Sin || !bf_mask)
        throw runtime_error("launch_s012_station_downsample_kernel(): pointer was NULL");
    if (M <= 0)
    ...
    if (M % 3 != 0)
        throw runtime_error(
            "launch_s012_station_downsample_kernel(): expected M to be a multiple of 3");
    if (S <= 0)
        throw runtime_error("launch_s012_station_downsample_kernel(): expected S > 0");
    if (S & 127)
        throw runtime_error(
...
void launch_s012_station_downsample_kernel(Array<ulong>& Sout, const Array<ulong>& Sin,
                                           const Array<uint8_t>& bf_mask, cudaStream_t stream) {
    check_array(Sout, "launch_s012_station_downsample_kernel", "Sout", 3,
                true); // ndim=3, contiguous=true
    check_array(Sin, "launch_s012_station_downsample_kernel", "Sin", 4,
                true); // ndim=4, contiguous=true
    check_array(bf_mask, "launch_s012_station_downsample_kernel", "bf_mask", 1,
                true); // ndim=1, contiguous=true

    if (Sin.shape[0] != Sout.shape[0])
        throw runtime_error("launch_s012_station_downsample_kernel(): inconsistent number of time "
                            "samples in input/output S012 arrays");
    if (Sin.shape[1] != Sout.shape[1])
        throw runtime_error("launch_s012_station_downsample_kernel(): inconsistent number of "
                            "frequency channels in input/output S012 arrays");
    if (Sout.shape[2] != 3)
    ...
    if (Sin.shape[3] != bf_mask.shape[0])
        throw runtime_error("launch_s012_station_downsample_kernel(): inconsistent number of "
                            "stations in S012 arrays and bad feed mask");

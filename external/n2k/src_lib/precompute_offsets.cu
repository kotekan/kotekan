...
struct PointerOffsets {
    ...
    __host__ PointerOffsets(const CorrelatorParams& params_, int blockId_, int warpId_,
                            int laneId_) :
    ...
    __host__ void _init_prefetch_offsets() {
        ...
        gp = 32 * (bflag ? block_btile : block_atile); // each block_atile/block_btile index
                                                       // corresponds to 128 stations = 32 int32s
    ...
    __host__ void _init_warp_tiling() {
        ...
        ap = (wx * 4 * CorrelatorParams::shmem_s8_stride)
             + laneId; // one x-tile corresponds to 32 stations
        bp = (wy * 8 * CorrelatorParams::shmem_s8_stride) + laneId
             + CorrelatorParams::shmem_ab_stride; // one y-tile corresponds to 64 stations

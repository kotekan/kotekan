...
__global__ void __launch_bounds__(128, 8)
    correlate_pl_kernel_S16(int* counts, const ulong* pl_mask, const uint* rfimask,
                            int rfimask_fstride, int Tout, int F, uint N128) {
...
__global__ void __launch_bounds__(256, 2)
    correlate_pl_kernel_S128(int* counts, const ulong* pl_mask, const uint* rfimask,
                             int rfimask_fstride, uint N128) {
    constexpr int S = 128;
...
void launch_pl_1bit_correlator(int* counts, const ulong* pl_mask, const uint* rfimask,
                               long rfimask_fstride, long T, long F, long Sds, long Nds,
                               cudaStream_t stream) {
    if (!counts)
    ...
    if (Nds & 127)
        throw runtime_error(
            "launch_pl_1bit_correlator: expected Nds to be a multiple of 128 (could be relaxed)");
    ...
    if (Sds == 16) {
        ...
        correlate_pl_kernel_S16<<<nblocks, nthreads, 0, stream>>>(counts, pl_mask, rfimask,
                                                                  rfimask_fstride, Tout, F, N128);
    } else if (Sds == 128) {
        dim3 nblocks = {1, uint(F), uint(Tout)};

        correlate_pl_kernel_S128<<<nblocks, 256, 0, stream>>>(counts, pl_mask, rfimask,
                                                              rfimask_fstride, N128);
    } else {
        throw runtime_error(
            "launch_pl_1bit_correlator: Currently, only Sds=16 and Sds=128 are implemented."
...
void launch_pl_1bit_correlator(Array<int>& counts, const Array<ulong>& pl_mask,
                               const Array<uint>& rfimask, long Nds, cudaStream_t stream) {
    ...
    if (!rfimask.shape_equals({F, T / 32})) {
        ...
        ss << "launch_pl_1bit_correlator: rfimask.shape (=" << rfimask.shape_str() << ")."
           << " Based on pl_mask.shape (=" << pl_mask.shape_str() << ", expected shape (" << F
           << "," << (T / 32) << ")";
        throw runtime_error(ss.str());
    }
    ...
    if (rfimask.strides[1] != 1)
        throw runtime_error(
            "launch_pl_1bit_correlator: expected inner (time) axis of rfimask to be contiguous");

    launch_pl_1bit_correlator(counts.data, pl_mask.data, rfimask.data, rfimask.strides[0], T, F,
                              Sds, Nds, stream);
}

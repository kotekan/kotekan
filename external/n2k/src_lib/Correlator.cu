...
CorrelatorParams::CorrelatorParams(int nstations_, int nfreq_) :
    nstations(nstations_), nfreq(nfreq_), emat_fstride(nstations / 4), // int32 stride, not bytes
    ...
    ntiles_2d_tot(ntiles_2d_offdiag + ntiles_1d), threadblocks_per_freq(ntiles_2d_tot) {
    assert(nstations > 0);
    assert(nfreq > 0);

    if (nstations % CorrelatorParams::ns_divisor)
        throw runtime_error("n2k: expected nstations(=" + to_str(nstations)
                            + ") to be a multiple of " + to_str(CorrelatorParams::ns_divisor));
}
...
void Correlator::launch(int* vis_out, const int8_t* e_in, const uint* rfimask, int nt_outer,
    ...
    if (nt_inner % CorrelatorParams::nt_divisor)
        throw runtime_error("n2k::Correlator::launch: expected nt_inner(=" + to_str(nt_inner)
                            + ") to be a multiple of " + to_str(CorrelatorParams::nt_divisor));
...
void Correlator::launch(Array<int>& vis_out, const Array<int8_t>& e_in, const Array<uint>& rfimask,
                        int nt_outer, int nt_inner, cudaStream_t stream, bool sync) const {
    ...
    if (!e_in.shape_equals({nt_expected, nfreq, nstat})) {
        stringstream ss;
        ss << "Correlator::launch(nfreq=" << nfreq << ", nt_outer=" << nt_outer
           << ", nt_inner=" << nt_inner << ")"

...
__global__ void sk_kernel(
    ...
    uint* out_rfimask,               // Shape ((T*Nds)/128, F, 128/32), can be NULL
    ...
    int T,   // Number of time samples to process
...
{
    // TODO: Disentangle all the index optimizations below, then use `&` instead of `%` for the ring
    // buffer access
    ...
    for (int s = threadIdx.x; s < S; s += blockDim.x) {
        ...
        bool sf_valid =
            (S0i >= single_feed_S0_min) && (S1f >= mu_min * S0f) && (S1f <= mu_max * S0f);
        ...
        float sigma = interpolate_sigma_gpu(shmem_bsigma_coeffs, x)
                      * sqrtf(y); // note sqrt(y) = N^(-1/2) here
    ...
    if (warpId == 0) {
        ...
        bool rfi_good = fsum_valid && (y >= 1.0f - sk_rfimask_sigmas * sigma)
...
void SkKernel::check_params(const SkKernel::Params& params) {
    ...
    if (single_feed_min_good_frac <= 0.0)
        throw runtime_error(
            "SkKernel::Params::single_feed_min_good_frac was uninitialized or <= 0");
    if (feed_averaged_min_good_frac <= 0.0)
        throw runtime_error(
            "SkKernel::Params::feed_averaged_min_good_frac was uninitialized or <= 0");
    if (mu_min <= 0.0)
    ...
    if (Nds < bias_nmin) {
        stringstream ss;
        ss << "SkKernel::Params::Nds=" << Nds << " was specified, and min allowed value is "
           << bias_nmin
    ...
    if (mu_min < n2k::sk_globals::mu_min - 1.0e-6) {
        stringstream ss;
        ss << "SkKernel::Params::mu_min=" << mu_min << " was specified, and min allowed value is "
           << mu_min << " (= n2k::sk_globals::mu_min), since the SK-interpolation table has not"
    ...
    if (mu_max > n2k::sk_globals::mu_max + 1.0e-6) {
        stringstream ss;
        ss << "SkKernel::Params::mu_max=" << mu_max << " was specified, and max allowed value is "
           << mu_max << " (= n2k::sk_globals::mu_max), since the SK-interpolation table has not"
...
void SkKernel::launch(
    ...
    cudaStream_t stream, bool check_params) const {
    if (check_params)
    ...
    if (dev != this->device)
        throw runtime_error("SkKernel::launch: current CUDA device doesn't match current device "
                            "when SkKernel constructor was called");
    ...
    sk_kernel<<<{1, Bf, Bt}, {32 * Ws, Wf, Wt}, shmem_nbytes, stream>>>(
        out_sk_feed_averaged, out_sk_single_feed, out_rfimask, in_S012,
...
void SkKernel::launch(
    ...
    check_array(out_sk_feed_averaged, "SkKernel::launch", "out_sk_feed_averaged", 3,
                true);                                                  // ndim=3, contiguous=true
    ...
    if (out_sk_feed_averaged.shape[0] != in_S012.shape[0])
        throw runtime_error("SkKernel::launch: inconsistent value of T between "
                            "'out_sk_feed_averaged' and 'in_S012' arrays");
    if (out_sk_feed_averaged.shape[1] != in_S012.shape[1])
        throw runtime_error("SkKernel::launch: inconsistent value of F between "
                            "'out_sk_feed_averaged' and 'in_S012' arrays");
    if (in_S012.shape[3] != in_bf_mask.shape[0])
        throw runtime_error(
            "SkKernel::launch: inconsistent value of S between 'in_S012' and 'in_bf_mask' arrays");
    ...
    if (out_sk_single_feed.data != NULL) {
        check_array(out_sk_single_feed, "SkKernel::launch", "out_sk_single_feed", 4,
                    true); // ndim=4, contiguous=true

        if (!out_sk_single_feed.shape_equals({T, F, 3, S}))
            throw runtime_error(
                "SkKernel::launch: 'out_sk_single_feed' array has wrong shape (expected {T,F,3,S}");
    }

    if (out_rfimask.data != NULL) {
        check_array(out_rfimask, "SkKernel::launch", "out_rfimask", 2,
                    false); // ndim=2, contiguous=false

        if (!out_rfimask.shape_equals({F, (T * params.Nds) / 32}))
            throw runtime_error(
                "SkKernel::launch: 'out_rfimask' array has wrong shape (expected {F,(T*Nds)/32})");
        if (out_rfimask.strides[1] != 1)
            throw runtime_error("SkKernel::launch: expected inner (time) axis of 'out_rfimask' "
                                "array to be contiguous");
    }

    this->launch(out_sk_feed_averaged.data, out_sk_single_feed.data, out_rfimask.data, in_S012.data,
                 in_bf_mask.data, T, F, S, 0, 1 << 30, 0, 1 << 30, 0, 1 << 30, 0, 1 << 30, stream,
                 false); // check_params
}

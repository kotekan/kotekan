...
Correlator::kernel_t get_kernel(int nstations, int nfreq) {
    ...
    if (nstations % CorrelatorParams::ns_divisor)
        throw runtime_error("n2k::Correlator::get_kernel(): expected nstations(="
                            + to_str(nstations) + ") to be a multiple of "
                            + to_str(CorrelatorParams::ns_divisor));
    ...
    stringstream ss;
    ss << "n2k: You have requested (nstations,nfreq)=(" << nstations << "," << nfreq
       << "). This pair of values is not supported."
       << " Sadly, you will need to add 'template_instantiations/kernel_" << nstations << "_"
       << nfreq << ".o' to the Makefile and recompile."

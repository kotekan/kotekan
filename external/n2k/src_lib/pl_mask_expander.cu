...
__global__ void pl_mask_expand_kernel(uint* pl_out, const uint* pl_in, long Tmin_in, long Tsize_in,
                                      long Tmin_out, long Tsize_out, int F, int M, int N) {
...
void launch_pl_mask_expander(ulong* pl_out, const ulong* pl_in, long Tmin_in, long Tsize_in,
                             long Tmin_out, long Tsize_out, long Tout, long Fout, long Sds,
                             cudaStream_t stream) {
    if (!pl_out)
    ...
    pl_mask_expand_kernel<<<nblocks, nthreads, 0, stream>>>(
        (uint*)pl_out, (const uint*)pl_in, Tmin_in, Tsize_in, Tmin_out, Tsize_out, Fout, M, N);
...
void launch_pl_mask_expander(Array<ulong>& pl_out, const Array<ulong>& pl_in, cudaStream_t stream) {
    ...
    if (!pl_in.shape_equals({Tout / 128, (Fout + 3) / 4, Sds})) {
        ...
        ss << "launch_pl_mask_expander: pl_out.shape=" << pl_out.shape_str()
           ...
           << " are inconsistent (expected pl_in.shape=(" << (Tout / 128) << "," << ((Fout + 3) / 4)
           << "," << Sds << "))";
        throw runtime_error(ss.str());
    }

    launch_pl_mask_expander(pl_out.data, pl_in.data, 0, 1L << 32, 0, 1L << 32, Tout, Fout, Sds,
                            stream);
}

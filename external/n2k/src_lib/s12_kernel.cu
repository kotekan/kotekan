...
//   int      out_fstride;           // output stride (minimal value is 2*S), measured in ulong
//   elements
//
...
__global__ void __launch_bounds__(128, 8)
    s12_kernel(ulong* S12, const uint* E, int Tds, int Tmin, int Tsize, int F, int S, int Nds,
               int out_fstride, bool offset_encoded) {
    ...
    // Each thread processes 4 stations, at the following base (time, freq, station_quadruple)
    // indices.
    ...
    int t = tout * Nds;
    int tind = (Tmin + t) & (Tsize - 1);
    E += (F * Q) * tind + Q * freq + sq;
...
void launch_s12_kernel(ulong* S12, const uint8_t* E, long T, long Tmin, long Tsize, long F, long S,
                       long Nds, long out_fstride, bool offset_encoded, cudaStream_t stream) {
    ...
    s12_kernel<<<nblocks, nthreads, 0, stream>>>(S12, (const uint*)E, Tds, Tmin, Tsize, F, S, Nds,
                                                 out_fstride, offset_encoded);

    CUDA_PEEK("s12 kernel launch");
}


void launch_s12_kernel(ulong* S12, const uint8_t* E, long T, long F, long S, long Nds,
                       long out_fstride, bool offset_encoded, cudaStream_t stream) {
    launch_s12_kernel(S12, E, T, 0, 1 << 30, F, S, Nds, out_fstride, offset_encoded, stream);
}


void launch_s12_kernel(Array<ulong>& S12, const Array<uint8_t>& E, long Nds, bool offset_encoded,
                       cudaStream_t stream) {
    // ulong    S12[T/Tds][F][2][S];
    // uint4+4  E[T][F][S];
    ...
    if (S12.shape[0] * Nds != E.shape[0])
        throw runtime_error(
            "launch_s12_kernel(): inconsistent number of time samples in S12 and E arrays");
    if (S12.shape[1] != E.shape[1])
        throw runtime_error(
            "launch_s12_kernel(): inconsistent number of frequency channels in S12 and E arrays");
    if (S12.shape[2] != 2)
        throw runtime_error("launch_s12_kernel(): expected axis 2 of S12 array to have length 2");
    if (S12.shape[3] != E.shape[2])
        throw runtime_error(
            "launch_s12_kernel(): inconsistent number of stations in S12 and E arrays");

    if ((S12.strides[2] != S) || (S12.strides[3] != 1))
        throw runtime_error("launch_s12_kernel(): expected inner two axes (with shape (2,S)) of "
                            "S12 array to be contiguous");

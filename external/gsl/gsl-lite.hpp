...
#define gsl_CPP11_LB                                                                               \
    (gsl_CPP11_OR_GREATER || gsl_COMPILER_MSVC_VERSION >= 14                                       \
     || (gsl_COMPILER_MSVC_VERSION >= 9 && gsl_HAS_CPP0X))
...
#define gsl_CPP11_LD                                                                               \
    (gsl_CPP11_OR_GREATER || gsl_COMPILER_MSVC_VERSION >= 14                                       \
     || (gsl_COMPILER_MSVC_VERSION >= 10 && gsl_HAS_CPP0X))
...
#define gsl_STRINGIFY_(x) #x
    ...
    gsl_api inline gsl_constexpr14 auto fail_fast_assert(bool cond, char const* const message)
        ->void {
    ...
    public:
        gsl_api explicit final_action_return(Action action) : final_action(action) {}
    ...
    public:
        gsl_api explicit final_action_error(Action action) : final_action(action) {}
    ...
    class not_null {
    public:
        gsl_api gsl_constexpr14 not_null(T t) : ptr_(t) {
            Expects(ptr_ != gsl_nullptr);
        }
        gsl_api not_null& operator=(T t) {
            ptr_ = t;
            Expects(ptr_ != gsl_nullptr);
            return *this;
        }
        ...
        gsl_api gsl_constexpr operator T() const {
            return get();
        }
        gsl_api gsl_constexpr T operator->() const {
            return get();
        }
...
#define gsl_ENABLE_IF_INTEGRAL_T(T)
    ...
    public:
...
#if gsl_HAVE_IS_DELETE
        gsl_api gsl_constexpr14 span(reference data_in) : span(&data_in, 1) {}
        ...
        gsl_api gsl_constexpr14 span(U (&arr)[N]) gsl_noexcept : first_(&arr[0]),
                                                                 last_(&arr[0] + N) {}
    ...
    template<class Cont>
    gsl_api inline gsl_constexpr14 auto make_span(Cont const& cont)
        ->span<const typename Cont::value_type> {
    ...
    template<class Ptr>
    span<typename Ptr::element_type> make_span(
    ...
    public:
...
#if gsl_HAVE_NULLPTR
        gsl_api gsl_constexpr basic_string_span(std::nullptr_t ptr) gsl_noexcept : span_(ptr, 0) {}
#endif
        ...
        basic_string_span(std::array<typename detail::remove_const<element_type>::type, N>& arr) :
            span_(remove_z(arr)) {}

        template<std::size_t N>
        gsl_api gsl_constexpr basic_string_span(
            std::array<typename detail::remove_const<element_type>::type, N> const& arr) :
            span_(remove_z(arr)) {}
        ...
        template<
            ...
            class = typename std::enable_if<
                !detail::is_std_array<Cont>::value && !detail::is_basic_string_span<Cont>::value
                && std::is_convertible<typename Cont::pointer, pointer>::value
                && std::is_convertible<typename Cont::pointer,
                                       decltype(std::declval<Cont>().data())>::value>::type>
        gsl_api gsl_constexpr basic_string_span(Cont& cont) : span_((cont)) {}
        ...
        template<
            ...
            class = typename std::enable_if<
                ...
                && std::is_convertible<typename Cont::pointer,
                                       decltype(std::declval<Cont>().data())>::value>::type>
        gsl_api gsl_constexpr basic_string_span(Cont const& cont) : span_((cont)) {}
        ...
        template<class U
                 ...
                 class = typename std::enable_if<std::is_convertible<
                     typename basic_string_span<U>::pointer, pointer>::value>::type
#endif
                 >
        ...
        gsl_api gsl_constexpr
        basic_string_span(std::basic_string<typename detail::remove_const<element_type>::type,
    ...
    private:
        ...
        remove_z(std::array<typename detail::remove_const<element_type>::type, N>& arr) {
            return remove_z(&arr[0], narrow_cast<std::size_t>(N));
        }

        template<size_t N>
        gsl_api static gsl_constexpr14 span_type
        remove_z(std::array<typename detail::remove_const<element_type>::type, N> const& arr) {
            return remove_z(&arr[0], narrow_cast<std::size_t>(N));
        }
#endif
...
gsl_api inline std::basic_string< typename std::remove_const<T>::type > to_string( basic_string_span<T> spn )
{
    ...
    template<class Cont>
    gsl_api inline span<typename std::remove_pointer<typename Cont::pointer>::type> ensure_z(
        Cont & cont) {

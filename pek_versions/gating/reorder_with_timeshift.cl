...
#define NUM_ELEMENTS_DIV_DATASIZE   (NUM_ELEMENTS_DIV_4*4/NUM_COMPLEX_PAIRS)    //DATASIZE = NUM_COMPLEX_PAIRS here since each complex pair = 1 B
...
__kernel void reorder_input_data_with_timeshift(__global unsigned int  *input_data,
                                                ...
                                                __global const    int  *timeshift_map //NOTE: the map for timeshifting is based on the remapped addressing
                                                ) {
    ...
    for (int i = 0; i < NUM_COMPLEX_PAIRS; i++){
        ...
        if (output_address >0 && output_address < (NUM_ELEMENTS_DIV_DATASIZE*NUM_COMPLEX_PAIRS*NUM_FREQUENCIES*NUM_TIMESTEPS))

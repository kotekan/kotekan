...
void corr ( __global const uint *packed,
            ...
            __global const uint *num_time_bins,
            __global const uint *offset_remainder,
            __global const uint *delta_time_bin,
...
//#pragma unroll 8
    ...
    for (int i = 0; i < gating_group_count; i ++){
        ...
        //get a time in our current time units

        addr_o = addr_o % (num_time_bins[i]*delta_time_bin[i]); //wrap the time-with-offset to a single zone of the proper period
        //if (get_global_id(0)==0 && get_global_id(1) == 0 && get_global_id(2) == 0)
        //    printf("%d %d %d\n", addr_o, delta_time_bin[i], addr_o/delta_time_bin[i]);
        addr_o = addr_o / delta_time_bin[i];//check what bin it would fall in
        addr_o = (addr_o >= bin_cut[i]) ? 1 : 0;

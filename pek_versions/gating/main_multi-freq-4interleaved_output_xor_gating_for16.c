...
#define OPENCL_FILENAME_1               "test0xB_simplifiedfor16_multifreq_gating_attempts_to_optimize_non-vectorized.cl"
#define OPENCL_FILENAME_2               "offsetAccumulator_gating.cl"
#define OPENCL_FILENAME_2b              "offsetAccumulatorXOR_gating.cl"
#define OPENCL_FILENAME_3               "preseed_multifreq_gating.cl"
...
#define NUM_ELEM                        32//32u //minimum needs to be 32 for the kernels
#define NUM_FREQ                        64//63u
#define ACTUAL_NUM_ELEM                 16//16u
#define ACTUAL_NUM_FREQ                 128//126u //product of ACTUAL_NUM_ELEM*ACTUAL_NUM_FREQ should equal NUM_ELEM*NUM_FREQ
...
#define NUM_DATA_SETS                   128

#define NUM_TIMESAMPLES                 128u*8u*256u //
#define NUM_REPEATS_GPU                 1000u

#define GATE_PERIOD_IN_10ns_UNITS       65536//100000 // i.e. 100000 x 10ns = 1000000 ns = 1 ms
// ////////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char ** argv) {
    ...
    uint true_time_offset = 0; //can make this whatever you like (should start off with a value smaller than GATE_PERIOD_IN_10ns_UNITS*NUM_DATA_SETS)
    uint true_time_increment = (NUM_TIMESAMPLES) * 256; //time in units of 10 ns (the base time is 2048/800MHz = 2560 ns or 256 (in units of 10 ns)
...
//     }
    ...
    printf("Gate Size (Accumulate Kernel arg 4): %d\n", gate_size);

    clSetKernelArg(preseed_kernel, 2, sizeof(id_x_map), (void*) &id_x_map); //this should maybe be sizeof(void *)?
    printf("ID_X_MAP (Preseed Kernel arg 2 (not printed))\n");
    clSetKernelArg(preseed_kernel, 3, sizeof(id_y_map), (void*) &id_y_map);
    printf("ID_Y_MAP (Preseed Kernel arg 3 (not printed))\n");
    clSetKernelArg(preseed_kernel, 4, 64* sizeof(cl_uint), NULL);
    printf("Local Memory allocation 64 ints big (Preseed Kernel arg 4)\n");
    clSetKernelArg(preseed_kernel, 5, 64* sizeof(cl_uint), NULL);
    printf("Local Memory allocation 64 ints big (Preseed Kernel arg 5)\n");
    ...
    clSetKernelArg(corr_kernel, 2, 8*8*4 * sizeof(cl_uint), NULL);
    printf("Local Memory allocation 256 ints big (1024B) (Corr Kernel arg 2)\n");
    clSetKernelArg(corr_kernel, 4, sizeof(cl_uint), &gate_size);
    printf("Gate Size (Corr Kernel arg 4): %d\n", gate_size);
    ...
    printf("GWS accum: X: %d, Y: %d, Z: %d\n",(int)gws_accum[0],(int)gws_accum[1],(int)gws_accum[2]);
    size_t lws_accum[3]={64, 1, 1};
    printf("LWS accum: X: %d, Y: %d, Z: %d\n",(int)lws_accum[0],(int)lws_accum[1],(int)lws_accum[2]);

    size_t gws_preseed[3]={8*NUM_DATA_SETS, 8*NUM_FREQ, num_blocks};
    printf("GWS preseed: X: %d, Y: %d, Z: %d\n",(int)gws_preseed[0],(int)gws_preseed[1],(int)gws_preseed[2]);
    size_t lws_preseed[3]={8, 8, 1};
    printf("LWS preseed: X: %d, Y: %d, Z: %d\n",(int)lws_preseed[0],(int)lws_preseed[1],(int)lws_preseed[2]);

    size_t gws_corr[3]={8,8*NUM_FREQ,num_blocks*n_cAccum}; //global work size array
    printf("GWS Corr: X: %d, Y: %d, Z: %d\n",(int)gws_corr[0],(int)gws_corr[1],(int)gws_corr[2]);
    size_t lws_corr[3]={8,8,1}; //local work size array
    printf("LWS Corr: X: %d, Y: %d, Z: %d\n",(int)lws_corr[0],(int)lws_corr[1],(int)lws_corr[2]);
...
//                     printf(".");
        ...
        if (lastWriteEvent[kernelStageIndex] !=0 && i <= nkern){//insert additional steps for processing here
            ...
            true_time_offset = 0;//true_time_offset % (GATE_PERIOD_IN_10ns_UNITS*NUM_DATA_SETS); // current_time%(time_per_bin*NUM_DATA_SETS)
            err |= clSetKernelArg(offsetAccumulate_kernel,
                                  3,
                                  sizeof(uint),
                                  &true_time_offset);
            ...
            if (i <2){
                printf("Set Accumulate Kernel Args 0, 1, 2 (memory)\n");
                printf("Accumulate Kernel Arg 3: Time ofset: %d \n", true_time_offset); 
            }
            ...
            if (i <2){
                printf("Set Preseed Kernel Args 0, 1, 6 (memory)\n");
            }
            ...
            if (i <2){
                printf("Set Correlation Kernel Args 0, 1 (memory)\n");
                printf("Correlation Kernel Arg 3: Time ofset: %d \n", true_time_offset); 
            }
        ...
        true_time_offset += true_time_increment;
    }
...
//    err = cpu_data_generate_and_correlate(NUM_TIMESAMPLES, ACTUAL_NUM_FREQ, ACTUAL_NUM_ELEM, correlated_CPU);
    if (UPPER_TRIANGLE){
        err = cpu_data_generate_and_correlate_upper_triangle_only_gated(NUM_TIMESAMPLES, ACTUAL_NUM_FREQ, ACTUAL_NUM_ELEM, NUM_DATA_SETS, correlated_CPU,0,GATE_PERIOD_IN_10ns_UNITS);
    }
    else{ 
        err = cpu_data_generate_and_correlate_gated(NUM_TIMESAMPLES, ACTUAL_NUM_FREQ, ACTUAL_NUM_ELEM, NUM_DATA_SETS, correlated_CPU, 0, GATE_PERIOD_IN_10ns_UNITS);
    }

...
//    for (int i = 0; i < gating_group_count; i ++){
        uint addr_o = (offset_remainder[0] + TIME_DIV_256_corr*TIME_FOR_256_TIMESTEPS);//for multi-dataset was simply this: (get_group_id(0) * NUM_FREQUENCIES * NUM_BLOCKS_x_2048); //dataset/bin offset


        addr_o = addr_o % (num_time_bins[0]*delta_time_bin[0]);
        //if (get_global_id(0)==0 && get_global_id(1) == 0 && get_global_id(2) == 0)
        //    printf("%d %d %d\n", addr_o, delta_time_bin[i], addr_o/delta_time_bin[i]);
        addr_o = addr_o / delta_time_bin[0];
        addr_o = (addr_o >= bin_cut[0]) ? 1 : 0;
        
        addr_o *= NUM_FREQUENCIES * num_blocks_x_2048;
        addr_o += ((block_id_x_2048) + (LOCAL_Y * 256u) + (LOCAL_X * 8u)) + (FREQUENCY_BAND * num_blocks_x_2048);
        //addr_o += i*2*NUM_FREQUENCIES*num_blocks_x_2048;

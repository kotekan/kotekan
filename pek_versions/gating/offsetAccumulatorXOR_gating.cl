                                        ...
                                        const uint delta_time_bin)
{
    ...
    if ((address * 8u + 7u) < ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2u){ //check to see if the output address falls in a useful range (i.e. < Num_Elements x Num_Freq)
        ...
        //8 output values
        address = (offset_remainder + TIME_DIV_256*TIME_FOR_256_TIMESTEPS);//for multi-dataset was simply this: (get_group_id(0) * NUM_FREQUENCIES * NUM_BLOCKS_x_2048); //dataset/bin offset
        address = address % (NUM_DATASETS*delta_time_bin);
        address = address / delta_time_bin;
        if (get_global_id(1)==0 && get_local_id(0)==0)
            atomic_inc(&counters[address]); //only do this once per time step
        address *= ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2;

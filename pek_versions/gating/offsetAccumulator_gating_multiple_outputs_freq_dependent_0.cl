                                        ...
                                        __global const uint *bin_zone_mask) //mask for output zones (frequency and gating group dependent)
{
    ...
    uint4   temp;
    uint    freq_band = (get_group_id(1)*256u)/ACTUAL_NUM_ELEMENTS; //needed because the method to split this for parallelism is different from the other kernels.
...
//         for (int i = 0; i < trick_the_compiler; i++){
    ...
    //    int i = 0;
        for (int i = 0; i < NUM_GATINGGROUPS; i++){
            ...
            if (ELEMENT_AND_FREQUENCY_GROUP==0 && get_local_id(0)==0) //so that the counters only get incremented 1 time per group
    ...
    //        address = (get_local_id(0) + ELEMENT_AND_FREQUENCY_GROUP*64u)*8u; //([0,504] + [0,M*N/256)*512)
            address += i*ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*NUM_ZONES*2; //

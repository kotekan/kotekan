...
__kernel void offsetAccumulateElements (__global const uint *inputData,
                                        ...
                                        __global const uint *bin_zone_mask) //mask for output zones (frequency and gating group dependent)
{
    uint    data;
    uint4   dataExpanded = (uint4)(0u,0u,0u,0u);
    uint4   temp;
    uint    freq_band = (get_group_id(1)*256u)/ACTUAL_NUM_ELEMENTS;
    //needed because the method to split this for parallelism is different from the other kernels.
...
//         for (int i = 0; i < trick_the_compiler; i++){
        for (int i = 0; i < NUM_TIMESTEPS_LOCAL; i++){ //accumulate NUM_TIMESTEPS_LOCAL terms
        ...
        //8 output values
    //    int i = 0;
        if (ACTUAL_NUM_FREQUENCIES*ACTUAL_NUM_ELEMENTS <=256){
            int local_freq_band = (get_local_id(0)*4)/ACTUAL_NUM_ELEMENTS;//ranges [0,7] for a 32 element case
            freq_band += local_freq_band;
            for (int i = 0; i < NUM_GATINGGROUPS; i++){

                //reuse 'address' for output address now
                address = (offset_remainder[i] + TIME_DIV_256*TIME_FOR_256_TIMESTEPS);
                address = address % (NUM_TIME_BINS*delta_time_bin[i]); //reduce the number so that the integer division in the next line picks out the proper time bin to look at
...
//                address = bin_zone_mask[(i*ACTUAL_NUM_FREQUENCIES + freq_band)*NUM_TIME_BINS + address];
                address = bin_zone_mask[(i*NUM_FREQUENCIES + freq_band)*NUM_TIME_BINS + address];

                //TODO figure what is wrong with frequency band counters
                if (((get_local_id(0)*4)%ACTUAL_NUM_ELEMENTS)==0){ //so that the counters only get incremented 1 time per group
//                    atomic_inc(&counters[i*NUM_ZONES*ACTUAL_NUM_FREQUENCIES + address*ACTUAL_NUM_FREQUENCIES + freq_band]);
                    atomic_inc(&counters[(i*NUM_ZONES + address)*ACTUAL_NUM_FREQUENCIES + freq_band]);

                    //printf("%6d, %6d, %6d, %6d, %6d \n",get_local_id(0), get_group_id(1), freq_band, local_freq_band, address);
                }
        ...
        //        address = (get_local_id(0) + ELEMENT_AND_FREQUENCY_GROUP*64u)*8u; //([0,504] + [0,M*N/256)*512)
                address += i*NUM_ZONES*ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2; //
            ...
            for (int i = 0; i < NUM_GATINGGROUPS; i++){
                ...
                address = bin_zone_mask[(i*NUM_FREQUENCIES + freq_band)*NUM_TIME_BINS + address];
                //TODO figure what is wrong with frequency band counters
                if (get_local_id(0)==0) //so that the counters only get incremented 1 time per group
        //            atomic_inc(&counters[0]);
                    atomic_inc(&counters[(i*NUM_ZONES + address)*NUM_FREQUENCIES+freq_band]);
                    //atomic_inc(&counters[(i*NUM_FREQUENCIES + freq_band)*NUM_ZONES + address]); //each gating group has multiple frequency bands and each frequency band has a number of zones//increment the counter for a particular dataset (only once for a particular bin) //these are correct--have been checked Apr 8/14
                address *= ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2;//for multi-dataset was simply this: get_group_id(0) * ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2;
                address += (get_local_id(0) + ELEMENT_AND_FREQUENCY_GROUP*64u)*8u; //([0,504] + [0,M*N/256)*512)
        //        address = (get_local_id(0) + ELEMENT_AND_FREQUENCY_GROUP*64u)*8u; //([0,504] + [0,M*N/256)*512)
                address += i*NUM_ZONES*ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2; //

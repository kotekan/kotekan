                                        ...
                                        __global const uint *bin_cut) //number of bins to keep in the 'on' section (entry values must be >= 1)
{
    ...
    uint8   dataExpanded = (uint8)(0u,0u,0u,0u,0u,0u,0u,0u);
    uint8   temp;
...
#pragma unroll 1
        for (int i = 0; i < NUM_TIMESTEPS_LOCAL; i++){
            data[get_local_id(0)] = inputData[address]; //should be a coalesced load with local work group
            ...
            temp.s0 = ((data[get_local_id(0)] & 0x0000000f) >>  0u);
            temp.s1 = ((data[get_local_id(0)] & 0x000000f0) >>  4u);//
            
            temp.s2 = ((data[get_local_id(0)] & 0x00000f00) >>  8u);
            temp.s3 = ((data[get_local_id(0)] & 0x0000f000) >> 12u);
            
            temp.s4 = ((data[get_local_id(0)] & 0x000f0000) >> 16u);
            temp.s5 = ((data[get_local_id(0)] & 0x00f00000) >> 20u);
            
            temp.s6 = ((data[get_local_id(0)] & 0x0f000000) >> 24u);
            temp.s7 = ((data[get_local_id(0)] & 0xf0000000) >> 28u);
            //accumulate
            dataExpanded += temp;
...
//             atomic_add(&outputData[address+7u], (dataExpanded.s3&0x0000ffff) );  //imaginary
            atomic_add(&outputData[address+0u], (dataExpanded.s1) );         //real value
            atomic_add(&outputData[address+1u], (dataExpanded.s0) );  //imaginary
            atomic_add(&outputData[address+2u], (dataExpanded.s3) );         //real value
            atomic_add(&outputData[address+3u], (dataExpanded.s2) );  //imaginary
            atomic_add(&outputData[address+4u], (dataExpanded.s5) );         //real value
            atomic_add(&outputData[address+5u], (dataExpanded.s4) );  //imaginary
            atomic_add(&outputData[address+6u], (dataExpanded.s7) );         //real value
            atomic_add(&outputData[address+7u], (dataExpanded.s6) );  //imaginary

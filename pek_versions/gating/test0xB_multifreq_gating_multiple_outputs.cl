...
//non-vectorized version of test0xB_multifreq_gatingb.cl
//it was found that there were nominal gains in speed when using the non-vectorized version (vectors of 8 elements were the slowest, and as vector length
//decreased, execution time tended to reduce as well.

//some readability is lost when vectors are used since the code expands, but ultimately we are after the most efficient routine.
...
void corr ( __global const uint *packed,
            ...
            const unsigned int offset_remainder2,
            const unsigned int delta_time_bin2,
            const unsigned int offset_remainder3,
            const unsigned int delta_time_bin3)
{
    ...
    const uint block_id_x_2048 = BLOCK_ID_corr * 2048u;//(get_global_id(2) % NUM_BLOCKS) * 2048u;
    const uint num_blocks_x_2048 = NUM_BLOCKS * 2048u;
...
//#pragma unroll 8
    ...
    uint addr_o = (offset_remainder + TIME_DIV_256_corr*TIME_FOR_256_TIMESTEPS);//for multi-dataset was simply this: (get_group_id(0) * NUM_FREQUENCIES * NUM_BLOCKS_x_2048); //dataset/bin offset


    addr_o = addr_o % (NUM_DATASETS*delta_time_bin);
    addr_o = addr_o / delta_time_bin;
    addr_o *= NUM_FREQUENCIES * num_blocks_x_2048;
    addr_o += ((block_id_x_2048) + (LOCAL_Y * 256u) + (LOCAL_X * 8u)) + (FREQUENCY_BAND * num_blocks_x_2048);
    ...
    ///
    addr_o = (offset_remainder2 + TIME_DIV_256_corr*TIME_FOR_256_TIMESTEPS);//for multi-dataset was simply this: (get_group_id(0) * NUM_FREQUENCIES * NUM_BLOCKS_x_2048); //dataset/bin offset


    addr_o = addr_o % (NUM_DATASETS*delta_time_bin2);
    addr_o = addr_o / delta_time_bin2;
    addr_o *= NUM_FREQUENCIES * num_blocks_x_2048;
    addr_o += ((block_id_x_2048) + (LOCAL_Y * 256u) + (LOCAL_X * 8u)) + (FREQUENCY_BAND * num_blocks_x_2048);
    addr_o += NUM_DATASETS*NUM_FREQUENCIES*num_blocks_x_2048;
    //uint addr_o = ((BLOCK_ID * 2048u) + (LOCAL_Y * 256u) + (LOCAL_X * 8u)) + (FREQUENCY_BAND * NUM_BLOCKS_x_2048);
    ...
    ///
    addr_o = (offset_remainder3 + TIME_DIV_256_corr*TIME_FOR_256_TIMESTEPS);//for multi-dataset was simply this: (get_group_id(0) * NUM_FREQUENCIES * NUM_BLOCKS_x_2048); //dataset/bin offset


    addr_o = addr_o % (NUM_DATASETS*delta_time_bin3);
    addr_o = addr_o / delta_time_bin3;
    addr_o *= NUM_FREQUENCIES * num_blocks_x_2048;
    addr_o += ((block_id_x_2048) + (LOCAL_Y * 256u) + (LOCAL_X * 8u)) + (FREQUENCY_BAND * num_blocks_x_2048);
    addr_o += NUM_DATASETS*NUM_FREQUENCIES*num_blocks_x_2048*2;
    //uint addr_o = ((BLOCK_ID * 2048u) + (LOCAL_Y * 256u) + (LOCAL_X * 8u)) + (FREQUENCY_BAND * NUM_BLOCKS_x_2048);

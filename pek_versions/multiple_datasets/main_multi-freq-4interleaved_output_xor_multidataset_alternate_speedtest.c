// note: this alternate version allows the user to specify how many time steps can be integrated (as an integer multiple of 256 steps)
...
#define OPENCL_FILENAME_1               "test0xB_multifreq_multiple_stripped2.cl" //"test0xB_multifreq_multiple.cl"
#define OPENCL_FILENAME_2               "offsetAccumulator_multiple_b.cl"
#define OPENCL_FILENAME_2b              "offsetAccumulatorXOR_multiple_b.cl"
...
#define NUM_ELEM                        256//32//32u //minimum needs to be 32 for the kernels
#define NUM_FREQ                        1//64//63u
#define ACTUAL_NUM_ELEM                 256//16//16u
#define ACTUAL_NUM_FREQ                 1//128//126u //product of ACTUAL_NUM_ELEM*ACTUAL_NUM_FREQ should equal NUM_ELEM*NUM_FREQ
#define HDF5_FREQ                       1//1024
#define UPPER_TRIANGLE                  1
#define INTERLEAVED                     0
#define XOR_MANUAL                      0
#define TIMER_FOR_PROCESSING_ONLY       1

#define NUM_DATA_SETS                   1//128

#define NUM_TIMESAMPLES                 16u*4u*128u*256u //
...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < N_QUEUES; i++){
        queue[i] = clCreateCommandQueue( context, deviceID[dev_number], 0, &err );
...
// <---------------------------------------------------
    ...
    size_t gws_accum[3]={64*NUM_DATA_SETS, (int)ceil(NUM_ELEM*NUM_FREQ/256.0),NUM_TIMESAMPLES/BASE_TIMESAMPLES_INT_ACCUM}; //1024 is the number of iterations performed in the kernel--hardcoded, so if the number in the kernel changes, change here as well
    ...
    if (TIMER_FOR_PROCESSING_ONLY){
        ...
        err = clFlush(queue[0]);
        if (err){
            printf("Error in flushing transfer to device memory.\n");
            exit(err);
        }
    }
    ///////////////////////////////////////////////////////////////////////////////
...
//                     printf(".");
        ...
        //if (lastWriteEvent[kernelStageIndex] !=0 && i <= nkern){//insert additional steps for processing here
            ...
            err = clEnqueueNDRangeKernel(queue[1],
                                         ...
                                         lws_accum,
                                         0,
                                         NULL,//&lastWriteEvent[kernelStageIndex], //  make sure data is present, first
                                         NULL);//&offsetAccumulateEvent);
            if (err){
            ...
            err = clEnqueueNDRangeKernel(queue[1],
                                         ...
                                         lws_corr,
                                         0,//1,
                                         NULL,//&lastWriteEvent[kernelStageIndex],//dependent on previous step so don't use &lastWriteEvent[kernelStageIndex],
                                         NULL);//&lastKernelEvent[kernelStageIndex]);
            if (err){
...
//
 ...
 //printf("HELLO\n");
    ...
    //-------------------------------------------------------------- 
    //-------------------------------------------------------------- 
    double unpack_Rate =(1.0*NUM_TIMESAMPLES*ACTUAL_NUM_FREQ*nkern*NUM_DATA_SETS) /cputime/1000.0 ;// /cputime/1000.0;
    ...
    printf("    [Theoretical max: @%.1f TFLOPS, %.1f kHz; %2.0f%% efficiency]\n", card_tflops,
                                    ...
                                    100.*NUM_TIMESAMPLES*ACTUAL_NUM_FREQ*nkern*NUM_DATA_SETS/cputime / ((card_tflops*1e12) / (ACTUAL_NUM_ELEM/2.*(ACTUAL_NUM_ELEM+1.) * 2. * 2.)));
    if (ACTUAL_NUM_ELEM == 16){
        printf("    [Algorithm max:   @%.1f TFLOPS, %.1f kHz; %2.0f%% efficiency]\n", card_tflops,
                                    card_tflops*1e12 / (num_blocks * 16 * 16 * 2. * 2.) / 1e3,
                                    100.*NUM_TIMESAMPLES*ACTUAL_NUM_FREQ*nkern*NUM_DATA_SETS/cputime / ((card_tflops*1e12) / (num_blocks * 16 * 16 * 2. * 2.)));
    }
    else{
        printf("    [Algorithm max:   @%.1f TFLOPS, %.1f kHz; %2.0f%% efficiency]\n", card_tflops,
                                    card_tflops*1e12 / (num_blocks * size1_block * size1_block * 2. * 2.) / 1e3,
                                    100.*NUM_TIMESAMPLES*ACTUAL_NUM_FREQ*nkern*NUM_DATA_SETS/cputime / ((card_tflops*1e12) / (num_blocks * size1_block * size1_block * 2. * 2.)));
...
//     printf("Full Corr: %4.2fs on CPU (%.2f kHz)\n",cputime,NUM_TIMESAMPLES/cputime/1e3);
    printf("Use as a speed/power test for the kernel only.  This program is not to be used for correctness in its current state\n");
    err = munlockall();

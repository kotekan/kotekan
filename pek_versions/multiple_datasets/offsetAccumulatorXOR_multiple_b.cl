...
//#define NUMBER_OF_DATA_GROUPS       8u //just as an example
#define NUM_TIMESTEPS_LOCAL         BASE_TIMESAMPLES_INT_ACCUM //1024u //if you change this, check the gws_accum in the main program, too! (also note that the max possible value here is 4096)
...
__kernel void offsetAccumulateElements (__global uint *inputData,
    ...
    if (address * 8u + 7u < ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2u){ //check to see if the output address falls in a useful range (i.e. < Num_Elements x Num_Freq)
        address += get_group_id(0)*OFFSET_FOR_1_TIMESTEP*NUM_TIMESAMPLES + get_group_id(2)*NUM_TIMESTEPS_LOCAL*OFFSET_FOR_1_TIMESTEP;

        for (int i = 0; i < NUM_TIMESTEPS_LOCAL; i++){
            data = inputData[address]; //should be a coalesced load with local work group
            data = data ^ 0x88888888; // ^ xor operator
            inputData[address] = data; //saves it back to the array for next step
        ...
        //8 output values
        address = get_group_id(0)*ACTUAL_NUM_ELEMENTS*ACTUAL_NUM_FREQUENCIES*2+(get_local_id(0) + get_group_id(1)*64u)*8u; //offset_for_data_group+([0,504] + [0,M*N/256)*512)

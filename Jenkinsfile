pipeline {
  ...
  stages {
      stage ('Parallel stage for iwyu & rest') {
          parallel {
            stage('include-what-you-use') {
              steps {
                sh '''mkdir -p iwyu-build
                      cd iwyu-build
                      ...
                      iwyu_tool -j 8 -p . -- -Xiwyu --no_fwd_decls -Xiwyu --mapping_file=iwyu.kotekan.imp \
                      -Xiwyu --max_line_length=100 > iwyu.out
                      cat iwyu.out

...
- ``USE_DPDK``: Control DPDK support. Values: ``ON`` (default, enable if libdpdk >= 19.11 is available via pkg-config) or ``OFF`` (disable).
...
- Developer note: When introducing a new CMake option, update this page, the configure
  summary (``cmake/Summary.cmake``), and the version metadata (``lib/version/version.c.in``).

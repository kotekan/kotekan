...
def subset_data(tmpdir_factory):
    ...
    try:
      ...
      dump_buffer_gen = kotekan_runner.DumpVisBuffer(
              str(tmpdir))
      ...
      fakevis_buffer_subset = kotekan_runner.FakeVisBuffer(
          num_frames=params['total_frames'],
          mode=params['mode'],
          freq_ids=params['freq_ids'],
          use_dataset_manager=True,
      ...
      dump_buffer_subset = kotekan_runner.DumpVisBuffer(
          str(tmpdir))
      ...
      fakevis_buffer_split = kotekan_runner.FakeVisBuffer(
          num_frames=params['total_frames'],
          mode=params['mode'],
      ...
      dump_buffer_split_lower = kotekan_runner.DumpVisBuffer(
          str(tmpdir))
      dump_buffer_split_higher = kotekan_runner.DumpVisBuffer(
          str(tmpdir))
      ...
      write_buffer_lower = kotekan_runner.VisWriterBuffer(
              str(tmpdir), 'raw', None,
      ...
      # the writer is not given the subset list, it get's it through the broker
      write_buffer_higher = kotekan_runner.VisWriterBuffer(
              str(tmpdir), 'raw', None,
              extra_config={'use_dataset_manager': True})
...
def test_subset_broker(subset_data):
    ...
    assert len(data_write_lower) == 16
    assert len(data_write_higher) == 16

    # split < 512
    for f in data_write_lower:
        assert len(f) == len(subset_lower)

    counts = [ 0 ] * len(subset_lower)
    for t in range(params['total_frames']):
      for f in range(len(subset_lower)):
        # get freq ids from fakeVis
        fid = int(data_write_lower[t][f].vis[2].real)
        assert fid in subset_lower
        # Check the order
    ...
    # split > 512
    for f in data_write_higher:
        assert len(f) == len(subset_higher)

    counts = [ 0 ] * len(subset_higher)
    for t in range(params['total_frames']):
      for f in range(len(subset_higher)):
        # get freq ids from fakeVis
        fid = int(data_write_higher[t][f].vis[2].real)
        assert fid in subset_higher
        # Check the order

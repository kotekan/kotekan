...
// The CHORD pathfinder crate wiring: element_reorder[input] = output, mapping
// element = source_id * 8 + lane onto CHORDBeamformer order, pol * 64 + dish,
// with dish numbered grid row-major. See config/chord_pathfinder.j2.
static std::vector<uint32_t> chord_pathfinder_reorder() {
    std::vector<uint32_t> reorder(num_elements);
    for (uint32_t source_id = 0; source_id < 16; source_id++) {
        for (uint32_t lane = 0; lane < 8; lane++) {
            const uint32_t pol = source_id % 2;
            const uint32_t pair = source_id / 2;
            const uint32_t grid_x = 4 * (pair % 2) + (lane % 4);
            const uint32_t grid_y = 2 * (pair / 2) + (lane / 4);
            reorder[source_id * 8 + lane] = pol * 64 + (grid_y * 8 + grid_x);
        }
    }
    return reorder;
}

// A permutation that moves no aligned pair of elements together, so it can only be
// applied at byte granularity.
static std::vector<uint32_t> reversed_reorder() {
    std::vector<uint32_t> reorder(num_elements);
    for (uint32_t e = 0; e < num_elements; e++)
        reorder[e] = num_elements - 1 - e;
    return reorder;
}

// Lets the invalid-table test catch the stage's FATAL_ERROR, which calls
// exit_kotekan and raises SIGTERM before throwing FatalError.
static kotekan_test_logging::SigtermGuard g_sigterm_guard;

static Config make_test_config(const std::string& stage_name, const std::vector<uint32_t>& reorder,
                               bool disable_avx512) {
    json cfg;
    cfg["type"] = "config";
    cfg["log_level"] = "WARN";
...
static std::vector<uint8_t> expected_output(const std::vector<uint32_t>& reorder,
                                            const std::vector<uint32_t>& live_boards,
                                            bool packet_loss) {
    std::vector<uint32_t> source(num_elements);
    if (reorder.empty()) {
        for (uint32_t e = 0; e < num_elements; e++)
            source[e] = e;
    } else {
        for (uint32_t in_e = 0; in_e < num_elements; in_e++)
            source[reorder[in_e]] = in_e;
    }
    ...
    for (uint32_t time = 0; time < timesamples_per_frame; time++) {
        for (uint32_t freq = 0; freq < num_local_freq; freq++) {
            for (uint32_t out_e = 0; out_e < num_elements; out_e++) {
                ...
                const uint32_t src = source[out_e];
...
static void run_case(const std::string& name, const std::vector<uint32_t>& reorder,
                     const std::vector<uint32_t>& live_boards, bool packet_loss,
                     bool disable_avx512) {
    std::cout << "  " << name << (disable_avx512 ? " [scalar]" : " [avx512]") << "\n";

    const std::string stage_name = "/test_transpose";
    Config config = make_test_config(stage_name, reorder, disable_avx512);
    ...
    check_frame(out, expected_output(reorder, live_boards, packet_loss));
...
BOOST_AUTO_TEST_CASE(test_no_reorder) {
    std::cout << "Testing TransposeBasebandArray: no element_reorder...\n";
    for (bool scalar : {false, true})
        run_case("identity", {}, all_boards, false, scalar);
    std::cout << "Success.\n";
}

BOOST_AUTO_TEST_CASE(test_chord_pathfinder_reorder) {
    std::cout << "Testing TransposeBasebandArray: CHORD pathfinder element_reorder...\n";
    for (bool scalar : {false, true})
        run_case("all boards wired", chord_pathfinder_reorder(), all_boards, false, scalar);
    std::cout << "Success.\n";
}

// Unwritten source elements must reach the output as 0x88, wherever the table sends them.
BOOST_AUTO_TEST_CASE(test_unwired_boards_are_encoded_zero) {
    ...
    for (bool scalar : {false, true})
        run_case("4 of 16 boards live", chord_pathfinder_reorder(), live_boards, false, scalar);
    std::cout << "Success.\n";
}

// A permutation with no structure at all, to confirm nothing depends on the CHORD map's
// happening to move aligned groups of 4 elements together.
BOOST_AUTO_TEST_CASE(test_byte_granular_reorder) {
    std::cout << "Testing TransposeBasebandArray: fully byte-granular element_reorder...\n";
    for (bool scalar : {false, true})
        run_case("reversed", reversed_reorder(), all_boards, false, scalar);
    std::cout << "Success.\n";
}
...
BOOST_AUTO_TEST_CASE(test_packet_loss_fill) {
    ...
    for (bool scalar : {false, true})
        run_case("all blocks lost", chord_pathfinder_reorder(), all_boards, true, scalar);
    std::cout << "Success.\n";
}

// A table that is not a permutation would duplicate feeds and drop others silently.
BOOST_AUTO_TEST_CASE(test_invalid_reorder_rejected) {
    std::cout << "Testing TransposeBasebandArray: invalid element_reorder is rejected...\n";

    const std::string stage_name = "/test_transpose";

    std::vector<uint32_t> too_short(num_elements - 1, 0);

    std::vector<uint32_t> out_of_range = chord_pathfinder_reorder();
    out_of_range[0] = num_elements;

    std::vector<uint32_t> duplicated = chord_pathfinder_reorder();
    ...
    for (auto& c : cases) {
        std::cout << "  " << c.what << "\n";
        Config config = make_test_config(stage_name, c.reorder, false);
        TestSetup setup(config);
        bufferContainer bc;
        bc.add_buffer("in_buf", setup.in_buf.get());
        bc.add_buffer("out_buf", setup.out_buf.get());
        bc.add_buffer("pl_mask_buf", setup.pl_mask_buf.get());
        BOOST_CHECK_THROW(TransposeBasebandArray(config, stage_name, bc), FatalError);
    }

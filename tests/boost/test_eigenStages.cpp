...
// erms should be small and non-negative: the stage reports the residual RMS only when the
// decomposition converged, and a negative value (-eps_eval, or -FLT_MAX for a fully failed
// decomposition) otherwise, so a negative result here is a convergence failure, not a small
// residual.

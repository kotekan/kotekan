...
def scrape_board_id(string):
    ...
    if len(chunks) == 3:
        ...
        return int(
            chunks[4][0:-3]
        )  # board_id is non-zero in 16-element mode if more than one FPGA is used.
    else:
...
def test_8_multifreq(tmpdir_factory):
    ...
    ), f"Incorrect number ({len(dump_files)}) of files dumped."
    num_elements = default_params["num_elements"]
    filenames = sorted(dump_files)
    print(filenames)
    files = [
        h5py.File(filename, "r") for filename in filenames
    ]  # makes it easier to debug across frequencies
    ...
    for ii, f in enumerate(files):
        ...
        assert (
            f.attrs["time0_fpga_count"] * 2560
            == rest_commands[2 + int(ii / params["num_local_freq"])][2][
                "start_unix_nano"
            ]
        ), f"time0_fpga_count not recorded properly for trigger {ii}!"  # makes sure time0_fpga_count is recorded properly?
        assert (
            f.attrs["event_id"]
            == rest_commands[2 + int(ii / params["num_local_freq"])][2]["event_id"]
        )  # ii / num_local_freq indexing because there are many frequency files for each event
        assert f.attrs["freq_id"] == scrape_freq_id(
            f.filename
        )  # where is this defined? not in run_baseband or default_params.
        eshape = (
            rest_commands[2 + int(ii / params["num_local_freq"])][2]["duration_nano"]
            / 2560,
            num_elements,
        )
        assert (
            shape == eshape
        ), "Shape ({shape}) incorrect; expected  {eshape}"  # axes: [time samples][feed]. Eventually want [time][freq][feed] as [slow][med][fast]
        assert np.all(
            f["index_map/input"][:]["chan_id"] == np.arange(num_elements)
        ), "index_map/input incorrect."
        edata = (
            2 * (f.attrs["time0_fpga_count"] + np.arange(shape[0], dtype=int))
            + 3 * f.attrs["freq_id"]
        )  # increment freq_idx every time
        edata = edata[:, None] + 5 * np.arange(shape[1], dtype=int)
        edata = edata % 256
        assert np.all(
            f["baseband"][:] == edata
        ), "contents don't correspond to 2 t + 3 e + 5 f !"
...
def test_128_multifreq(tmpdir_factory):
    ...
    for ii, f in enumerate(files):
        ...
        assert np.all(
            f["baseband"][:] == edata
        ), "contents don't correspond to 2t + 3f + 5e !"
...
def test_128_multifreq_board_id(tmpdir_factory):
    ...
    for ii, f in enumerate(files):
        ...
        ), f"time0_fpga_count not recorded properly for trigger {ii}!"
        assert (
            f.attrs["event_id"]
            == rest_commands[2 + int(ii / params["num_local_freq"])][2]["event_id"]
        )  # ii / num_local_freq indexing because there are many frequency files for each event
    ...
    # assert(1 == 0, "Enter pdb by running pytest with --pdb and uncommenting me")

...
def test_pulsar_metadata(pulsar_data):
    ...
    assert pulsar_data.file_metadata['gating_data']['phase_ref'] == [pulsar_params['phase_ref']]
    assert pulsar_data.file_metadata['gating_data']['rot_freq'] == pulsar_params['rot_freq']
    assert pulsar_data.file_metadata['gating_data']['segment'] == 100.
    assert pulsar_data.file_metadata['gating_data']['pulse_width'] == pytest.approx(1e-3)

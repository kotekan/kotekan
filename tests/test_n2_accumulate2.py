...
def accum_list(setup, accum_setup):
    ...
    for accum in accums:
        if accum_setup["bin_in_ERA"]:
            ...
            accum["bin_start_ERAL_deg"] = np.remainder(
                accum["bin_start_ERA_deg"] + setup["tel"]["origin_itrs_lon_deg"], 360.0
            )
            accum["bin_end_ERAL_deg"] = np.remainder(
                accum["bin_end_ERA_deg"] + setup["tel"]["origin_itrs_lon_deg"], 360.0
            )
        else:
            seq_start = accum["seq"]

...
def gen_gains(
    ...
    freq_ds[...] = np.linspace(800.0, 400.0, nfreq, dtype=float)
...
def combine_gains(tframe, tcombine, new_tmstp, old_tmstp, new_gains, old_gains):
    if tframe < old_tmstp:
...
def test_apply(tmpdir_factory):
    ...
    new_tmstp = time.time() + 5.0
    new_tag = "gains{0}".format(int(new_tmstp))

    gen_gains(gains_dir)
    gen_gains(gains_dir, tag=new_tag, mult_factor=2.0)
    ...
    # REST commands
    cmds = [["post", "gains", {"tag": new_tag, "start_time": new_tmstp}]]
    ...
    for frame in gains_dump:
        ...
        for ii in range(len(gain_weight)):
            if gain_weight[ii] == 0.0:
                gains[ii] = 1.0
                weight_factor[ii] = 0.0
            elif abs(gains[ii]) == 0.0:
                gains[ii] = 1.0
                weight_factor[ii] = 0.0
            else:
                weight_factor[ii] = 1.0 / abs(gains[ii]) ** 2
        ...
        evecs = (
            np.arange(global_params["num_ev"])[:, None]
            + 1.0j * np.arange(global_params["num_elements"])[None, :]
        ).flatten()
        ...
        assert (abs(frame.gain.real - gains.real) <= 1e-5 * abs(gains.real)).all()
        assert (abs(frame.gain.imag - gains.imag) <= 1e-5 * abs(gains.imag)).all()

        expweight = []
        for ii in range(n_el):
            for jj in range(ii, n_el):
                expweight.append(1.0 * weight_factor[ii] * weight_factor[jj])
        expweight = np.array(expweight)
        assert (abs(frame.weight - expweight) <= 1e-5 * abs(frame.weight)).all()

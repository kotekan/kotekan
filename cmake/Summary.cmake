...
set(KFeature_name_col_width 24)
set(KFeature_state_col_width 8)
...
function(kfeature_row name enabled reason present)
    ...
    set(kfeat_enabled FALSE)
    string(TOUPPER "${enabled}" kfeat_enabled_upper)
    set(kfeat_true "ON;TRUE;YES;Y;1")
    set(kfeat_false "OFF;FALSE;NO;N;0")
    if(kfeat_enabled_upper IN_LIST kfeat_true)
        set(kfeat_enabled TRUE)
    elseif(kfeat_enabled_upper IN_LIST kfeat_false OR kfeat_enabled_upper STREQUAL "")
        set(kfeat_enabled FALSE)
    endif()

    string(TOLOWER "${reason}" kfeat_reason_lc)
    set(kfeat_reason_state "")
    if(kfeat_reason_lc MATCHES "auto-?detected|enabled|found")
        set(kfeat_reason_state TRUE)
    endif()
    if(kfeat_reason_lc MATCHES "disabled|not found|not available|not runnable|compiler not")
        set(kfeat_reason_state FALSE)
    endif()
    if(NOT "${kfeat_reason_state}" STREQUAL "")
        set(kfeat_enabled ${kfeat_reason_state})
    endif()

    set(state "")
    set(kfeat_toggle_upper "")
    if(NOT "${toggle_var}" STREQUAL "")
        get_property(
            kfeat_toggle
            CACHE ${toggle_var}
            PROPERTY VALUE)
        if(DEFINED kfeat_toggle)
            string(TOUPPER "${kfeat_toggle}" kfeat_toggle_upper)
        endif()
    endif()
    if(kfeat_toggle_upper STREQUAL "AUTO")
        if(kfeat_enabled)
            set(state "AUTO-ON")
        else()
            set(state "AUTO-OFF")
        endif()
    elseif(kfeat_toggle_upper STREQUAL "ON" OR kfeat_toggle_upper STREQUAL "OFF")
        set(state "${kfeat_toggle_upper}")
    else()
        if(kfeat_enabled)
            set(state "ON")
        else()
    ...
    elseif(kfeat_enabled)
        set(color "${KTK_GREEN}")
    else()
        if(present)
            set(color "${KTK_YELLOW}")
        elseif(kfeat_reason_lc MATCHES "missing|not found|not runnable|not available|compiler not")
            set(color "${KTK_RED}")
        else()
            set(color "${KTK_YELLOW}")
        endif()
    ...
    set(name_col "${name}:")
    kpad_right(name_col "${name_col}" ${KFeature_name_col_width})
    kpad_right(state_col "${state}" ${KFeature_state_col_width})
...
kmsg_status("${KTK_PALE_BLUE}${_bt_name} ${CMAKE_BUILD_TYPE} \
(-DCMAKE_BUILD_TYPE=Debug|Release|Test)${KTK_RESET}")

...
cudaEvent_t cuda{{{kernel_name}}}::execute(cudaPipelineState& /*pipestate*/, const std::vector<cudaEvent_t>& /*pre_events*/) {
    ...
    // Poison outputs
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            const std::ptrdiff_t Ttilde_length = (num_chunks == 1 ?
                                                     Ttilde_max_arg - Ttilde_min_arg :
                                                     chunk == 0 ? Ttilde_ringbuf - Ttilde_min_arg : Ttilde_max_arg - Ttilde_ringbuf);
            ...
            CHECK_CUDA_ERROR(cudaMemset2DAsync((std::uint8_t*)I_memory +
                                                   2 * Ttilde_offset * Ttilde_stride + 2 * Fbar_out_offset * Fbar_out_stride,
                                               2 * Ttilde_stride,
    ...
    // Check outputs for poison
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            const std::ptrdiff_t Fbar_out_offset = Fbar_out_min;
            const std::ptrdiff_t Fbar_out_length = Fbar_out_max - Fbar_out_min;
            std::vector<std::uint16_t> I_buffer(Ttilde_length * Fbar_out_length * Fbar_out_stride, 0xfffe);
            ...
            if (I_found_error)
                WARN("CUDA kernel {{{kernel_name}}} produced non-finite results");
        } // for chunk
    }
#endif

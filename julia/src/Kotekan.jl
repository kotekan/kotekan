...
function read_hdf5(filename::AbstractString)
    ...
    # Apply DimArray; do this last, it doesn't survive `mappedarray`
    sample0_offset = dataset["sample0_offset"][]::Integer
    dimoffsets = [dimnames[d] ∈ ["T", "Tbar"] ? sample0_offset : 0 for d in 1:dims]
    ...
    data = DimArray(data, Tuple(dimspecs))
...
function rms(A::AbstractDimArray; dims)
    ...
    R = dropdims(mapreduce(x -> abs2(float(x)), +, A; dims=dims); dims=dims)
    R .= sqrt.(R ./ n)

...
@fastmath @inbounds(
    begin #= /home/eschnett/src/kotekan/julia/kernels/chimefrb.jl:1339 =#
        info = 1
        info_memory[(((IndexSpaces.assume_inrange(IndexSpaces.cuda_threadidx()::Int32, 0, 32) % 32) % 32 + ((IndexSpaces.assume_inrange(IndexSpaces.cuda_blockidx()::Int32, 0, 2048) % 2048) % 2048) * 256 + ((IndexSpaces.assume_inrange(IndexSpaces.cuda_warpidx()::Int32, 0, 8) % 8) % 8) * 32) + 0) + 0x01] =
            info
        if !(
            0i32 ≤ Tbarmin < 512 && (

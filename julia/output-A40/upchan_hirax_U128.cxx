...
private:
    ...
    static constexpr std::ptrdiff_t G_U128_length = type_total_bytes(G_U128_type) * 128;
    ...
    static constexpr std::ptrdiff_t Ebar_length = type_total_bytes(Ebar_type) * 256 * 2 * 128 * 512;
...
cudaEvent_t cudaUpchannelizer_hirax_U128::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_hirax_U128 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_hirax_U128 returned 'info' value {:d} "

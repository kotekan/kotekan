/**
 * @file
 * @brief CUDA Upchannelizer_chime_U128 kernel
 *
...
/**
 * @class cudaUpchannelizer_chime_U128
 * @brief cudaCommand for Upchannelizer_chime_U128
 */
class cudaUpchannelizer_chime_U128 : public cudaCommand {
public:
    cudaUpchannelizer_chime_U128(Config& config, const std::string& unique_name,
                                 bufferContainer& host_buffers, cudaDeviceInterface& device,
                                 const int instance_num);
    virtual ~cudaUpchannelizer_chime_U128();
...
private:
    ...
    static constexpr const char* kernel_symbol =
        "_Z9upchan1285Int32S_S_S_S_S_13CuDeviceArrayI9Float16x2Li1ELi1EES0_I6Int4x8Li1ELi1EES4_S0_"
        "IS_Li1ELi1EE";

    // Kernel arguments:
    enum class args { T_min, T_max, Tbar_min, Tbar_max, Fmin, Fmax, G_U128, E, Ebar, info, count };
    ...
    //
    // G_U128: upchan_U128_gain_name
    static constexpr const char* G_U128_quantity = "G_U128";
    static constexpr kotekan::DataType G_U128_type = kotekan::float16;
    enum G_U128_indices {
        G_U128_index_Fbar,
        G_U128_rank,
    };
    ...
    static constexpr std::ptrdiff_t G_U128_length = G_U128_strides[G_U128_rank];
    static constexpr std::ptrdiff_t G_U128_length_in_bytes =
        type_total_bytes(G_U128_type) * G_U128_length;
    static_assert(G_U128_length_in_bytes <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    //
    // E: voltage_name
    ...
    const std::string G_U128_name;
    const std::string E_name;
    const std::string Ebar_name;
    const std::string info_name;

    // Buffers
    NDArrayBuffer<kotekan::GetType_t<G_U128_type>, G_U128_rank> G_U128_buffer;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_chime_U128);

cudaUpchannelizer_chime_U128::cudaUpchannelizer_chime_U128(Config& config,
                                                           const std::string& unique_name,
                                                           bufferContainer& host_buffers,
                                                           cudaDeviceInterface& device,
                                                           const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_chime_U128", "Upchannelizer_chime_U128.ptx"),
    Fmin(config.get<int>(unique_name, "Fmin")), Fmax(config.get<int>(unique_name, "Fmax")),

    G_U128_name(config.get<std::string>(unique_name, "upchan_U128_gain_name")),
    ...
    G_U128_buffer(G_U128_name, G_U128_quantity, reverse(G_U128_lengths), reverse(G_U128_labels),
    ...
    dummy() // avoid trailing comma
{
    ...
    G_U128_buffer.register_consumer();
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_chime_U128_");
    }
}
...
std::int64_t
cudaUpchannelizer_chime_U128::num_consumed_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chime_U128::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chime_U128::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_chime_U128::wait_on_precondition() {
    {
...
cudaEvent_t cudaUpchannelizer_chime_U128::execute(cudaPipelineState& /*pipestate*/,
                                                  const std::vector<cudaEvent_t>& /*pre_events*/) {
    pre_execute();
    record_start_event();

    void* const G_U128_memory = G_U128_buffer.get_ndarray().data();
    ...
    array_desc G_U128_arg(G_U128_memory, G_U128_length_in_bytes);
    ...
    const std::string symname = "Upchannelizer_chime_U128_" + std::string(kernel_symbol);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_chime_U128 returned error code: {}", error_code);
        ...
        for (int block = 0; block < blocks; ++block) {
            for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                    ...
                    if (val != 0)
                        ERROR("CUDA kernel Upchannelizer_chime_U128 returned 'info' value {:d} "
...
void cudaUpchannelizer_chime_U128::finalize_frame() {

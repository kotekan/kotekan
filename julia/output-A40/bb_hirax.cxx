...
private:
    ...
    static constexpr std::ptrdiff_t A_length = type_total_bytes(A_type) * 2 * 256 * 16 * 2 * 64;
    ...
    static constexpr std::ptrdiff_t s_length = type_total_bytes(s_type) * 16 * 2 * 64;
    ...
    static constexpr std::ptrdiff_t J_length = type_total_bytes(J_type) * 16384 * 2 * 64 * 16;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 4 * 128;
    ...
    static constexpr std::ptrdiff_t log_length = type_total_bytes(log_type) * 128;
...
cudaEvent_t cudaBasebandBeamformer_hirax::execute(cudaPipelineState& /*pipestate*/,
    ...
    *J_meta = *E_meta;
    ...
    for (int block = 0; block < info_lengths[info_index_block]; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel BasebandBeamformer_hirax returned 'info' value {:d} "
    ...
    if (log_code != 0)
        WARN("CUDA kernel BasebandBeamformer_hirax returned log code cuLaunchKernel: {}", log_code);

    for (std::size_t i = 0; i < log_host.size() / type_total_bytes(log_type); ++i) {
        const std::uint32_t val = ((const std::uint32_t*)log_host.data())[i];
        if (val != 0)
            WARN("CUDA kernel BasebandBeamformer_hirax returned 'log' value {:d} at index {:d} "
                 "(zero "
                 "indicates success)",
                 val, i);
    }

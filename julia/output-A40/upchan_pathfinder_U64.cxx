...
private:
    ...
    static constexpr std::ptrdiff_t G_U64_length = type_total_bytes(G_U64_type) * 2048;
    ...
    static constexpr std::ptrdiff_t Ebar_length = type_total_bytes(Ebar_type) * 64 * 2 * 2048 * 512;
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_pathfinder_U64 returned 'info' value {:d} "

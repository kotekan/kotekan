/**
 * @file
 * @brief CUDA Upchannelizer_chord_U8 kernel
 *
...
/**
 * @class cudaUpchannelizer_chord_U8
 * @brief cudaCommand for Upchannelizer_chord_U8
 */
class cudaUpchannelizer_chord_U8 : public cudaCommand {
public:
    cudaUpchannelizer_chord_U8(Config& config, const std::string& unique_name,
                               bufferContainer& host_buffers, cudaDeviceInterface& device,
                               const int instance_num);
    virtual ~cudaUpchannelizer_chord_U8();
...
private:
    ...
    static constexpr std::ptrdiff_t G8_length = chord_datatype_bytes(G8_type) * 384;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_chord_U8);

cudaUpchannelizer_chord_U8::cudaUpchannelizer_chord_U8(Config& config,
                                                       const std::string& unique_name,
                                                       bufferContainer& host_buffers,
                                                       cudaDeviceInterface& device,
                                                       const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_chord_U8", "Upchannelizer_chord_U8.ptx"),
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_chord_U8_");
    }
...
std::int64_t
cudaUpchannelizer_chord_U8::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) - cuda_algorithm_overlap;
}
std::int64_t
cudaUpchannelizer_chord_U8::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chord_U8::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_chord_U8::wait_on_precondition() {
...
cudaEvent_t cudaUpchannelizer_chord_U8::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar8_meta = *E_meta;
    ...
    DEBUG("Running CUDA Upchannelizer_chord_U8 on GPU frame {:d}", gpu_frame_id);
    const CUresult err =
...
void cudaUpchannelizer_chord_U8::finalize_frame() {
    const std::ptrdiff_t Tlength = Tmax - Tmin;
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;

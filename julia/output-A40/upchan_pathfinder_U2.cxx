...
private:
    ...
    static constexpr std::ptrdiff_t G_U2_length = type_total_bytes(G_U2_type) * 256;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        type_total_bytes(Ebar_type) * 64 * 2 * 256 * 16384;
...
cudaUpchannelizer_pathfinder_U2::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    DEBUG("Finished CUDA Upchannelizer_pathfinder_U2 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_pathfinder_U2 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_pathfinder_U2 returned 'info' value {:d} "

...
private:
    ...
    static constexpr int blocks_per_frequency = 1;
    ...
    static constexpr std::ptrdiff_t G_U16_length = type_total_bytes(G_U16_type) * 1024;
    ...
    static_assert(G_U16_length == type_total_bytes(G_U16_type) * G_U16_strides[G_U16_rank]);
    //
    ...
    static constexpr std::ptrdiff_t E_length = type_total_bytes(E_type) * 64 * 2 * 384 * 32768;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 16 * 384;
...
cudaUpchannelizer_pathfinder_U16::execute(cudaPipelineState& /*pipestate*/,
    ...
    void* const Ebar_memory =
        ...
        : args::Ebar == args::G_U16 ? device.get_gpu_memory(Ebar_memname, Ebar_length)
    ...
    *Ebar_meta = *E_meta;
    ...
    DEBUG("Finished CUDA Upchannelizer_pathfinder_U16 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_pathfinder_U16 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_pathfinder_U16 returned 'info' value {:d} "

...
private:
    ...
    static constexpr std::ptrdiff_t log_length = chord_datatype_bytes(log_type) * 384;
...
cudaBasebandBeamformer_pathfinder::execute(cudaPipelineState& /*pipestate*/,
    ...
    *J_meta = *E_meta;
    ...
    for (std::size_t i = 0; i < log_host.size(); ++i)
        if (log_host[i] != 0)
            ERROR("cudaBasebandBeamformer_pathfinder returned 'log' value {:d} at index {:d} (zero "
                  "indicates success)",
                  log_host[i], i);
#endif

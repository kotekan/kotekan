...
private:
    ...
    static constexpr std::ptrdiff_t A_length = type_total_bytes(A_type) * 2 * 64 * 16 * 2 * 384;
    ...
    static constexpr std::ptrdiff_t s_length = type_total_bytes(s_type) * 16 * 2 * 384;
    ...
    static constexpr std::ptrdiff_t J_length = type_total_bytes(J_type) * 8192 * 2 * 384 * 16;
    ...
    static constexpr std::ptrdiff_t log_length = type_total_bytes(log_type) * 384;
...
cudaBasebandBeamformer_pathfinder::cudaBasebandBeamformer_pathfinder(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "BasebandBeamformer_pathfinder_");
    }
...
std::int64_t cudaBasebandBeamformer_pathfinder::num_consumed_elements(
    std::int64_t num_available_elements) const {
    return num_produced_elements(num_available_elements);
}
std::int64_t cudaBasebandBeamformer_pathfinder::num_produced_elements(
    std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements);
}

std::int64_t cudaBasebandBeamformer_pathfinder::num_processed_elements(
    std::int64_t num_available_elements) const {
    assert(num_available_elements >= cuda_granularity_number_of_timesamples);
    return cuda_granularity_number_of_timesamples;
}

int cudaBasebandBeamformer_pathfinder::wait_on_precondition() {

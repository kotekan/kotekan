...
private:
    ...
    static constexpr const char* kernel_symbol =
        "_Z3frb5Int32S_S_S_S_S_S_S_13CuDeviceArrayI7Int16x2Ll1ELl1EES0_I9Float16x2Ll1ELl1EES0_"
        "I6Int4x8Ll1ELl1EES0_IS2_Ll1ELl1EES0_IS_Ll1ELl1EE";
    ...
    static constexpr std::ptrdiff_t S_length = chord_datatype_bytes(S_type) * 2 * 576;
    ...
    static constexpr std::ptrdiff_t W_length = chord_datatype_bytes(W_type) * 2 * 24 * 24 * 2 * 128;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 512 * 2 * 128 * 256;
    ...
    static constexpr std::ptrdiff_t I_length = chord_datatype_bytes(I_type) * 48 * 48 * 512 * 1024;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 24 * 128;
...
cudaEvent_t cudaFRBBeamformer_chord_U128::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    // Poison outputs
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            CHECK_CUDA_ERROR(cudaMemset2DAsync((std::uint8_t*)I_memory
                                               ...
                                               2 * Fbar_out_length * Fbar_out_stride, Ttildelength,
                                               device.getStream(cuda_stream_id)));
        } // for chunk
    ...
    DEBUG("Finished CUDA FRBBeamformer_chord_U128 on GPU frame {:d}", gpu_frame_id);
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            for (std::ptrdiff_t ttilde = 0; ttilde < Ttildelength; ++ttilde) {
                for (std::ptrdiff_t ftilde = 0; ftilde < Fbar_out_length; ++ftilde) {
                    ...
                    //         DEBUG("    U=128 [{},{},{}]={:#02x}", ttilde, ftilde, n, 0x88);
                    //     }
                    // }
                    ...
                    if (any_error) {
                        DEBUG("    U=128 [{},{}]=(any={},all={})", ttilde, ftilde, any_error,
                              all_error);
                    }

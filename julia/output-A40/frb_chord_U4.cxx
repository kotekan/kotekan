...
cudaFRBBeamformer_chord_U4::cudaFRBBeamformer_chord_U4(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "FRBBeamformer_chord_U4_");
    }
...
cudaEvent_t cudaFRBBeamformer_chord_U4::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Running CUDA FRBBeamformer_chord_U4 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA FRBBeamformer_chord_U4 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaFRBBeamformer_chord_U4 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            for (std::ptrdiff_t ttilde = 0; ttilde < Ttildelength; ++ttilde) {
                for (std::ptrdiff_t ftilde = 0; ftilde < Fbar_out_length; ++ftilde) {
                    ...
                    if (any_error) {
                        DEBUG("    U=4 [{},{}]=(any={},all={})", ttilde, ftilde, any_error,
                              all_error);
                    }

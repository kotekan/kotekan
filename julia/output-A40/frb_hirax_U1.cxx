...
private:
    ...
    static constexpr std::ptrdiff_t W_length = chord_datatype_bytes(W_type) * 2 * 16 * 16 * 2 * 1;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 256 * 2 * 64 * 65536;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 16 * 64;
...
cudaEvent_t cudaFRBBeamformer_hirax_U1::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA FRBBeamformer_hirax_U1 on GPU frame {:d}", gpu_frame_id);
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            for (std::ptrdiff_t ttilde = 0; ttilde < Ttildelength; ++ttilde) {
                for (std::ptrdiff_t ftilde = 0; ftilde < Fbar_out_length; ++ftilde) {
                    ...
                    //         DEBUG("    U=1 [{},{},{}]={:#02x}", ttilde, ftilde, n, 0x88);
                    //     }
                    // }
                    ...
                    if (any_error) {
                        DEBUG("    U=1 [{},{}]=(any={},all={})", ttilde, ftilde, any_error,
                              all_error);
                    }

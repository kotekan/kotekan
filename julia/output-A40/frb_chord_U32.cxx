...
private:
    ...
    static constexpr std::ptrdiff_t W_length = chord_datatype_bytes(W_type) * 2 * 24 * 24 * 2 * 256;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 24 * 256;
...
cudaEvent_t cudaFRBBeamformer_chord_U32::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA FRBBeamformer_chord_U32 on GPU frame {:d}", gpu_frame_id);
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            for (std::ptrdiff_t ttilde = 0; ttilde < Ttildelength; ++ttilde) {
                for (std::ptrdiff_t ftilde = 0; ftilde < Fbar_out_length; ++ftilde) {
                    ...
                    //         DEBUG("    U=32 [{},{},{}]={:#02x}", ttilde, ftilde, n, 0x88);
                    //     }
                    // }
                    ...
                    if (any_error) {
                        DEBUG("    U=32 [{},{}]=(any={},all={})", ttilde, ftilde, any_error,
                              all_error);
                    }

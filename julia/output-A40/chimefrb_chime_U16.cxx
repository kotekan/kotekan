/**
 * @file
 * @brief CUDA CHIMEFRBBeamformer_chime_U16 kernel
 *
...
/**
 * @class cudaCHIMEFRBBeamformer_chime_U16
 * @brief cudaCommand for CHIMEFRBBeamformer_chime_U16
 */
class cudaCHIMEFRBBeamformer_chime_U16 : public cudaCommand {
public:
    cudaCHIMEFRBBeamformer_chime_U16(Config& config, const std::string& unique_name,
                                     bufferContainer& host_buffers, cudaDeviceInterface& device,
                                     const int instance_num);
    virtual ~cudaCHIMEFRBBeamformer_chime_U16();
...
private:
    ...
    static constexpr std::ptrdiff_t W_length = type_total_bytes(W_type) * 2 * 256 * 4 * 2 * 256;
    ...
    static constexpr std::ptrdiff_t I_length = type_total_bytes(I_type) * 512 * 8 * 256 * 1024;
...
REGISTER_CUDA_COMMAND(cudaCHIMEFRBBeamformer_chime_U16);

cudaCHIMEFRBBeamformer_chime_U16::cudaCHIMEFRBBeamformer_chime_U16(Config& config,
                                                                   const std::string& unique_name,
                                                                   bufferContainer& host_buffers,
                                                                   cudaDeviceInterface& device,
                                                                   const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "CHIMEFRBBeamformer_chime_U16", "CHIMEFRBBeamformer_chime_U16.ptx"),
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "CHIMEFRBBeamformer_chime_U16_");
    }
...
std::int64_t
cudaCHIMEFRBBeamformer_chime_U16::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_produced_elements(num_available_elements) * cuda_downsampling_factor;
}
std::int64_t
cudaCHIMEFRBBeamformer_chime_U16::num_produced_elements(std::int64_t num_available_elements) const {
...
cudaEvent_t
cudaCHIMEFRBBeamformer_chime_U16::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    const std::string symname = "CHIMEFRBBeamformer_chime_U16_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA CHIMEFRBBeamformer_chime_U16 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA CHIMEFRBBeamformer_chime_U16 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel CHIMEFRBBeamformer_chime_U16 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel CHIMEFRBBeamformer_chime_U16 returned 'info' value {:d} "
    ...
    // Check outputs for poison
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            if (I_found_error)
                WARN("CUDA kernel CHIMEFRBBeamformer_chime_U16 returned produced non-finite "
                     "results");
        } // for chunk
...
void cudaCHIMEFRBBeamformer_chime_U16::finalize_frame() {
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;
    const std::ptrdiff_t Ttildelength = Ttildemax - Ttildemin;

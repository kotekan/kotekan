...
private:
    ...
    // Kernel arguments:
    enum class args { Tmin, Tmax, Tbarmin, Tbarmax, Fmin, Fmax, G_U32, E, Ebar, info, count };
    ...
    //
    // G_U32: upchan_U32_gain_name
    static constexpr const char* G_U32_quantity = "G_U32";
    static constexpr kotekan::DataType G_U32_type = kotekan::float16;
    enum G_U32_indices {
        G_U32_index_Fbar,
        G_U32_rank,
    };
    ...
    static constexpr std::ptrdiff_t G_U32_length = type_total_bytes(G_U32_type) * 1024;
    ...
    static constexpr std::array<std::ptrdiff_t, G_U32_rank + 1> G_U32_strides = {
        G_U32_calc_stride(G_U32_index_Fbar),
        G_U32_calc_stride(G_U32_rank),
    };
    static_assert(G_U32_length == type_total_bytes(G_U32_type) * G_U32_strides[G_U32_rank]);
    //
    // E: voltage_name
    ...
    static constexpr std::ptrdiff_t E_length = type_total_bytes(E_type) * 256 * 2 * 64 * 65536;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        type_total_bytes(Ebar_type) * 256 * 2 * 1024 * 2048;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 16 * 256;
...
cudaUpchannelizer_hirax_U32::cudaUpchannelizer_hirax_U32(Config& config,
    ...
    G_U32_name(config.get<std::string>(unique_name, "upchan_U32_gain_name")),
    ...
    output_ringbuf_signal(dynamic_cast<RingBuffer*>(host_buffers.get_generic_buffer(
        config.get<std::string>(unique_name, "upchan_U32_voltage_signal_out")))),
    ...
    gpu_buffers_used.push_back(std::make_tuple(G_U32_name, true, true, false));
...
cudaEvent_t cudaUpchannelizer_hirax_U32::execute(cudaPipelineState& /*pipestate*/,
    ...
    void* const Ebar_memory =
        ...
        : args::Ebar == args::G_U32 ? device.get_gpu_memory(Ebar_memname, Ebar_length)
    ...
    assert(std::strncmp(G_U32_meta->name, G_U32_quantity, sizeof G_U32_meta->name) == 0);
    ...
    *Ebar_meta = *E_meta;
    ...
    void* args[] = {
        ...
        &Fmax_arg, &G_U32_arg, &E_arg,    &Ebar_arg,    &info_arg,
    };
    ...
    DEBUG("Finished CUDA Upchannelizer_hirax_U32 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_hirax_U32 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_hirax_U32 returned 'info' value {:d} "

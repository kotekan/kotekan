/**
 * @file
 * @brief CUDA Upchannelizer_chime_U16 kernel
 *
...
/**
 * @class cudaUpchannelizer_chime_U16
 * @brief cudaCommand for Upchannelizer_chime_U16
 */
class cudaUpchannelizer_chime_U16 : public cudaCommand {
public:
    cudaUpchannelizer_chime_U16(Config& config, const std::string& unique_name,
                                bufferContainer& host_buffers, cudaDeviceInterface& device,
                                const int instance_num);
    virtual ~cudaUpchannelizer_chime_U16();
...
private:
    ...
    static constexpr std::ptrdiff_t G_U16_length = chord_datatype_bytes(G_U16_type) * 256;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 1024 * 2 * 256 * 4096;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_chime_U16);

cudaUpchannelizer_chime_U16::cudaUpchannelizer_chime_U16(Config& config,
                                                         const std::string& unique_name,
                                                         bufferContainer& host_buffers,
                                                         cudaDeviceInterface& device,
                                                         const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_chime_U16", "Upchannelizer_chime_U16.ptx"),
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_chime_U16_");
    }
...
std::int64_t
cudaUpchannelizer_chime_U16::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) - cuda_algorithm_overlap;
}
std::int64_t
cudaUpchannelizer_chime_U16::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chime_U16::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_chime_U16::wait_on_precondition() {
...
cudaEvent_t cudaUpchannelizer_chime_U16::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    const std::string symname = "Upchannelizer_chime_U16_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA Upchannelizer_chime_U16 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA Upchannelizer_chime_U16 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaUpchannelizer_chime_U16 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
...
void cudaUpchannelizer_chime_U16::finalize_frame() {
    const std::ptrdiff_t Tlength = Tmax - Tmin;
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;

...
private:
    ...
    static constexpr std::ptrdiff_t G_U16_length = type_total_bytes(G_U16_type) * 256;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        type_total_bytes(Ebar_type) * 1024 * 2 * 256 * 4096;
...
cudaEvent_t cudaUpchannelizer_chime_U16::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_chime_U16 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_chime_U16 returned 'info' value {:d} "

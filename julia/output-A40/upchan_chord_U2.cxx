/**
 * @file
 * @brief CUDA Upchannelizer_chord_U2 kernel
 *
...
/**
 * @class cudaUpchannelizer_chord_U2
 * @brief cudaCommand for Upchannelizer_chord_U2
 */
class cudaUpchannelizer_chord_U2 : public cudaCommand {
public:
    cudaUpchannelizer_chord_U2(Config& config, const std::string& unique_name,
                               bufferContainer& host_buffers, cudaDeviceInterface& device,
                               const int instance_num);
    virtual ~cudaUpchannelizer_chord_U2();
...
private:
    ...
    static constexpr int shmem_bytes = 49664;

    // Kernel name:
    const char* const kernel_symbol =
        "_Z7upchan25Int32S_S_S_S_S_13CuDeviceArrayI9Float16x2Li1ELi1EES0_I6Int4x8Li1ELi1EES4_S0_IS_"
        "Li1ELi1EE";

    // Kernel arguments:
    enum class args { Tmin, Tmax, Tbarmin, Tbarmax, Fmin, Fmax, G_U2, E, Ebar, info, count };
    ...
    //
    // G_U2: upchan_U2_gain_name
    static constexpr const char* G_U2_quantity = "G_U2";
    static constexpr kotekan::DataType G_U2_type = kotekan::float16;
    enum G_U2_indices {
        G_U2_index_Fbar,
        G_U2_rank,
    };
    ...
    static constexpr std::ptrdiff_t G_U2_length = type_total_bytes(G_U2_type) * 32;
    ...
    static constexpr std::array<std::ptrdiff_t, G_U2_rank + 1> G_U2_strides = {
        G_U2_calc_stride(G_U2_index_Fbar),
        G_U2_calc_stride(G_U2_rank),
    };
    static_assert(G_U2_length == type_total_bytes(G_U2_type) * G_U2_strides[G_U2_rank]);
    //
    // E: voltage_name
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        type_total_bytes(Ebar_type) * 512 * 2 * 32 * 16384;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 2 * 384;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_chord_U2);

cudaUpchannelizer_chord_U2::cudaUpchannelizer_chord_U2(Config& config,
                                                       const std::string& unique_name,
                                                       bufferContainer& host_buffers,
                                                       cudaDeviceInterface& device,
                                                       const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_chord_U2", "Upchannelizer_chord_U2.ptx"),
    G_U2_name(config.get<std::string>(unique_name, "upchan_U2_gain_name")),
    ...
    output_ringbuf_signal(dynamic_cast<RingBuffer*>(host_buffers.get_generic_buffer(
        config.get<std::string>(unique_name, "upchan_U2_voltage_signal_out")))),
    ...
    gpu_buffers_used.push_back(std::make_tuple(G_U2_name, true, true, false));
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_chord_U2_");
    }
...
std::int64_t
cudaUpchannelizer_chord_U2::num_consumed_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chord_U2::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chord_U2::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_chord_U2::wait_on_precondition() {
...
cudaEvent_t cudaUpchannelizer_chord_U2::execute(cudaPipelineState& /*pipestate*/,
                                                const std::vector<cudaEvent_t>& /*pre_events*/) {
    pre_execute();

    const std::string G_U2_memname = G_U2_name + "_buffer";
    ...
    void* const Ebar_memory =
        ...
        : args::Ebar == args::G_U2 ? device.get_gpu_memory(Ebar_memname, Ebar_length)
    ...
    assert(std::strncmp(G_U2_meta->name, G_U2_quantity, sizeof G_U2_meta->name) == 0);
    ...
    *Ebar_meta = *E_meta;
    ...
    void* args[] = {
        ...
        &Fmax_arg, &G_U2_arg, &E_arg,    &Ebar_arg,    &info_arg,
    };
    ...
    const std::string symname = "Upchannelizer_chord_U2_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA Upchannelizer_chord_U2 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA Upchannelizer_chord_U2 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_chord_U2 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_chord_U2 returned 'info' value {:d} "
...
void cudaUpchannelizer_chord_U2::finalize_frame() {
    const std::ptrdiff_t Tlength = Tmax - Tmin;
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;

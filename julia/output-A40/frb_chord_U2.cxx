...
private:
    ...
    static constexpr std::ptrdiff_t W_length = chord_datatype_bytes(W_type) * 2 * 24 * 24 * 2 * 32;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 24 * 32;
...
cudaFRBBeamformer_chord_U2::cudaFRBBeamformer_chord_U2(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "FRBBeamformer_chord_U2_");
    }
...
cudaEvent_t cudaFRBBeamformer_chord_U2::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Running CUDA FRBBeamformer_chord_U2 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA FRBBeamformer_chord_U2 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaFRBBeamformer_chord_U2 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            for (std::ptrdiff_t ttilde = 0; ttilde < Ttildelength; ++ttilde) {
                for (std::ptrdiff_t ftilde = 0; ftilde < Fbar_out_length; ++ftilde) {
                    ...
                    //         DEBUG("    U=2 [{},{},{}]={:#02x}", ttilde, ftilde, n, 0x88);
                    //     }
                    // }
                    ...
                    if (any_error) {
                        DEBUG("    U=2 [{},{}]=(any={},all={})", ttilde, ftilde, any_error,
                              all_error);
                    }

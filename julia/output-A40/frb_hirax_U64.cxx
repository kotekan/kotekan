...
private:
    ...
    static constexpr std::ptrdiff_t W_length = type_total_bytes(W_type) * 2 * 16 * 16 * 2 * 64;
...
cudaEvent_t cudaFRBBeamformer_hirax_U64::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA kernel FRBBeamformer_hirax_U64 on GPU frame {:d}", gpu_frame_id);
    ...
    // Check outputs for poison
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            if (I_found_error)
                WARN("CUDA kernel FRBBeamformer_hirax_U64 returned produced non-finite results");
        } // for chunk

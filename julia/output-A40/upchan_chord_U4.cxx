/**
 * @file
 * @brief CUDA Upchannelizer_chord_U4 kernel
 *
...
/**
 * @class cudaUpchannelizer_chord_U4
 * @brief cudaCommand for Upchannelizer_chord_U4
 */
class cudaUpchannelizer_chord_U4 : public cudaCommand {
public:
    cudaUpchannelizer_chord_U4(Config& config, const std::string& unique_name,
                               bufferContainer& host_buffers, cudaDeviceInterface& device,
                               const int instance_num);
    virtual ~cudaUpchannelizer_chord_U4();
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_chord_U4);

cudaUpchannelizer_chord_U4::cudaUpchannelizer_chord_U4(Config& config,
                                                       const std::string& unique_name,
                                                       bufferContainer& host_buffers,
                                                       cudaDeviceInterface& device,
                                                       const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_chord_U4", "Upchannelizer_chord_U4.ptx"),
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_chord_U4_");
    }
...
std::int64_t
cudaUpchannelizer_chord_U4::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) - cuda_algorithm_overlap;
}
std::int64_t
cudaUpchannelizer_chord_U4::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chord_U4::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_chord_U4::wait_on_precondition() {
...
cudaEvent_t cudaUpchannelizer_chord_U4::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    const std::string symname = "Upchannelizer_chord_U4_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA Upchannelizer_chord_U4 on GPU frame {:d}", gpu_frame_id);
...
void cudaUpchannelizer_chord_U4::finalize_frame() {
    const std::ptrdiff_t Tlength = Tmax - Tmin;
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;

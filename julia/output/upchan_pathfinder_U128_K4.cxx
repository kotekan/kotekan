/**
 * @file
 * @brief CUDA Upchannelizer_pathfinder_U128_K4 kernel
 *
...
/**
 * @class cudaUpchannelizer_pathfinder_U128_K4
 * @brief cudaCommand for Upchannelizer_pathfinder_U128_K4
 */
class cudaUpchannelizer_pathfinder_U128_K4 : public cudaCommand {
public:
    cudaUpchannelizer_pathfinder_U128_K4(Config& config, const std::string& unique_name,
                                         bufferContainer& host_buffers, cudaDeviceInterface& device,
                                         const int instance_num);
    virtual ~cudaUpchannelizer_pathfinder_U128_K4();
...
private:
    ...
    static constexpr int cuda_output_bits = 4;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_pathfinder_U128_K4);

cudaUpchannelizer_pathfinder_U128_K4::cudaUpchannelizer_pathfinder_U128_K4(
    Config& config, const std::string& unique_name, bufferContainer& host_buffers,
    cudaDeviceInterface& device, const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_pathfinder_U128_K4", "Upchannelizer_pathfinder_U128_K4.ptx"),
    ...
    did_set_metadata(false), dummy() // avoid trailing comma
{
    ...
    if (!device.runtime_kernels.count("Upchannelizer_pathfinder_U128_K4_"
                                      + std::string(kernel_symbol))) {
        const std::vector<std::string> opts = {
            "--gpu-name=sm_86",
            "--verbose",
        };
        device.build_ptx("lib/cuda/generated/Upchannelizer_pathfinder_U128_K4.ptx", {kernel_symbol},
                         opts, "Upchannelizer_pathfinder_U128_K4_");
    }
}

cudaUpchannelizer_pathfinder_U128_K4::~cudaUpchannelizer_pathfinder_U128_K4() {}

std::int64_t cudaUpchannelizer_pathfinder_U128_K4::num_consumed_elements(
...
std::int64_t cudaUpchannelizer_pathfinder_U128_K4::num_produced_elements(
    std::int64_t num_available_elements) const {
    return div_noremainder(num_consumed_elements(num_available_elements),
                           cuda_upchannelization_factor);
}

std::int64_t cudaUpchannelizer_pathfinder_U128_K4::num_processed_elements(
    std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_pathfinder_U128_K4::wait_on_precondition() {
    {
...
cudaEvent_t
cudaUpchannelizer_pathfinder_U128_K4::execute(cudaPipelineState& /*pipestate*/,
    ...
    if (instance_num == 0 && !did_set_metadata) {
        ...
        if (!(0 <= Fmin && Fmin <= Fmax && Fmax <= E_nfreq))
            FATAL_ERROR("Invalid frequency span [{:d},{:d}) for kernel "
                        "Upchannelizer_pathfinder_U128_K4: input "
        ...
        if (G_nfreq != Ebar_nfreq)
            FATAL_ERROR(
                "Gain buffer G holds {:d} frequencies, but kernel Upchannelizer_pathfinder_U128_K4 "
                "produces {:d}",
                G_nfreq, Ebar_nfreq);
        ...
        for (int freq = 0; freq < Ebar_nfreq; ++freq)
            if (Ebar_coarse_freq.at(freq) != G_coarse_freq.at(freq))
                FATAL_ERROR("Gain buffer G is for coarse frequency {:d} at index {:d}, but kernel "
                            "Upchannelizer_pathfinder_U128_K4 produces coarse frequency {:d} there",
        ...
        if (Ebar_meta->dim[Ebar_rank - 1 - Ebar_index_Fbar] < Ebar_nfreq)
            FATAL_ERROR("Kernel Upchannelizer_pathfinder_U128_K4 produces {:d} frequencies, but "
                        "the frequency "
    ...
    if (!Ebar_buffer.has_metadata())
        FATAL_ERROR("Output buffer Ebar has no metadata; kernel Upchannelizer_pathfinder_U128_K4 "
                    "cannot run");
    ...
    if (!(0 <= blocks && blocks <= max_blocks))
        FATAL_ERROR("Kernel Upchannelizer_pathfinder_U128_K4 would launch {:d} blocks, but the "
                    "`info` buffer "
    ...
    const std::string symname = "Upchannelizer_pathfinder_U128_K4_" + std::string(kernel_symbol);
    ...
    if (poison_buffers) {
        ...
        if (error_code != 0)
            ERROR("CUDA kernel Upchannelizer_pathfinder_U128_K4 returned error code: {}",
                  error_code);
            ...
            for (int block = 0; block < blocks; ++block) {
                for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                    for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                        ...
                        if (val != 0)
                            ERROR("CUDA kernel Upchannelizer_pathfinder_U128_K4 returned 'info' "
                                  "value {:d} "
...
void cudaUpchannelizer_pathfinder_U128_K4::finalize_frame() {

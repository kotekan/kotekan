...
cudaEvent_t cudaBasebandBeamformer_chord::execute(cudaPipelineState& /*pipestate*/,
    ...
    // Update metadata
    {
        ...
        if (E_meta->get_time_downsampling_fpga() != 1)
            FATAL_ERROR(
                ...
                "generated for kernel BasebandBeamformer_chord assume an input sampled at the "
                "FPGA rate",
        ...
        if (A_buffer.get_metadata()->get_fpga_seq_num() != E_meta->get_fpga_seq_num())
            FATAL_ERROR("Buffer A begins at FPGA sequence number {:d}, but the "
                        "voltage buffer E begins at {:d}; kernel BasebandBeamformer_chord requires "
                        "them to be aligned",
    ...
    if (poison_buffers) {
        ...
        if (log_code != 0)
            WARN("CUDA kernel BasebandBeamformer_chord returned log code cuLaunchKernel: {}",
                 log_code);

...
cudaUpchannelizer_pathfinder_U64::execute(cudaPipelineState& /*pipestate*/,
    ...
    if (instance_num == 0 && !did_set_metadata) {
        ...
        if (!(0 <= Fmin && Fmin <= Fmax && Fmax <= E_nfreq))
            FATAL_ERROR(
                "Invalid frequency span [{:d},{:d}) for kernel Upchannelizer_pathfinder_U64: input "
        ...
        if (G_nfreq != Ebar_nfreq)
            FATAL_ERROR(
                "Gain buffer G holds {:d} frequencies, but kernel Upchannelizer_pathfinder_U64 "
                "produces {:d}",
                G_nfreq, Ebar_nfreq);
        ...
        for (int freq = 0; freq < Ebar_nfreq; ++freq)
            if (Ebar_coarse_freq.at(freq) != G_coarse_freq.at(freq))
                FATAL_ERROR("Gain buffer G is for coarse frequency {:d} at index {:d}, but kernel "
                            "Upchannelizer_pathfinder_U64 produces coarse frequency {:d} there",
        ...
        if (Ebar_meta->dim[Ebar_rank - 1 - Ebar_index_Fbar] < Ebar_nfreq)
            FATAL_ERROR(
                "Kernel Upchannelizer_pathfinder_U64 produces {:d} frequencies, but the frequency "
    ...
    if (!Ebar_buffer.has_metadata())
        FATAL_ERROR(
            "Output buffer Ebar has no metadata; kernel Upchannelizer_pathfinder_U64 cannot run");
    ...
    if (!(0 <= blocks && blocks <= max_blocks))
        FATAL_ERROR(
            "Kernel Upchannelizer_pathfinder_U64 would launch {:d} blocks, but the `info` buffer "
            ...
            for (int block = 0; block < blocks; ++block) {
                for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                    for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                        ...
                        if (val != 0)
                            ERROR("CUDA kernel Upchannelizer_pathfinder_U64 returned 'info' value "
                                  "{:d} "

...
cudaCHIMEFRBBeamformer_chime_U128::execute(cudaPipelineState& /*pipestate*/,
    ...
    if (instance_num == 0 && !did_set_metadata) {
        ...
        if (!(Telescope::instance().get_grid_size_x() <= std::uint64_t(cuda_dish_layout_N)
            ...
            FATAL_ERROR("Telescope dish grid {:d}x{:d} does not fit the dish layout {:d}x{:d} "
                        "(N x M) for which kernel CHIMEFRBBeamformer_chime_U128 was compiled",
        ...
        if (I_has_metadata)
            FATAL_ERROR("Output buffer I already has metadata; kernel "
                        "CHIMEFRBBeamformer_chime_U128 must be "
                        "its only producer");
        ...
        if (W_nfreq != I_nfreq)
            FATAL_ERROR(
                "Weight buffer W holds {:d} frequencies, but kernel CHIMEFRBBeamformer_chime_U128 "
                "processes {:d}",
                W_nfreq, I_nfreq);
        ...
        for (int freq = 0; freq < W_nfreq; ++freq)
            if (I_coarse_freq.at(freq) != W_coarse_freq.at(freq))
                FATAL_ERROR(
                    ...
                    "kernel CHIMEFRBBeamformer_chime_U128 processes coarse frequency {:d} there",
    ...
    if (!I_buffer.has_metadata())
        FATAL_ERROR(
            "Output buffer I has no metadata; kernel CHIMEFRBBeamformer_chime_U128 cannot run");

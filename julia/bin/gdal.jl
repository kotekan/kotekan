# Experiment with Zarr files via the GDAL library
...
function write_attribute(mdarrayh::GDAL.GDALMDArrayH, name::AbstractString, value::AbstractString)
    datatypeh = GDAL.gdalextendeddatatypecreatestring(length(value))
    check_last_error()
    @assert datatypeh != C_NULL
    attributeh = GDAL.gdalmdarraycreateattribute(mdarrayh, name, 0, C_NULL, datatypeh, C_NULL)
    check_last_error()
...
function write_attribute(mdarrayh::GDAL.GDALMDArrayH, name::AbstractString, value::Integer)
    ...
    check_last_error()
    @assert attributeh != C_NULL
    noerr = GDAL.gdalattributewriteraw(attributeh, Ref(Int64(value)), sizeof(Int64))
    check_last_error()
    @assert noerr == true
    GDAL.gdalattributerelease(attributeh)
    check_last_error()
...
function write_attribute(mdarrayh::GDAL.GDALMDArrayH, name::AbstractString, value::Real)
    ...
    check_last_error()
    @assert attributeh != C_NULL
    noerr = GDAL.gdalattributewriteraw(attributeh, Ref(Float64(value)), sizeof(Float64))
    check_last_error()
    @assert noerr == true
    GDAL.gdalattributerelease(attributeh)
    check_last_error()
...
function write_attribute(mdarrayh::GDAL.GDALMDArrayH, name::AbstractString, values::Array{<:Integer})
    ...
    attributeh = GDAL.gdalmdarraycreateattribute(mdarrayh, name, rank, sizes, datatypeh, C_NULL)
    check_last_error()
    @assert attributeh != C_NULL
    noerr = GDAL.gdalattributewriteraw(attributeh, Int64.(values), sizeof(Int64) * length(values))
    check_last_error()
    @assert noerr == true
    GDAL.gdalattributerelease(attributeh)
    check_last_error()
...
function write_attribute(mdarrayh::GDAL.GDALMDArrayH, name::AbstractString, values::Array{<:Real})
    ...
    check_last_error()
    @assert attributeh != C_NULL
    noerr = GDAL.gdalattributewriteraw(attributeh, Float64.(values), sizeof(Float64) * length(values))
    check_last_error()
    @assert noerr == true
    GDAL.gdalattributerelease(attributeh)
    check_last_error()
    ...
    elseif class === GDAL.GEDTC_NUMERIC
        ...
        if numericdatatype === GDAL.GDT_Int64
            ...
            if dimensioncount == 0
                ...
                value = [unsafe_load(Ptr{Int64}(valueptr), n) for n in 1:nvalues]
                value::Vector{Int64}
            end
        elseif numericdatatype === GDAL.GDT_Float64
            ...
            if dimensioncount == 0
                ...
                value = [unsafe_load(Ptr{Float64}(valueptr), n) for n in 1:nvalues]
                value::Vector{Float64}
            end
        else
            @assert false
        end
        GDAL.gdalattributefreerawresult(attributeh, valueptr, attributesizeref[])
        check_last_error()
    else
        @assert false
    end
...
function create_file(driver::AbstractString, path::AbstractString)
    println("Creating file \"$(path)\" via $(driver) driver...")

    driverh = GDAL.gdalgetdriverbyname(driver)
    check_last_error()
    ...
    dimensionhs = GDAL.GDALDimensionH[
        let
            dimh = GDAL.gdalgroupcreatedimension(grouph, dimensionnames[d], "UNUSED", "UNUSED", size(data, d), GDAL.CSLConstList())
            check_last_error()
            @assert dimh != C_NULL
            dimh
    ...
    arrayoptionptrs = Cstring[[pointer(opt) for opt in arrayoptions]; Cstring(C_NULL)]

    mdarrayh = GDAL.gdalgroupcreatemdarray(grouph, "data", length(dimensionhs), dimensionhs, datatypeh, arrayoptionptrs)
    check_last_error()
...
function read_file(path::AbstractString)
    ...
    dataseth = GDAL.gdalopenex(
        path,
        GDAL.GDAL_OF_MULTIDIM_RASTER | GDAL.GDAL_OF_READONLY | GDAL.GDAL_OF_SHARED | GDAL.GDAL_OF_VERBOSE_ERROR,
        C_NULL,
        C_NULL,
        C_NULL,
    )
    ...
    attributehptr = GDAL.gdalmdarraygetattributes(mdarrayh, attributescountref, C_NULL)
    check_last_error()
    attributehs = GDAL.GDALAttributeH[unsafe_load(attributehptr, d) for d in 1:attributescountref[]]
    ...
    check_last_error()
    dimensionhs = GDAL.GDALDimensionH[unsafe_load(dimensionhptr, d) for d in 1:dimensionscountref[]]

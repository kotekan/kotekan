#!/bin/bash

# This script generates the Julia-based CUDA kernels for the FRB beamforming kernel.
# Run it from the kotekan base directory like ./julia/bin/frb.sh

set -euxo pipefail

scriptdir=$(dirname "$0")
cd "$scriptdir/.."

setups='
    chord_U1 chord_U2 chord_U4 chord_U8 chord_U16 chord_U32 chord_U64 chord_U128
    hirax_U1 hirax_U2 hirax_U4 hirax_U8 hirax_U16 hirax_U32 hirax_U64 hirax_U128
    pathfinder_U1 pathfinder_U2 pathfinder_U4 pathfinder_U8 pathfinder_U16 pathfinder_U32 pathfinder_U64
'
# smallfinder_U1 smallfinder_U2 smallfinder_U4 smallfinder_U8 smallfinder_U16 smallfinder_U32 smallfinder_U64

mkdir -p output

# Delete previous output (so that we don't accidentally re-use it)
for setup in $setups; do
    rm -f output/frb_$setup.cxx
    rm -f output/frb_$setup.jl
    rm -f output/frb_$setup.ptx
    rm -f output/frb_$setup.sass
    rm -f output/frb_$setup.yaml
    rm -f ../lib/cuda/generated/cudaFRBBeamformer_$setup.cpp
    rm -f ../lib/cuda/generated/FRBBeamformer_$setup.jl
    rm -f ../lib/cuda/generated/FRBBeamformer_$setup.ptx
    rm -f ../lib/cuda/generated/FRBBeamformer_$setup.yaml
done

# Generate kernel
# for setup in $setups; do
#     julia --project=@. --optimize kernels/frb_$setup.jl 2>&1 | tee output/frb_$setup.out &
# done
# wait
# for setup in $setups; do
#     julia --project=@. --optimize kernels/frb_$setup.jl 2>&1 | tee output/frb_$setup.out
# done
for setup in $setups; do echo $setup; done |
    xargs -I'$setup' -n 1 -P 8 bash -c 'julia --project=@. --optimize kernels/frb_$setup.jl 2>&1 | tee output/frb_$setup.out'

# Check whether kernels were generated
for setup in $setups; do
    test -f output/frb_$setup.cxx
    test -f output/frb_$setup.jl
    test -f output/frb_$setup.ptx
    test -f output/frb_$setup.sass
    test -f output/frb_$setup.yaml
done

# Format generated C++ code
for setup in $setups; do
    clang-format-18 -i output/frb_$setup.cxx &
done

# Format generated Julia code
for setup in ${setups}; do
    julia --project=@. --eval 'using JuliaFormatter; JuliaFormatter.format_file("'output/frb_${setup}.jl'")' &
done
wait

# Copy kernels into Kotekan
for setup in $setups; do
    cp output/frb_$setup.cxx ../lib/cuda/generated/cudaFRBBeamformer_$setup.cpp
    cp output/frb_$setup.jl ../lib/cuda/generated/FRBBeamformer_$setup.jl
    cp output/frb_$setup.ptx ../lib/cuda/generated/FRBBeamformer_$setup.ptx
    cp output/frb_$setup.yaml ../lib/cuda/generated/FRBBeamformer_$setup.yaml
done

...
class Timespec(object):
    ...
    def __repr__(self):
        return 'tv_sec={:.1f}, tv_nsec={:.1f}'.format(self.tv_sec,
                                                      self.tv_nsec)
...
class PolycoFile(object):
    ...
    head_type = [('name', str), ('date', str), ('UTC', str),
                 ('TMID', float), ('DM', float), ('doppler', float),
                 ('log10rms', float),
                 ('RPHASE', float), ('F0', float), ('obs', str),
                 ('span', float), ('num_coeff', int), ('freq', float),
                 ('bin_phase', float)]
                ...
                if line[:6] == 'TEMPO2':
                    ...
                    for i in range(len(self.head_type)):
                        ...
                        if self.head_type[i][0] == 'bin_phase':
                            continue
                        new_header[self.head_type[i][0]] = self.head_type[i][
                            1](line.strip())
                    ...
                    params = [new_header[k] for k in
                              ('TMID', 'DM', 'RPHASE', 'F0')] + [coeff, ]
        ...
        for p in self.polyco_specs:
            if p['span'] != self.polyco_specs[0]['span']:
                raise Exception(
                    ...
                    + " different lengths.")
        self.spans = np.array([
            np.array((-0.5, 0.5)) * self.polyco_specs[i]['span'] / 60. / 24. +
            self.tmid[i]
    ...
    @classmethod
    def generate(cls, start, end, parfile, dm=None, seg=300., ncoeff=12,
                 max_ha=12.):

        cmd = [
            'tempo2', '-f', parfile, '-polyco',
            '"{:f} {:f} {:d} {:d} {:f} chime inf"'.format(start, end, int(seg),
                                                          ncoeff, max_ha),
            '-tempo1'
        ]
        cmd = ' '.join(cmd)
        ...
        try:
            ...
            print("Command '{}' failed with return code {:d}."
...
@click.option("--end-time", type=float, default=None,
              help="Specify an end time to generate polyco / match segment "
                   "with maximum overlap")
@click.option("--dm", type=float, default=None,
              help="The DM in cm^-3/pc. If not specified will try and get "
                   "from file.")
...
@click.option("--ncoeff", type=int, default=12,
              help="(generate-polyco) Number of polyco coefficients to "
                   "generate.")
@click.option("--max_ha", type=float, default=12.,
              help="(generate-polyco) Maximum hour angle for timing solution "
                   "to span.")
def polyco_config(fname, start_time, generate_polyco, end_time, dm, segment,
                  ncoeff, max_ha):
    if generate_polyco:
        if end_time is None:
            end = start_time + 1.
        ...
        pfile = PolycoFile.generate(start_time, end, fname, dm, segment,
                                    ncoeff, max_ha)
    else:

    ...
    with open(fname, "r") as f:
        ...
        while line != "":
            if line[0] != "#":
                ...
                if not len(line) == 71:
                    raise ValueError(
                        "File {} contains line with unexpected number of entries.".format(fname)
                    )
                # skip index
                _ = line.pop[0]
...
@click.option("--end-time", type=float, default=None,
              help="Specify an end time to generate polyco. Defaults to start time + 1.")
...
@click.option("--tempo-dir", type=str, default=TEMPO_DIR,
...
@click.argument("fname", type=str)
@click.option("--url", type=str, default=COCO_URL, help="URL for coco.")
@click.option("--tempo-dir", type=str, default=TEMPO_DIR, help="TEMPO2 runtime directory")
@click.option("--parfile-dir", type=str, default=PARFILE_DIR,
...
def import_schedule(ctx, fname, url, tempo_dir, parfile_dir, reference):
    ...
    for obs in sched:
        ...
        cur_t = ephem.lsa_to_unix(obs["lst"] / 24. * 360., cur_t)
        end = cur_t + obs["duration"] * 3600.
        ctx.invoke(update_polyco, pfile, unix2mjd(Timespec(cur_t)), url=url, tempo_dir=tempo_dir,
                   end_time=unix2mjd(Timespec(end)), name=obs["name"], schedule=True)
        cur_t = end

...
class N2Raw(object):
    ...
    def frame_struct(cls, size_frame, num_elements, num_stack, num_ev, align_valid):
        ...
        numpy.dtype
            Frame structure.
        """
        layout = N2Buffer.calculate_layout(num_elements, num_stack, num_ev)
        ...
        frame_struct = np.dtype(
            {
                ...
                "formats": [np.uint8, N2Metadata, data_struct],
    ...
    def from_buffer(cls, buffer, size_frame, num_time, num_freq, comet_manager=None):
        """
        Create a N2Raw object from a buffer.
        ...
        frame_start_time_ns = metadata["frame_start_time_ns"]
        fpga_start_tick = metadata["fpga_start_tick"]

        num_prod = metadata["num_prod"]

        time = np.ndarray(
            shape=(num_time, num_freq),
            dtype=[("fpga_start_tick", np.uint64), ("frame_start_time_ns", np.uint64)],
        )
        ...
        for t in range(num_time):
            ...
            for f in range(num_freq):
                if valid_frames[t, f].astype(np.bool):
                    ts.append(
                        frame_start_time_ns[t, f]
                    )
                    fpga.append(fpga_start_tick[t, f])
            ts = np.unique(ts)
            fpga = np.unique(fpga)
            if len(ts) != 1 or len(fpga) != 1:
                if len(ts) == 0 and len(fpga) == 0:
                    ...
                    raise ValueError(
                        "Found {} fpga sequences and {} times for time index {} "
    ...
    def from_file(cls, filename, mode="r", mmap=False):
        ...
        # Get filenames
        filename = N2Raw._parse_filename(filename)
        ...
        time = np.array(
            [(t["fpga_start_tick"], t["frame_start_time_ns"]) for t in index_map["time"]],
            dtype=[("fpga_start_tick", np.uint64), ("frame_start_time_ns", np.uint64)],
        )
    ...
    def create(cls, name, time, freq, input_, prod, nev, stack=None):
        ...
        time : list
            Must be a list of dicts with `fpga_start_tick` and `frame_start_time_ns` keys.
        ...
        if (
            ...
            or "fpga_start_tick" not in time[0]
            or "frame_start_time_ns" not in time[0]
        ):
        ...
        fpga = np.array([t["fpga_start_tick"] for t in time])
        frame_start_time_ns = np.array(
            [(int(np.floor(t["frame_start_time_ns"])), int((t["frame_start_time_ns"] % 1.0) * 1e9)) for t in time],
            dtype=[("tv", np.int64), ("tv_nsec", np.uint64)],
        )
        rawfile.metadata["fpga_start_tick"][:] = fpga[:, np.newaxis]
        rawfile.metadata["frame_start_time_ns"][:] = frame_start_time_ns[:, np.newaxis]
...
def simple_n2raw_data(filename, ntime, nfreq, ninput):
    """Create a simple N2Raw test file that kotekan can read.
    ...
    time = [{"frame_start_time_ns": (10.0 * i), "fpga_start_tick": i} for i in range(ntime)]
    ...
    raw = N2Raw.create(filename, time, freq, input_, prod, nev)
    ...
    del raw
    return N2Raw.from_file(filename, mode="r")

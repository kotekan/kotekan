...
def build_parser(description):
    ...
    ap.add_argument("--fleet-trim-establish-hold-s", type=float, default=300.0,
                    help="freeze the C++ fast loop (gain=leak=0, PRNs still armed) from broker "
                         ...
                         "cause.\n"
                         "\n"
                         "⚠️ THIS IS THE ~8-MINUTE q THRASH AFTER EVERY BROKER RESTART, AND IT "
                         "WAS MEASURED BEFORE IT WAS FIXED. The FLEET-TRIM READBACK series for "
                         "the 17:32 restart on 2026-09-02: max|trim| 0.546 at restart, 2.182 at "
                         "+2 min, then 3.000 -- THE CLAMP -- at +4.5 and +6.5 min, back to 0.591 "
                         "at +8.7 min, against a steady state of 0.2-0.5. The excursion is "
                         "COMMON-MODE (all seven PRNs one sign, mean +1.1 to +1.7), so what the "
                         "loop is chasing is a clock/seed STEP, not per-sat noise: the clock "
                         "bootstrap, the seed audit's whole-chip steps, and the joint feed "
                         "opening 34 s before the first clamped readback.\n"
                         "\n"

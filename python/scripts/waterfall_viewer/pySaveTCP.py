...
info_header = connection.recv(pkt_freqs * 4 * 2 +
                              pkt_elems * 1)
...
def data_listener():
    ...
    while True:
        ...
        for i in np.arange(plot_integration * pkt_elems):
            ...
            d[:, data_pkt_elem_idx] +=\
                np.fromstring(data[pkt_header:], type=np.uint32) * 1.0 /\
                plot_integration / data_pkt_samples_summed
        times = np.roll(times,
                        (data_pkt_frame_idx - last_idx) / plot_integration)
        times[0] = sec_per_pkt_frame * (
            data_pkt_frame_idx - pkt_idx0) + pkt_utc0
        waterfall = np.roll(waterfall,
                            (data_pkt_frame_idx - last_idx) / plot_integration,
                            axis=0)

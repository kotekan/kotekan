...
p.append(ax[0][0].imshow(waterfall[:,:,0],aspect='auto',animated=True,origin='upper',interpolation='nearest', \
...
ax[1][0].set_ylim(colorscale)
im, = ax[1][0].plot(freqlist.reshape(plot_freqs,-1).mean(axis=1),np.nanmean(waterfall[:,:,0],axis=0),'.')
p.append(im)

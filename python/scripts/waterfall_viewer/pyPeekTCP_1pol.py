...
def updatefig(*args):
    ...
    if medsub:
        p[0].set_data(waterfall[:, :, 0] -
                      np.nanmedian(waterfall[:, :, 0], axis=0)[np.newaxis, :])
    else:
    ...
    p[2].set_data(freqlist.reshape(plot_freqs, -1).mean(axis=1),
                  d - spec_baseline[np.newaxis, :])
...
f, ax = plt.subplots(
    2, 2, gridspec_kw={
        'height_ratios': [
            4, 1], 'width_ratios': [
            4, 1]})
f.subplots_adjust(right=0.8, top=0.95, wspace=0., hspace=0., )
ax[-1, -1].axis('off')
...
p.append(ax[0][0].imshow(waterfall[:, :, 0], aspect='auto', animated=True,
                         origin='upper', interpolation='nearest', cmap='gray',
...
ax[1][0].set_ylim(colorscale)
im, = ax[1][0].plot(freqlist.reshape(plot_freqs, -1).mean(axis=1),

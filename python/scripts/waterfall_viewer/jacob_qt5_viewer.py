...
class Settings(QDialog):
    ...
    def UpdateGraph(self):
        ...
        if((self.WriteTrigger) and (not os.path.exists("ViewerGraphsAndData"))):
            os.makedirs("ViewerGraphsAndData") #Insure Directory is made for Graph data
        ...
        if(self.WriteTrigger): #Handles Writting data to file
            FolderName = "ViewerGraphsAndData/" + datetime.datetime.utcnow().strftime('%Y%m%dT%H:%M:%S') #Create New Folder
            os.makedirs(FolderName)
            f = open(FolderName + '/info.txt',"w") #Create Info Text File
            f.write("Current Graph State: %d\nTime of Write: %s\nPlease Find X and Y Axis Data in Current Folder\n\n"%(self.GraphState,FolderName[20:]))
            if(self.GraphState == 2):
                f.write("Reference Time for Calibration: t0 = %s\n\n"%(self.t0.strftime('%Y%m%dT%H:%M:%S')))
            f.write("Graph State Legend\n0 - Pulsar Profile\n1 - Gain\n2 - Calibration\n\nEnjoy!\n")
            f.close()
...
class Window(QDialog):
    ...
    def __init__(self, main_startup, parent=None): #Constructor
        ...
        self.ani = animation.FuncAnimation(self.f, self.Updatefig, frames=100, interval=100) #Animated by self.Updatefig
    ...
    def UpdateLabel(self): #Handles Labels at top of Main window
        ...
        self.info_text_1.setText("Dispersion Measure: " + str(self.psrdata['dmeasure'])) #Pulsar information labels
        self.info_text_2.setText("Fold Period: " + str(self.fold_period))

    def Updatefig(self,*args): #Animates the waterfall plots

        self.tmin=md.date2num(datetime.datetime.fromtimestamp(np.amin(self.times))) #Find new time bounds of waterfalls
        self.tmax=md.date2num(datetime.datetime.fromtimestamp(np.amax(self.times)))
        self.dedispersed = np.copy(self.waterfold) #Create a copy of the folded array to dedisperse
        ...
        for i in np.arange(self.pkt_elems): #For each polirization
            if(self.DeDisperse): #De-disperse
                for j in range(self.dedispersed.shape[1]): #Iterate through frequencies
                    time = (4.148808/1000)*((1/0.4)**2 - (1/(0.4+(128-j)*(0.4/self.dedispersed.shape[1])))**2)*self.psrdata['dmeasure'] #Compute Time lag
                    location = 1*int((time/self.fold_period)*self.dedispersed.shape[0]) #Compute Index
                    self.dedispersed[:,j,i] = np.roll(self.dedispersed[:,j,i],location) #Adjust the frequency by the tim lag
            ...
            self.p[i].set_extent([self.freqlist[0,0],self.freqlist[-1,-1], self.tmin,self.tmax]) #Update Graph Scale
            self.p[i].set_clim(vmin=self.colorscale[0], vmax=self.colorscale[1]) #Update colorbar Scale
    ...
    while True:
        try:
            ...
            for i in np.arange(int(main.local_integration*main.pkt_elems)):
                ...
                data_array = np.fromstring(data[int(main.pkt_header):],dtype=np.uint32) #Unpack Data

...
Widgets:
    ...
    - PulseProfileRadio: Radio Button for toggling what the Graph displays,
                         Pulse Profile
    - GainProfileRadio: Radio Button for toggling what the Graph displays,
                        Gain Profile
    - CalibrationRadio: Radio Button for toggling what the Graph displays,
                        Focus Calibration
    - GraphSlider: Slider used to change Graph's Y axis
    - settingscanvas: The Matplotlib Graph
    - Brightness_Text: Label Reading
                       "Current Frequency For Calibration (MHz): "
    - Brightness_Edit: Text box to edit the brightness
    - WriteButton: Button that write the current graph to disk
Methods:
    WriteGraphToDisk - Function that triggers the data on the graph to be
                       written
...
class Settings(QDialog):
    ...
    def __init__(self, parent):
        ...
        # Pulsar Info
        hbox = QHBoxLayout()
        self.dm_text = QLabel("Dispersion Measure: " +
                              str(self.main.psrdata['dmeasure']))
        hbox.addWidget(self.dm_text)
        # + str(self.main.fold_period))
        self.fold_text = QLabel("Folding Period: ")
        ...
        hbox = QHBoxLayout()
        self.Brightness_Text = QLabel(
            "Center Frequency For Calibration (MHz): ")
    ...
    def ChangeDispersion(self):
        # Toggler Dispersion Flag
        self.main.DeDisperse = not self.main.DeDisperse

    def TogglePulseCenter(self):
        # Toggler Pulsar Centering
        self.main.centerPulsar = not self.main.centerPulsar
    ...
    def UpdateSettings(self, text):
        ...
        self.dm_text.setText("Dispersion Measure: " +
                             str(self.main.psrdata['dmeasure']))
    ...
    def UpdateFreq(self, text):
        ...
        self.Bright_Freq_index = np.round(
            (self.freq_upper - float(text)) * (
                        self.main.waterfall.shape[1] / 400.0))
    ...
    def UpdateWidth(self, text):
        ...
        try:  # Is it a valid entry?
            ...
            x = float(text)
            if (x + self.Bright_Freq > self.freq_upper
        ...
        if (self.Width_index < 0):
            self.Width_index = 0
        elif (self.Bright_Freq_index + self.Width_index >=
              self.main.waterfall.shape[1]):
            self.Width_index = self.main.waterfall.shape[1] - \
                               1 - self.Bright_Freq_index
        self.BrightnessMeasures = []
        self.BrightnessTimes = []
        self.t0 = datetime.datetime.fromtimestamp(np.amin(self.main.times))
    ...
    def UpdateGraph(self):
        ...
        if ((self.WriteTrigger)
                and (not os.path.exists("ViewerGraphsAndData"))):
            # Insure Directory is made for Graph data
            os.makedirs("ViewerGraphsAndData")
            print("Directory Made")

        if (self.GraphState == 0):  # Pulse Profile
            x = np.arange(0, 1, 1.0 /
                          self.main.dedispersed.shape[0])  # phase, x axis
            ...
            for i in range(self.main.MedSubbed.shape[0]):  # Compute Pulse
                y[y.size - 1 - i] = np.mean(
                    self.main.MedSubbed[i, :][(self.main.MedSubbed[i, :] >
                                               (5 * y_lower)) *
            ...
            x = np.linspace(
                self.freq_upper,
                self.freq_lower,
                num=self.main.waterfall.shape[1])  # X axis, frequency
            # Compute Median Values for each freq
            y = np.array(
                [np.median(self.main.waterfall[
                           :, self.main.waterfall.shape[1] - t - 1, 0])
                 for t in range(self.main.waterfall.shape[1])])
            # Y axis scale
            y_lower = np.round(np.median(y)) - 2 * self.GraphSlider.value()
            ...
            # Record Time
            self.BrightnessTimes.append((datetime.datetime.fromtimestamp(
                np.amin(self.main.times)) - self.t0).total_seconds())
            # np.arange(len(self.BrightnessMeasures))
            x = np.array(self.BrightnessTimes)
        ...
        if (self.WriteTrigger):  # Handles Writting data to file
            # Create New Folder
            FolderName = "ViewerGraphsAndData/"\
            ...
            f.write(
                ...
                "and Y Axis Data in Current Folder\n\n" % (
...
class Window(QDialog):
    ...
    def __init__(self, main_startup, parent=None):  # Constructor
        ...
        self.stokes_lookup = ['YX', 'XY', 'YY', 'XX',
                              'LR', 'RL', 'LL', 'RR', 'I', 'Q', 'U', 'V']
        ...
        self.freqlist = np.fromstring(self.info_header[:int(
            self.pkt_freqs) * 4 * 2], dtype=np.float32).reshape(-1, 2)
        ...
        if (self.pkt_int_len > self.total_integration):
            ...
            print("Resetting integration length to {}"
                  .format(self.pkt_int_len))
        ...
        self.waterfall = np.zeros(
            (self.plot_times, int(
                self.plot_freqs), int(
                self.pkt_elems)), dtype=np.float32) + np.nan
        self.countfold = np.zeros(
            (self.plot_phase, int(
                self.plot_freqs), int(
                self.pkt_elems)), dtype=np.float32)
        try:
        ...
        self.times = self.pkt_utc0 - np.arange(self.plot_times) *\
            self.local_integration * self.sec_per_pkt_frame
        self.date_format = md.DateFormatter('%H:%M:%S')

        # Create Waterfall Plots and add to list
        for i in np.arange(self.pkt_elems):
            self.p.append(
                self.ax[0, i].imshow(self.waterfall[:, :, i], aspect='auto',
                                     animated=True, origin='upper',
                                     interpolation='nearest',
                                     cmap='viridis', vmin=self.colorscale[0],
                                     vmax=self.colorscale[1],
                                     extent=[self.freqlist[0, 0],
                                             self.freqlist[-1, -1], self.tmin,
                                             self.tmax]))
        ...
        for i in np.arange(self.pkt_elems):
            self.p.append(
                self.ax[1, i].imshow(self.waterfold[:, :, i], aspect='auto',
                                     animated=True, origin='upper',
                                     interpolation='nearest',
                                     cmap='viridis', vmin=self.colorscale[0],
                                     vmax=self.colorscale[1],
                                     extent=[self.freqlist[0, 0],
                                             self.freqlist[-1, -1], 0, 1]))
        ...
        self.ani = animation.FuncAnimation(
            ...
            self.Updatefig,
            frames=100,
            interval=100)  # Animated by self.Updatefig
    ...
    def UpdateColorbar(self):  # Update Colobar scale
        ...
        if self.medsub:  # If Median Subtracted
            ...
            self.colorscale = [-1 * self.ColorSlider.value() / 10.0,
    ...
    def ChangeMode(self):  # Toggle Viewing Mode
        ...
        if self.medsub:  # Update Color bar
            self.colorscale = [-1 * self.ColorSlider.value() /
                               10.0, self.ColorSlider.value() / 10.0]
        else:
            self.colorscale = [np.median(self.waterfall) - 1 *
                               self.ColorSlider.value() * 2,
                               np.median(self.waterfall) +
                               self.ColorSlider.value() * 2]

    def UpdateLabel(self):  # Handles Labels at top of Main window

        if (self.curtime != 0):  # Calulate J2000 pointing
            Pointing = self.curtime.isot + '        ' + \
                       self.curpoint.altaz.to_string('dms')
            c = SkyCoord(
        ...
        if (self.temp_target == ""):  # Handle the Pulsar information
            ...
            self.title_text.setText(
                self.temp_target + ' (PSRCAT: ' + self.target + ')')
    ...
    def Updatefig(self, *args):  # Animates the waterfall plots
        ...
        self.tmin = md.date2num(datetime.datetime.fromtimestamp(
            np.amin(self.times)))  # Find new time bounds of waterfalls
        self.tmax = md.date2num(
            datetime.datetime.fromtimestamp(
                np.amax(
                    self.times)))
        ...
        for i in np.arange(self.pkt_elems):  # For each polirization
            if (self.DeDisperse):  # De-disperse
                ...
                for j in range(self.dedispersed.shape[1]):
                    ...
                    time = (4.148808 / 1000) *\
                           ((1 / 0.4) ** 2 -
                            (1 / (0.4 + (128 - j) *
                                  (0.4 / self.dedispersed.shape[1]))) ** 2) *\
                           self.psrdata['dmeasure']
                    # Compute Index
                    location = 1 * int((time / self.fold_period)
                                       * self.dedispersed.shape[0])
            ...
            if self.medsub:  # If median subtracted mode selected
                ...
                self.p[i].set_data(self.waterfall[:, :, i] -
                                   np.nanmedian(self.waterfall[:, :, i],
                                                axis=0)[np.newaxis, :])
                # Update Folded Graph
                tmpdata = 10 * np.log10(self.dedispersed[:, :, i] /
                                        self.dedispersed_count[:, :, i])
                if (self.centerPulsar):
                    pulse_max = np.max(np.median(tmpdata, axis=1))
                    pulse_max_index = np.where(
                        np.median(tmpdata, axis=1) == pulse_max)[0]
                    if (pulse_max_index.size > 0):
                        tmpdata = np.roll(
                            tmpdata, int(np.abs(pulse_max_index -
                                                tmpdata.shape[0] / 2)), axis=0)
                self.p[self.pkt_elems +
                       i].set_data(tmpdata -
                                   np.median(tmpdata, axis=0)[np.newaxis, :])
            ...
            self.MedSubbed += (self.dedispersed[:, :, i] /
                               self.dedispersed_count[:, :, i]) -\
                np.median((self.dedispersed[:, :, i] /
                           self.dedispersed_count[:, :, i]),
                          axis=0)[np.newaxis, :]
            # Update Graph Scale
            self.p[i].set_extent(
                [self.freqlist[0, 0], self.freqlist[-1, -1], self.tmin,
                 self.tmax])
            self.p[i].set_clim(
                vmin=self.colorscale[0],
                vmax=self.colorscale[1])  # Update colorbar Scale
...
class Startup(QDialog):
    ...
    def ExitApp(self):
        ...
        tunnelCommand = "ssh -R %d:localhost:2054 %s@%s" % (
            self.Receive_Port, self.Receive_User, self.Receive_Ip)
        process = subprocess.Popen(
            bashCommand,
            stdout=subprocess.PIPE,
        ...
        if (output is not None and tunnelCommand in output.decode("utf-8")):
            ...
            # process = subprocess.Popen(tunnelCommand,
...
def data_listener():  # Listens to Data and updates
    ...
    while True:
        try:
            ...
            d = np.zeros([int(main.pkt_freqs), main.pkt_elems])
            ...
            for i in np.arange(int(main.local_integration * main.pkt_elems)):
                data = main.receive(
                    main.connection,
                    main.pkt_length +
                    main.pkt_header)  # Receive Data from Port
                if (len(data) != main.pkt_length + main.pkt_header):
                    print("Lost Connection to port, exiting...")
                    main.connection.close()
                    return
                data_pkt_frame_idx, data_pkt_elem_idx, data_pkt_samples_summed\
                    = struct.unpack('III', data[:main.pkt_header])
                data_array = np.fromstring(
                    data[int(main.pkt_header):],
                    dtype=np.uint32)  # Unpack Data
                d[:, data_pkt_elem_idx] += data_array * 1.0  # Store Data
                # Store Integration lengths
                n[:, data_pkt_elem_idx] += data_pkt_samples_summed * 1.0
                # n[:,data_pkt_elem_idx][data_array != 0] +=
                # data_pkt_samples_summed * 1.0
                fold_idx = np.array(int(((main.sec_per_pkt_frame *
                                          data_pkt_frame_idx +
                                          0.5 *
                                          main.fold_period) %
                                         main.fold_period) /
                                        main.fold_period *
                                        main.plot_phase), dtype=np.int32)
                # Fold Waterfall
                main.waterfold[int(fold_idx), :, int(data_pkt_elem_idx)] +=\
                    data_array.reshape(
                        -1, int(main.pkt_freqs / main.plot_freqs)).mean(axis=1)
                main.countfold[int(fold_idx), :, int(
                    data_pkt_elem_idx)] += data_pkt_samples_summed
            ...
            main.times[0] = main.sec_per_pkt_frame * \
                (data_pkt_frame_idx - main.pkt_idx0) + main.pkt_utc0
            main.waterfall = np.roll(main.waterfall, int(roll_idx), axis=0)
            # Add data to waterfall
            main.waterfall[0, :, :] =\
                10 * np.log10((d / n).reshape(-1, int(main.pkt_freqs /
                                                      main.plot_freqs),
                                              int(main.pkt_elems))
                              .mean(axis=1))
            # main.waterfall[0,:,:] = (d/1000).reshape(-1,main.pkt_freqs /
            # main.plot_freqs,main.pkt_elems).mean(axis=1)
            last_idx = data_pkt_frame_idx

        except BaseException:  # When Connection is lost, try to reconnect
            ...
            # main.waterfall = np.zeros((self.plot_times,self.plot_freqs,
            # self.pkt_elems),dtype=np.float32) + np.nan;
            print("Reconnected!")
...
def get_pointing():  # Function to get Pointing information
    ...
    while True:
        ARO = EarthLocation(
            lat=45.95550333 * u.deg,
            lon=-78.073040402778 * u.deg,
            height=260.4 * u.m)  # Define current loction (ARO)
...
if __name__ == '__main__':
    # Remove numpy divide error messages

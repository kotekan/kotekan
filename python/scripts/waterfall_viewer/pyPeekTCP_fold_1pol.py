...
# 	char stokes_type;       // - description of stream
#                           // 	 (e.g. V / H pol, Stokes-I / Q / U / V)
        ...
        try:
            ...
            for i in np.arange(local_integration * pkt_elems):
                ...
                if (len(data) != pkt_length + pkt_header):
                    ...
                    connection.close()
                    return data_pkt_frame_idx, data_pkt_elem_idx,
                    data_pkt_samples_summed = struct.unpack('III',
                                                            data[:pkt_header])
                ...
                fold_idx = np.array(((sec_per_pkt_frame * data_pkt_frame_idx
                                      + 0.5 * fold_period)
                                     % fold_period) / fold_period * plot_phase,
                                    dtype=np.int32)
                waterfold[fold_idx, :, data_pkt_elem_idx]\
                    += np.fromstring(data[pkt_header:],
                                     dtype=np.uint32).reshape(
                    -1, pkt_freqs / plot_freqs).mean(axis=1)
                countfold[fold_idx, :, data_pkt_elem_idx] +=\
            ...
            waterfall[0, :, :] =\
                10 * np.log10((d / n).reshape(-1, pkt_freqs / plot_freqs,
                                              pkt_elems).mean(axis=1))
...
times = pkt_utc0 - np.arange(plot_times) * \
        local_integration * sec_per_pkt_frame
...
for i in np.arange(pkt_elems):
    p.append(ax[i].imshow(waterfall[:, :, i], aspect='auto', animated=True,
                          origin='upper', interpolation='nearest',
                          cmap='gray', vmin=colorscale[0], vmax=colorscale[1],
...
for i in np.arange(pkt_elems):
    p.append(ax[1].imshow(waterfold[:, :, i], aspect='auto', animated=True,
                          origin='upper', interpolation='nearest',
                          cmap='gray', vmin=colorscale[0], vmax=colorscale[1],

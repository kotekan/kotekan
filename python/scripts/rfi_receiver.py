...
* Purpose: A server to receive real-time rfi data from kotekan and send to
*          rfi_client.py
* Python Version: 3.6
    ...
    for key, value in _dict.items():
        if (isinstance(value, dict)):
            ...
            if key in cmd.config.keys():
                if (isinstance(cmd.config[key], type(value))):
                    print(
                        "Setting Config Paramter %s to %s" % (key, str(value)))
...
class CommandLine:
    ...
    def __init__(self):
        ...
        self.min_seq = -1
        self.max_seq = -1
        self.config = {'frames_per_packet': 4, 'num_global_freq': 1024,
                       'num_local_freq': 8, 'samples_per_data_set': 32768,
                       'num_elements': 2,
                       'timestep': 2.56e-6, 'bytes_per_freq': 16,
                       'waterfallX': 1024, 'waterfallY': 1024,
                       'bi_frames_per_packet': 10,
                       'sk_step': 256, 'chime_rfi_header_size': 35,
        ...
        parser.add_argument("-H", "--Help", help="Example: Help argument",
                            required=False, default="")
        parser.add_argument("-r", "--receive", help="Example: 127.0.0.1:2900",
                            required=False, default="")
        parser.add_argument("-s", "--send", help="Example: 10.10.10.2:41214",
                            required=False, default="")
        parser.add_argument("-c", "--config",
                            help="Example: ../kotekan/kotekan_opencl_rfi.yaml",
                            required=False, default="")
        parser.add_argument("-m", "--mode", help="Example: vdif, chime",
                            required=False, default="")
        parser.add_argument("-d", "--debug", help="Launch with warnings",
        ...
        if argument.Help:
            print("You have used '-H' or '--Help' with argument: {0}".format(
                argument.Help))
            status = True
        if argument.debug:
            ...
            status = True
        if argument.send:
            print("You have used '-s' or '--send' with argument: {0}".format(
                argument.send))
        ...
        if argument.mode:
            ...
            status = True
        if not status:
            print("Remember: You can use -H or - Help to see configuration "
                  "options")
...
class Stream:
    ...
    def __init__(self, thread_id, mode, header, known_streams):
        ...
        if (encoded_stream_id not in known_streams):
            ...
            if (mode == "pathfinder"):
                self.bins = [self.slot_id + self.link_id * 16 + i * 128 for i
                             in range(header['num_local_freq'])]
            elif (mode == "chime"):
                self.bins = [
                    self.crate * 16 + self.slot_id + self.link_id * 32
                    + self.unused * 256
            ...
            # print("Thread id %d Stream Created %d %d %d %d %d"%(thread_id,
            #       encoded_stream_id, self.slot_id, self.link_id, self.crate,
            #       self.unused))
...
def HeaderCheck(header, app):
    if (header['combined_flag'] != 1):
        print(
            "Header Error: Only Combined RFI values are currently supported ")
        return False
    if (header['sk_step'] != app.config['sk_step']):
        print("Header Error: SK Step does not match config; Got value %d" % (
            header['sk_step']))
        return False
    if (header['num_elements'] != app.config['num_elements']):
        print(
            "Header Error: Number of Elements does not match config; Got "
            "value %d" % (header['num_elements']))
        return False
    if (header['num_timesteps'] != app.config['samples_per_data_set']):
        print(
            "Header Error: Samples per Dataset does not match config; Got "
            "value %d" % (header['num_timesteps']))
        return False
    if (header['num_global_freq'] != app.config['num_global_freq']):
        print(
            "Header Error: Number of Global Frequencies does not match "
            "config; Got value %d" % (header['num_global_freq']))
        return False
    if (header['num_local_freq'] != app.config['num_local_freq']):
        print(
            "Header Error: Number of Local Frequencies does not match config; "
            "Got value %d" % (header['num_local_freq']))
        return False
    if (header['fpga_seq_num'] < 0):
        print("Header Error: Invalid FPGA sequence Number; Got value %d" % (
            header['fpga_seq_num']))
        return False
    if (header['frames_per_packet'] != app.config['frames_per_packet'] and
            header['frames_per_packet'] != app.config['bi_frames_per_packet']):
        print(
            "Header Error: Frames per Packet does not match config; Got value "
            "%d" % (header['frames_per_packet']))
        return False
...
def data_listener(thread_id):
    global waterfall, t_min, app, sk_receive_watchdogs,\
        InitialKotekanConnection
    ...
    HeaderDataType = np.dtype(
        [('combined_flag', np.uint8, 1), ('sk_step', np.uint32, 1),
         ('num_elements', np.uint32, 1),
         ('num_timesteps', np.uint32, 1), ('num_global_freq', np.uint32, 1),
         ('num_local_freq', np.uint32, 1),
         ('frames_per_packet', np.uint32, 1), ('fpga_seq_num', np.int64, 1),
    ...
    while True:
        ...
        if (packet != ''):
            ...
            if (packetCounter % (50 * len(stream_dict) + 1) == 0):
                print("Thread id %d: Receiving Packets from %d Streams" % (
                    thread_id, len(stream_dict)))
            packetCounter += 1

            header = np.fromstring(packet[:RFIHeaderSize],
                                   dtype=HeaderDataType)
            ...
            if (header['encoded_stream_ID'][0] not in known_streams):
                ...
                if (HeaderCheck(header, app) is False):
                    break
                # Add to the dictionary of Streams
                stream_dict[header['encoded_stream_ID'][0]] =\
                    Stream(thread_id, mode, header, known_streams)
            ...
            if (app.min_seq == -1):
                ...
                if (header['fpga_seq_num'][0] > app.max_seq):
                    ...
                    roll_amount = int(-1 * max(
                        (header['fpga_seq_num'][0] - app.max_seq) / (
                            timesteps_per_frame * frames_per_packet),
                        waterfall.shape[1] / 8))
                    # If the roll is larger than the whole waterfall (kotekan
                    # dies and rejoins)
                    if (-1 * roll_amount > waterfall.shape[1]):
                        ...
                        app.min_seq -= roll_amount * timesteps_per_frame\
                            * frames_per_packet
                        app.max_seq = app.min_seq + (waterfall.shape[1] - 1)\
                            * timesteps_per_frame * frames_per_packet
                        # Adjust Time
                        t_min += datetime.timedelta(
                            seconds=-1 * roll_amount * timestep
                            * timesteps_per_frame * frames_per_packet)
            # if(thread_id == 1):
            # print(header['fpga_seq_num'][0],min_seq,timesteps_per_frame,
            # frames_per_packet, (header['fpga_seq_num'][0]-min_seq)/
            # (float(timesteps_per_frame)*frames_per_packet), np.median(data))
            idx = int((header['fpga_seq_num'][0] - app.min_seq) / (
                timesteps_per_frame * frames_per_packet))
            if (idx >= 0 and idx < waterfall.shape[1]):
                waterfall[stream_dict[
                    header['encoded_stream_ID'][0]].bins, idx] = data
                sk_receive_watchdogs[thread_id] = datetime.datetime.now()
            elif (app.debug):
                print("Invalid Packet Location (Ignore on Startup)")
                print(datetime.datetime.utcnow())
                print(
                    "- IDX %d Header Seq %d Min Seq %d Max Seq %d Timesteps "
                    "Per Frame %d Frames Per Packet %d" % (
                        idx, header['fpga_seq_num'][0],
                        app.min_seq, app.max_seq,
                        timesteps_per_frame, frames_per_packet))


def bad_input_listener(thread_id):
    global bi_waterfall, bi_t_min, max_t_pos, app, bi_receive_watchdog,\
        InitialKotekanConnection
    ...
    while True:
        ...
        if (packet != ''):
            ...
            if (packetCounter % (25 * len(stream_dict) + 1) == 0):
                print(
                    "Bad Input Thread (id %d): Receiving Packets from %d "
                    "Streams" % (thread_id, len(stream_dict)))
            packetCounter += 1
            # Read the header
            header = np.fromstring(packet[:RFIHeaderSize],
                                   dtype=HeaderDataType)
            ...
            t = int((header['fpga_seq_num'][0] - bi_min_seq) / (
                timesteps_per_frame * frames_per_packet)) % \
    ...
    while True:
        ...
        while True:
            ...
            if not MESSAGE:
                ...
                if (app.debug):
                    print("Sending Watefall Data %d ..." % (
                        len(waterfall.tostring())))
                ...
                temp_bi_waterfall = (np.sum(bi_waterfall[:, :, :max_t_pos],
                                            axis=2) + np.count_nonzero(
                    bi_waterfall[:, :, :max_t_pos] == -1, axis=2)).astype(
                    float)
                temp_bi_waterfall /= max_t_pos - np.count_nonzero(
                    bi_waterfall[:, :, :max_t_pos] == -1, axis=2)
                # print(np.count_nonzero(bi_waterfall[:,:,:max_t_pos]==-1,
                # axis = 2).shape, np.min(np.count_nonzero(
                # bi_waterfall[:,:,:max_t_pos]==-1, axis = 2)),
                # np.max(np.count_nonzero(bi_waterfall[:,:,:max_t_pos]==-1,
                # axis = 2)))
                # print(np.nanmin(temp_bi_waterfall),
                # np.nanmax(temp_bi_waterfall), np.nanmean(temp_bi_waterfall))
                # print(np.where(temp_bi_waterfall[223,:] > 2)[0].size,
                # temp_bi_waterfall[223,143],
                # np.nanmax(temp_bi_waterfall[223,:]),
                # np.nanmin(temp_bi_waterfall[223,:]))
                if (app.debug):
                    print("Sending Bad Input Watefall Data %d ..." % (
...
def compute_metrics(bi_waterfall, waterfall, metric_dict, max_t_pos, app):
    if (not app.debug):
        ...
        np.warnings.filterwarnings('ignore',
                                   ...
                                   r'(greater|true_divide|double_scalars)')

    # Bad Input Metrics
    mean_bi_waterfall = (np.sum(bi_waterfall[:, :, :max_t_pos],
                                axis=2) + np.count_nonzero(
        bi_waterfall[:, :, :max_t_pos] == -1, axis=2)).astype(float)
    ...
    bad_input_band = 100.0 * np.nanmedian(mean_bi_waterfall, axis=0) / float(
        app.config['bi_frames_per_packet'])
    bad_input_mask = []
    for i in range(bad_input_band.size):
        if (np.isnan(bad_input_band[i])):
            ...
            metric_dict['rfi_input_mask'].labels(i + 10000, i).set(
                bad_input_band[i])
    ...
    # RFI metrics
    bad_locs = np.where(np.sum(waterfall, axis=0) == -1 * waterfall.shape[0])[
        0]
    if (bad_locs.size > 0):
        max_pos = bad_locs[0]
    else:
    ...
    med = np.nanmedian(
        band[band != -1])  # ((M+1)/(M-1))*(2.0*n**2/((n-2)*(n-1)) - 8.0/n - 1)
    std = 2.0 / np.sqrt(M)
    ...
    fbins_mhz = np.round(np.array(
        [800.0 - float(b) * 400.0 / 1024.0 for b in np.arange(band.size)]),
        decimals=2)
    ...
    for i in range(band_perc.size):
        if (np.isnan(band[i]) or band[i] < 0):
            ...
            metric_dict['rfi_band'].labels(fbins_mhz[i], fbins[i]).set(
                band_perc[i])
    overall_rfi = 100.0 * np.sum(
        rfi_mask[waterfall[:, :max_pos] != -1]) / float(
        rfi_mask[waterfall[:, :max_pos] != -1].size)
    ...
    if (app.debug):
        print("Metrics Log:")
        print("    - mean_bi_waterfall: min %.2f max %.2f" % (
            np.nanmin(mean_bi_waterfall), np.nanmax(mean_bi_waterfall)))
        print("    - Bad Input Band Computed: min %.2f max %.2f" % (
        ...
        print("    - Band Computed: min %.2f max %.2f" % (
            np.nanmin(band), np.nanmax(band)))
        print(
            "    - Expectation of SK: %.5f Deviation of SK: %.5f" % (med, std))
        print("    - Band Percent Computed: min %.2f max %.2f" % (
...
def metric_thread():
    ...
    metric_dict['overall_rfi_bad_input'] = Gauge('overall_rfi_bad_input',
                                                 'number_of_bad_inputs')
    metric_dict['rfi_band'] = Gauge('rfi_band', 'med_sk', ['freq', 'freq_bin'])
    metric_dict['rfi_input_mask'] = Gauge('rfi_input_mask',
                                          'defect_likelihood',
                                          ['input_nu', 'input_num'])
...
def watchdog_thread():
    global sk_receive_watchdogs, bi_receive_watchdog, EXIT,\
        InitialKotekanConnection
    print("Starting Watchdog Thread")
    while (not InitialKotekanConnection):
        time.sleep(1)
    while True:
        # print((datetime.datetime.now() - sk_receive_watchdog).
        # total_seconds())
        for i in range(len(sk_receive_watchdogs)):
            if ((datetime.datetime.now() - sk_receive_watchdogs[
                    i]).total_seconds() > 10):
                print("Watchdog Failed: Is kotekan running?")
                EXIT = True
        if ((datetime.datetime.now() - bi_receive_watchdog).total_seconds()
                > 10):
...
if (__name__ == '__main__'):
    ...
    bi_waterfall = np.empty(
        [app.config['num_global_freq'], app.config['num_elements'], 64],
        dtype=np.int8)
    bi_waterfall[:, :, :] = -1  # np.nan
    time.sleep(1)

    sk_receive_watchdogs = [datetime.datetime.now()] * app.config[
        'num_receive_threads']
    receive_threads = []
    for i in range(app.config['num_receive_threads']):
        receive_threads.append(
            threading.Thread(target=data_listener, args=(i,)))
        receive_threads[i].daemon = True
        receive_threads[i].start()

    bi_thread = threading.Thread(target=bad_input_listener,
                                 args=(app.config['num_receive_threads'],))

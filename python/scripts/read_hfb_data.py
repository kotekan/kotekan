...
def main():
    ...
    xticks = [round(num, 3) for num in np.arange(freq_start, freq_start + 0.39, 20 * 0.39 / 128.0)]
    ...
    plt.ylabel('Time (UTC)')
    plt.title('Freq bin: %d, Freq range: %.3f - %.3fMHz, \nBeam: %d, Date: %s' % (freq_bin_num, freq_start, freq_start + 0.39, beam_num, date))

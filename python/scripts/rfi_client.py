...
* File: rfi_client.py
* Purpose: A client to receive and display real-time rfi data from
*          rfi_receiver.py
* Python Version: 3.6
...
class CommandLine:
    ...
    def __init__(self):
        ...
        self.TCP_PORT = 2901
        self.config = {'samples_per_data_set': 32768, 'timestep': 2.56e-6,
                       'waterfallX': 1024, 'num_elements': 2048,
                       'waterfallY': 1024, 'waterfall_request_delay': 10,
                       'colorscale': 1.0, 'num_global_freq': 1024,
        ...
        parser.add_argument("-c", "--config",
                            ...
                            required=False, default="")
        parser.add_argument("-m", "--mode", help="Example: vdif, pathfinder",
                            required=False, default="")
        ...
        if argument.mode:
            ...
            if (argument.mode in self.supportedModes):
                ...
                print(
                    "This mode in currently not supported, reverting to "
                    "default")
...
def init():
    ...
    if (app.mode == 'badinput'):
        med_plot.set_ydata(100.0 * np.nanmedian(waterfall, axis=0) / float(
...
def animate(i):
    im.set_data(waterfall)
    if (app.mode != 'badinput'):
        x_lims = mdates.date2num([t_min, t_min + datetime.timedelta(
            seconds=waterfall.shape[1] * app.config['samples_per_data_set'] *
            app.config['timestep'])])
...
def data_listener():
    ...
    while True:
        ...
        if (data is None):
            print("Connection to %s:%s Broken... Exiting" % (
                  addr[0], str(addr[1])))
            break
        ...
        if (app.mode == 'badinput'):
            print(np.where(100.0 * np.nanmedian(waterfall, axis=0) / float(
                app.config['bi_frames_per_packet']) > 10)[0])
            print(np.where(100.0 * np.nanmedian(waterfall, axis=0) / float(
                app.config['bi_frames_per_packet']) > 10)[0].size)
...
class Callback(object):
    ...
    def SaveData(self, event):
        ...
        newDir = "RFIData/" + datetime.datetime.utcnow().strftime(
            '%Y%m%dT%H:%M:%S')  # Create New Folder
        os.makedirs(newDir)
...
if (__name__ == '__main__'):
    ...
    if (app.mode == 'badinput'):
        ...
        im = ax[0, 0].imshow(waterfall, aspect='auto', cmap='viridis',
                             extent=[x_lims[0], x_lims[1], 400, 800], vmin=0,
        ...
        med_plot, = ax[1, 0].plot(np.arange(waterfall.shape[1]),
                                  np.nanmedian(waterfall, axis=0))
        med_plot_input, = ax[0, 1].plot(np.nanmean(waterfall, axis=1),
                                        800 - 400.0 / 1024.0 * np.arange(
                                            waterfall.shape[0]), 'o')
        ...
        ticks = np.linspace(1 - app.config['colorscale'],
                            1 + app.config['colorscale'], num=10)
        cbar_ax = fig.add_axes([0.913, 0.32, 0.03, 0.58])
        cbar = fig.colorbar(im, cax=cbar_ax, label="SK Value", ticks=ticks)
        # cbar = fig.colorbar(im, cax=cbar_ax, label = "Detection Confidence",
        #                     ticks = ticks)
        ...
        ax[1, 0].set_xlim([x_lims[0], x_lims[1]])
        ax[1, 0].set_ylim([1 - app.config['colorscale'] / 2.0,
                           1 + app.config['colorscale'] / 2.0])
        ax[1, 0].set_ylabel("Median SK Value")
        ax[0, 1].set_xlabel("Median SK Value")
        ax[0, 1].set_xlim(
            [1 - app.config['colorscale'], 1 + app.config['colorscale']])
    ...
    anim = animation.FuncAnimation(fig, animate, init_func=init,
                                   frames=waterfall.size, interval=50)
    ...
    while not Connected:
        try:
            ...
            print("Could not connect to %s:%s Trying again in 5 seconds" % (
                  addr[0], addr[1]))
            time.sleep(5)

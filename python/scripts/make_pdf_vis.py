...
data_format = np.dtype(
    [("vis", [("real", np.int32), ("imag", np.int32)], 32896)])
...
centers = [800 - i * 400. / 1024. for i in range(1024)]
...
for i in range(0, 64):
    ...
    ma = ax.matshow(vis_real[i], cmap=cmap, aspect='equal',
                    interpolation='none')
    bin_num = bin_number(16, i / 8, i % 8)
    title("Real visibility matrix for slot 16, link " + str(
        i / 8) + ", index " + str(i % 8) + ", frequency " + "{:4.1f}".format(
        frequency(bin_num)) + "MHz, bin_num: " + str(bin_num))
    ...
    fig = figure()
    ax = fig.add_subplot(111)
    ma = ax.matshow(vis_imag[i], cmap=cmap, aspect='equal',
                    interpolation='none')
    bin_num = bin_number(16, i / 8, i % 8)
    title("Imaginary visibility matrix for slot 16, link " + str(
        i / 8) + ", index " + str(i % 8) + ", frequency " + "{:4.1f}".format(
        frequency(bin_num)) + "MHz, bin_num: " + str(bin_num))
    xlabel("Element Number")
    ylabel("Element Number")

...
class KotekanPowerStream():
   ...
   def receive_frame(self):
      for i in np.arange(self.frame_nvis):
        try:
        ...
        if (self.elem_idx != i):
            print("Missed index, skipping!")
            self.databuf[:,i,:] = np.zeros(self.data.shape)
            break
        self.databuf[:,self.elem_idx,:] = self.data
   ...
   def connect_tcp(self):
      ...
      self.databuf = np.zeros((self.ntime, self.frame_nvis, self.frame_nfreq), dtype=np.float32)
...
#            KotekanConn.frame_raw_cad * KotekanConn.frame_int_len * (KotekanConn.frame_idx - KotekanConn.frame_idx0))
      ...
      send_data = np.int8(MSG_TYPE['timestep']).tobytes() + \
                  ...
                  np.mean(KotekanConn.databuf.reshape(self.ntime, self.nvis, self.sendfreq, -1),axis=3).tobytes()
      self.sendMessage(send_data,isBinary=True)
      if self.recording:
         self.output_file.write(np.float64(sample_time).tobytes() + KotekanConn.databuf.tobytes())
...
if __name__ == '__main__':
   ...
   import webbrowser
   webbrowser.open('http://localhost:8080/index_crosscorr.html')

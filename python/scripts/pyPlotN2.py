...
if (data_type == "CORR_MATRIX"): #N^2
    ...
    num_sets = data_length / (num_blocks * block_dims[0]*block_dims[1]*block_dims[2])

    target.write("Data Length: {}\n".format(data_length))
    target.write("Elements: {}\n".format(num_elements))
    target.write("Block Dims: {}\n".format(block_dims))
    target.write("Blocks: {}\n".format(num_blocks))
    target.write("Data Sets: {}\n".format(num_sets))
    target.close()

    data = np.fromfile(sys.stdin, dtype=np.int32, count=data_length, sep='')
    data=data.reshape([num_sets,num_blocks,block_dims[0]*block_dims[1],2])

#    num_sets = 1

    pdf = PdfPages("correlation_{}.pdf".format(time.time()))
    for d in np.arange(num_sets):
        corr_matrix = np.zeros([num_elements,num_elements,2])
        x_blk=0
        y_blk=0
        for i in np.arange(num_blocks):
            corr_matrix[x_blk:x_blk+block_dims[0],y_blk:y_blk+block_dims[1],:]=data[d,i,:,:].reshape(block_dims).transpose(1,0,2)
            x_blk+=block_dims[0]
        ...
        pdf.savefig(dpi=axesdpi,bbox_inches='tight')
        plt.close()

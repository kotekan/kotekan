...
private:
    ...
    static constexpr std::ptrdiff_t W_length = type_total_bytes(W_type) * 2 * 16 * 16 * 2 * 4;
    ...
    static constexpr std::ptrdiff_t Ebar_length = type_total_bytes(Ebar_type) * 256 * 2 * 4 * 16384;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 16 * 4;
...
cudaFRBBeamformer_hirax_U4::cudaFRBBeamformer_hirax_U4(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "FRBBeamformer_hirax_U4_");
    }
...
cudaEvent_t cudaFRBBeamformer_hirax_U4::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA kernel FRBBeamformer_hirax_U4 on GPU frame {:d}", gpu_frame_id);
    ...
    // Check outputs for poison
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            if (I_found_error)
                WARN("CUDA kernel FRBBeamformer_hirax_U4 returned produced non-finite results");
        } // for chunk

...
private:
    ...
    static constexpr std::ptrdiff_t W_length = chord_datatype_bytes(W_type) * 2 * 8 * 12 * 2 * 512;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 6 * 512;
...
cudaFRBBeamformer_pathfinder_U8::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA FRBBeamformer_pathfinder_U8 on GPU frame {:d}", gpu_frame_id);

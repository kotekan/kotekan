...
private:
    ...
    static constexpr std::ptrdiff_t G_U4_length = chord_datatype_bytes(G_U4_type) * 4;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 1024 * 2 * 4 * 16384;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 4 * 256;
...
cudaEvent_t cudaUpchannelizer_chime_U4::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    DEBUG("Running CUDA Upchannelizer_chime_U4 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA Upchannelizer_chime_U4 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaUpchannelizer_chime_U4 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif

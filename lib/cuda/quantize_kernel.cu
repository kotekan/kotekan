...
__global__ void quantize (const __half2 *in_base, __half2 *outf_base, unsigned int *outi_base, const int *index_array)
{
    __half2 * outf = outf_base + index_array[64*blockIdx.x + 32];
    unsigned int * outi = outi_base + index_array[64*blockIdx.x + 33];
	...
	__half scale = __hadd(__low2half(h2sum), __high2half(h2sum));
        ...
        if (threadIdx.x == 0)
        {
            outf[chunk] = __halves2half2(mean, scale);
        }

...
private:
    ...
    static constexpr std::size_t E_length = chord_datatype_bytes(E_type) * 64 * 2 * 2048 * 512;
    ...
    static constexpr std::size_t I_length = chord_datatype_bytes(I_type) * 16 * 24 * 2048 * 16;
...
cudaFRBBeamformer_pathfinder::cudaFRBBeamformer_pathfinder(Config& config,
    ...
    if (inst == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "FRBBeamformer_pathfinder_");
    }
...
std::int64_t
cudaFRBBeamformer_pathfinder::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_produced_elements(num_available_elements) * cuda_downsampling_factor;
}
std::int64_t
cudaFRBBeamformer_pathfinder::num_produced_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) / cuda_downsampling_factor;
}

std::int64_t
cudaFRBBeamformer_pathfinder::num_processed_elements(std::int64_t num_available_elements) const {
    assert(num_available_elements >= cuda_number_of_timesamples);
    return cuda_number_of_timesamples;
}

int cudaFRBBeamformer_pathfinder::wait_on_precondition() {
...
cudaEvent_t cudaFRBBeamformer_pathfinder::execute(cudaPipelineState& /*pipestate*/,
    ...
    *(std::int32_t*)Tbarmax_host.data() = mod(Tbarmin, Tbar_ringbuf) + Tbarlength;
    ...
    const std::string symname = "FRBBeamformer_pathfinder_" + std::string(kernel_symbol);

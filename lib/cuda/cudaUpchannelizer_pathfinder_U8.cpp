...
private:
    ...
    const char* const kernel_symbol =
        "_Z7upchan85Int32S_S_S_S_S_13CuDeviceArrayI9Float16x2Li1ELi1EES0_I6Int4x8Li1ELi1EES4_S0_IS_"
        "Li1ELi1EE";
    ...
    //
    // G_U8: upchan_U8_gain_name
    static constexpr const char* G_U8_quantity = "G_U8";
    static constexpr kotekan::DataType G_U8_type = kotekan::float16;
    enum G_U8_indices {
        G_U8_index_Fbar,
        G_U8_rank,
    };
    ...
    static constexpr std::ptrdiff_t G_U8_length = type_total_bytes(G_U8_type) * 512;
    ...
    static_assert(G_U8_length == type_total_bytes(G_U8_type) * G_U8_strides[G_U8_rank]);
    //
    // E: voltage_name
    ...
    static constexpr std::ptrdiff_t Ebar_length = type_total_bytes(Ebar_type) * 64 * 2 * 512 * 4096;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 8 * 384;
...
cudaUpchannelizer_pathfinder_U8::cudaUpchannelizer_pathfinder_U8(Config& config,
    ...
    G_U8_name(config.get<std::string>(unique_name, "upchan_U8_gain_name")),
    E_name(config.get<std::string>(unique_name, "voltage_name")),
    Ebar_name(config.get<std::string>(unique_name, "upchan_U8_voltage_name")),
    ...
    output_ringbuf_signal(dynamic_cast<RingBuffer*>(host_buffers.get_generic_buffer(
        config.get<std::string>(unique_name, "upchan_U8_voltage_signal_out")))),
    ...
    gpu_buffers_used.push_back(std::make_tuple(G_U8_name, true, true, false));
...
cudaUpchannelizer_pathfinder_U8::execute(cudaPipelineState& /*pipestate*/,
    ...
    const std::string G_U8_memname = G_U8_name + "_buffer";
    ...
    assert(std::strncmp(G_U8_meta->name, G_U8_quantity, sizeof G_U8_meta->name) == 0);
    ...
    *Ebar_meta = *E_meta;
    ...
    DEBUG("Finished CUDA Upchannelizer_pathfinder_U8 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_pathfinder_U8 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_pathfinder_U8 returned 'info' value {:d} "

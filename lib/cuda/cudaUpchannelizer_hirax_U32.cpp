/**
 * @file
 * @brief CUDA Upchannelizer_hirax_U32 kernel
 *
...
/**
 * @class cudaUpchannelizer_hirax_U32
 * @brief cudaCommand for Upchannelizer_hirax_U32
 */
class cudaUpchannelizer_hirax_U32 : public cudaCommand {
public:
    cudaUpchannelizer_hirax_U32(Config& config, const std::string& unique_name,
                                bufferContainer& host_buffers, cudaDeviceInterface& device,
                                const int instance_num);
    virtual ~cudaUpchannelizer_hirax_U32();
...
private:
    ...
    // Kernel arguments:
    enum class args { Tmin, Tmax, Tbarmin, Tbarmax, Fmin, Fmax, G_U32, E, Ebar_U32, info, count };
    ...
    //
    // G_U32: gpu_mem_gain
    static constexpr const char* G_U32_name = "G_U32";
    static constexpr chordDataType G_U32_type = float16;
    enum G_U32_indices {
        G_U32_index_Fbar_U32,
        G_U32_rank,
    };
    static constexpr std::array<const char*, G_U32_rank> G_U32_labels = {
        "Fbar_U32",
    };
    static constexpr std::array<std::ptrdiff_t, G_U32_rank> G_U32_lengths = {
        2048,
    };
    static constexpr std::ptrdiff_t G_U32_length = chord_datatype_bytes(G_U32_type) * 2048;
    static_assert(G_U32_length <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    static constexpr auto G_U32_calc_stride = [](int dim) {
        std::ptrdiff_t str = 1;
        for (int d = 0; d < dim; ++d)
            str *= G_U32_lengths[d];
        return str;
    };
    static constexpr std::array<std::ptrdiff_t, G_U32_rank + 1> G_U32_strides = {
        G_U32_calc_stride(G_U32_index_Fbar_U32),
        G_U32_calc_stride(G_U32_rank),
    };
    static_assert(G_U32_length == chord_datatype_bytes(G_U32_type) * G_U32_strides[G_U32_rank]);
    //
    ...
    //
    // Ebar_U32: gpu_mem_output_voltage
    ...
    static constexpr std::array<std::ptrdiff_t, Ebar_U32_rank> Ebar_U32_lengths = {
        256,
        2,
        2048,
        1024,
    };
    static constexpr std::ptrdiff_t Ebar_U32_length =
        chord_datatype_bytes(Ebar_U32_type) * 256 * 2 * 2048 * 1024;
    static_assert(Ebar_U32_length <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    ...
    const std::string G_U32_memname;
    ...
    static constexpr std::ptrdiff_t E_T_sample_bytes = chord_datatype_bytes(E_type)
                                                       ...
                                                       * E_lengths[E_index_F];
    static constexpr std::ptrdiff_t Ebar_U32_Tbar_U32_sample_bytes =
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_hirax_U32);

cudaUpchannelizer_hirax_U32::cudaUpchannelizer_hirax_U32(Config& config,
                                                         const std::string& unique_name,
                                                         bufferContainer& host_buffers,
                                                         cudaDeviceInterface& device,
                                                         const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_hirax_U32", "Upchannelizer_hirax_U32.ptx"),
    G_U32_memname(config.get<std::string>(unique_name, "gpu_mem_gain")),
    E_memname(config.get<std::string>(unique_name, "gpu_mem_input_voltage")),
    Ebar_U32_memname(config.get<std::string>(unique_name, "gpu_mem_output_voltage")),
    ...
    assert(output_ringbuf_signal->size == Ebar_U32_length);
    ...
    gpu_buffers_used.push_back(std::make_tuple(G_U32_memname, true, true, false));
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_hirax_U32_");
    }
...
std::int64_t
cudaUpchannelizer_hirax_U32::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) - cuda_algorithm_overlap;
}
std::int64_t
cudaUpchannelizer_hirax_U32::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_hirax_U32::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_hirax_U32::wait_on_precondition() {
    ...
    // to bytes
    const std::ptrdiff_t output_bytes = Tbarlength * Ebar_U32_Tbar_U32_sample_bytes;
    ...
    assert(mod(output_cursor, Ebar_U32_Tbar_U32_sample_bytes) == 0);
    Tbarmin = output_cursor / Ebar_U32_Tbar_U32_sample_bytes;
...
cudaEvent_t cudaUpchannelizer_hirax_U32::execute(cudaPipelineState& /*pipestate*/,
                                                 const std::vector<cudaEvent_t>& /*pre_events*/) {
    pre_execute();

    void* const G_U32_memory =
        args::G_U32 == args::E ? device.get_gpu_memory(G_U32_memname, input_ringbuf_signal->size)
        : args::G_U32 == args::Ebar_U32
            ? device.get_gpu_memory(G_U32_memname, output_ringbuf_signal->size)
        : args::G_U32 == args::G_U32 ? device.get_gpu_memory(G_U32_memname, G_U32_length)
                                     : device.get_gpu_memory_array(G_U32_memname, gpu_frame_id,
                                                                   _gpu_buffer_depth, G_U32_length);
    void* const E_memory =
        args::E == args::E          ? device.get_gpu_memory(E_memname, input_ringbuf_signal->size)
        : args::E == args::Ebar_U32 ? device.get_gpu_memory(E_memname, output_ringbuf_signal->size)
        : args::E == args::G_U32
    ...
    // G_U32 is an input buffer: check metadata
    const std::shared_ptr<metadataObject> G_U32_mc =
        args::G_U32 == args::E ? input_ringbuf_signal->get_metadata(0)
                               : device.get_gpu_memory_array_metadata(G_U32_memname, gpu_frame_id);
    assert(G_U32_mc);
    assert(metadata_is_chord(G_U32_mc));
    const std::shared_ptr<chordMetadata> G_U32_meta = get_chord_metadata(G_U32_mc);
    DEBUG("input G_U32 array: {:s} {:s}", G_U32_meta->get_type_string(),
          G_U32_meta->get_dimensions_string());
    assert(std::strncmp(G_U32_meta->name, G_U32_name, sizeof G_U32_meta->name) == 0);
    assert(G_U32_meta->type == G_U32_type);
    assert(G_U32_meta->dims == G_U32_rank);
    for (std::ptrdiff_t dim = 0; dim < G_U32_rank; ++dim) {
        assert(std::strncmp(G_U32_meta->dim_name[G_U32_rank - 1 - dim], G_U32_labels[dim],
                            sizeof G_U32_meta->dim_name[G_U32_rank - 1 - dim])
               == 0);
        if (args::G_U32 == args::E && dim == E_index_T) {
            assert(G_U32_meta->dim[G_U32_rank - 1 - dim] <= int(G_U32_lengths[dim]));
            assert(G_U32_meta->stride[G_U32_rank - 1 - dim] <= G_U32_strides[dim]);
        } else {
            assert(G_U32_meta->dim[G_U32_rank - 1 - dim] == int(G_U32_lengths[dim]));
            assert(G_U32_meta->stride[G_U32_rank - 1 - dim] == G_U32_strides[dim]);
        }
    ...
    //
    // Ebar_U32 is an output buffer: set metadata
    std::shared_ptr<metadataObject> const Ebar_U32_mc =
        args::Ebar_U32 == args::Ebar_U32 ? output_ringbuf_signal->get_metadata(0)
                                         : device.create_gpu_memory_array_metadata(
                                             Ebar_U32_memname, gpu_frame_id, E_mc->parent_pool);
    std::shared_ptr<chordMetadata> const Ebar_U32_meta = get_chord_metadata(Ebar_U32_mc);
    *Ebar_U32_meta = *E_meta;
    std::strncpy(Ebar_U32_meta->name, Ebar_U32_name, sizeof Ebar_U32_meta->name);
    Ebar_U32_meta->type = Ebar_U32_type;
    Ebar_U32_meta->dims = Ebar_U32_rank;
    for (std::ptrdiff_t dim = 0; dim < Ebar_U32_rank; ++dim) {
        std::strncpy(Ebar_U32_meta->dim_name[Ebar_U32_rank - 1 - dim], Ebar_U32_labels[dim],
                     sizeof Ebar_U32_meta->dim_name[Ebar_U32_rank - 1 - dim]);
        Ebar_U32_meta->dim[Ebar_U32_rank - 1 - dim] = Ebar_U32_lengths[dim];
        Ebar_U32_meta->stride[Ebar_U32_rank - 1 - dim] = Ebar_U32_strides[dim];
    }
    DEBUG("output Ebar_U32 array: {:s} {:s}", Ebar_U32_meta->get_type_string(),
    ...
    array_desc G_U32_arg(G_U32_memory, G_U32_length);
    ...
    void* args[] = {
        ...
        &Fmax_arg, &G_U32_arg, &E_arg,    &Ebar_U32_arg, &info_arg,
    };
    ...
    const std::ptrdiff_t Tbar_ringbuf =
        output_ringbuf_signal->size / Ebar_U32_Tbar_U32_sample_bytes;
    ...
    // Update metadata
    Ebar_U32_meta->dim[Ebar_U32_rank - 1 - Ebar_U32_index_Tbar_U32] = Tbarlength;
    assert(Ebar_U32_meta->dim[Ebar_U32_rank - 1 - Ebar_U32_index_Tbar_U32]
           <= int(Ebar_U32_lengths[Ebar_U32_index_Tbar_U32]));
    // Since we use a ring buffer we do not need to update `meta->sample0_offset`

    assert(Ebar_U32_meta->nfreq >= 0);
    assert(Ebar_U32_meta->nfreq == E_meta->nfreq);
    for (int freq = 0; freq < Ebar_U32_meta->nfreq; ++freq) {
        Ebar_U32_meta->freq_upchan_factor[freq] =
            cuda_upchannelization_factor * E_meta->freq_upchan_factor[freq];
        Ebar_U32_meta->half_fpga_sample0[freq] =
            E_meta->half_fpga_sample0[freq] + cuda_number_of_taps - 1;
        Ebar_U32_meta->time_downsampling_fpga[freq] =
    ...
    const std::string symname = "Upchannelizer_hirax_U32_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA Upchannelizer_hirax_U32 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaUpchannelizer_hirax_U32 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
...
void cudaUpchannelizer_hirax_U32::finalize_frame() {
    const std::ptrdiff_t Tlength = Tmax - Tmin;
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;
    ...
    DEBUG("    Produced bytes:   {:d}", Tbar_produced * Ebar_U32_Tbar_U32_sample_bytes);
    output_ringbuf_signal->finish_write(unique_name, instance_num,
                                        Tbar_produced * Ebar_U32_Tbar_U32_sample_bytes);

...
cudaEvent_t cudaUpchannelizer_chord_U128::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    DEBUG("Finished CUDA Upchannelizer_chord_U128 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_chord_U128 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_chord_U128 returned 'info' value {:d} "

/**
 * @file
 * @brief CUDA FRBBeamformer_hirax kernel
 *
...
/**
 * @class cudaFRBBeamformer_hirax
 * @brief cudaCommand for FRBBeamformer_hirax
 */
class cudaFRBBeamformer_hirax : public cudaCommand {
public:
    cudaFRBBeamformer_hirax(Config& config, const std::string& unique_name,
                            bufferContainer& host_buffers, cudaDeviceInterface& device,
                            const int inst);
    virtual ~cudaFRBBeamformer_hirax();
...
private:
    ...
    static constexpr std::size_t E_length = chord_datatype_bytes(E_type) * 256 * 2 * 1024 * 512;
    ...
    static constexpr std::size_t I_length = chord_datatype_bytes(I_type) * 32 * 32 * 1024 * 32;
...
REGISTER_CUDA_COMMAND(cudaFRBBeamformer_hirax);

cudaFRBBeamformer_hirax::cudaFRBBeamformer_hirax(Config& config, const std::string& unique_name,
    ...
    if (inst == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "FRBBeamformer_hirax_");
    }
...
cudaFRBBeamformer_hirax::~cudaFRBBeamformer_hirax() {}

std::int64_t
cudaFRBBeamformer_hirax::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_produced_elements(num_available_elements) * cuda_downsampling_factor;
}
std::int64_t
cudaFRBBeamformer_hirax::num_produced_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) / cuda_downsampling_factor;
}

std::int64_t
cudaFRBBeamformer_hirax::num_processed_elements(std::int64_t num_available_elements) const {
    assert(num_available_elements >= cuda_number_of_timesamples);
    return cuda_number_of_timesamples;
}

int cudaFRBBeamformer_hirax::wait_on_precondition() {
...
cudaEvent_t cudaFRBBeamformer_hirax::execute(cudaPipelineState& /*pipestate*/,
    ...
    *(std::int32_t*)Tbarmax_host.data() = mod(Tbarmin, Tbar_ringbuf) + Tbarlength;
    ...
    const std::string symname = "FRBBeamformer_hirax_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA FRBBeamformer_hirax on GPU frame {:d}", gpu_frame_id);
    const CUresult err =
    ...
    for (std::size_t i = 0; i < info_host.size(); ++i)
        if (info_host[i] != 0)
            ERROR("cudaFRBBeamformer_hirax returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);

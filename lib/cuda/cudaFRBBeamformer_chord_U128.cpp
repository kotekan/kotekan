...
private:
    ...
    static constexpr std::ptrdiff_t S_length = type_total_bytes(S_type) * 2 * 576;
    ...
    static constexpr std::ptrdiff_t W_length = type_total_bytes(W_type) * 2 * 24 * 24 * 2 * 128;
    ...
    static constexpr std::ptrdiff_t Ebar_length = type_total_bytes(Ebar_type) * 512 * 2 * 128 * 256;
    ...
    static constexpr std::ptrdiff_t I_length = type_total_bytes(I_type) * 48 * 48 * 512 * 1024;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 24 * 128;
...
cudaEvent_t cudaFRBBeamformer_chord_U128::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA kernel FRBBeamformer_chord_U128 on GPU frame {:d}", gpu_frame_id);
    ...
    // Check outputs for poison
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            if (I_found_error)
                WARN("CUDA kernel FRBBeamformer_chord_U128 returned produced non-finite results");
        } // for chunk

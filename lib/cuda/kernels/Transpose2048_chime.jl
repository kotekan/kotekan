@fastmath @inbounds(
    begin #= /localhome/eschnett/src/kotekan/julia/kernels/xpose2048.jl:324 =#
        info = 1
        info_memory[(((IndexSpaces.assume_inrange(IndexSpaces.cuda_threadidx()::Int32, 0, 32) % 32) % 32 + ((IndexSpaces.assume_inrange(IndexSpaces.cuda_warpidx()::Int32, 0, 16) % 16) % 16) * 32 + ((IndexSpaces.assume_inrange(IndexSpaces.cuda_blockidx()::Int32, 0, 16) % 16) % 16) * 512) + 0) + 0x01] =
            info
        ...
        for time_loop in 0:32:65535
            ...
            let register_loop = 0
                ...
                E_shared[let
                    addr = (((IndexSpaces.assume_inrange(IndexSpaces.cuda_threadidx()::Int32, 0, 32) % 4 + (register_loop::Int32 % 32) * 4 + (IndexSpaces.assume_inrange(IndexSpaces.cuda_warpidx()::Int32, 0, 16) % 8) * 128) % 1024) * 8 + ((((IndexSpaces.assume_inrange(time_loop::Int32, 0, 32, 65536) ÷ 32) % 2048) * 32 + ((IndexSpaces.assume_inrange(IndexSpaces.cuda_threadidx()::Int32, 0, 32) ÷ 4) % 8) * 4) ÷ 4) % 8 + (((IndexSpaces.assume_inrange(IndexSpaces.cuda_warpidx()::Int32, 0, 16) ÷ 8) % 2) % 2) * 8192) + 0
                    time = addr % (8i32)

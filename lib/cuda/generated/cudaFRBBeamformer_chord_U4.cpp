/**
 * @file
 * @brief CUDA FRBBeamformer_chord_U4 kernel
 *
...
/**
 * @class cudaFRBBeamformer_chord_U4
 * @brief cudaCommand for FRBBeamformer_chord_U4
 */
class cudaFRBBeamformer_chord_U4 : public cudaCommand {
public:
    cudaFRBBeamformer_chord_U4(Config& config, const std::string& unique_name,
                               bufferContainer& host_buffers, cudaDeviceInterface& device,
                               const int instance_num);
    virtual ~cudaFRBBeamformer_chord_U4();
...
REGISTER_CUDA_COMMAND(cudaFRBBeamformer_chord_U4);

cudaFRBBeamformer_chord_U4::cudaFRBBeamformer_chord_U4(Config& config,
                                                       const std::string& unique_name,
                                                       bufferContainer& host_buffers,
                                                       cudaDeviceInterface& device,
                                                       const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "FRBBeamformer_chord_U4", "FRBBeamformer_chord_U4.ptx"),
...
std::int64_t
cudaFRBBeamformer_chord_U4::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_produced_elements(num_available_elements) * cuda_downsampling_factor;
}
std::int64_t
cudaFRBBeamformer_chord_U4::num_produced_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) / cuda_downsampling_factor;
}

std::int64_t
cudaFRBBeamformer_chord_U4::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaFRBBeamformer_chord_U4::wait_on_precondition() {
    {
...
cudaEvent_t cudaFRBBeamformer_chord_U4::execute(cudaPipelineState& /*pipestate*/,
    ...
    if (instance_num == 0 && !did_set_metadata) {
        ...
        if (!(Telescope::instance().get_grid_size_x() <= std::uint64_t(cuda_dish_layout_M)
            ...
            FATAL_ERROR("Telescope dish grid {:d}x{:d} does not fit the dish layout {:d}x{:d} "
                        "for which kernel FRBBeamformer_chord_U4 was compiled",
        ...
        if (Fbar_out_max - Fbar_out_min != Fbar_in_max - Fbar_in_min)
            FATAL_ERROR("Input and output frequency spans have different lengths for kernel "
                        "FRBBeamformer_chord_U4: input [{:d},{:d}), output [{:d},{:d})",
        ...
        if (!(I_nfreq >= 0 && I_nfreq <= I_all_nfreq))
            FATAL_ERROR(
                "Kernel FRBBeamformer_chord_U4 would write {:d} frequencies, but its output "
                "buffer I holds {:d}",
                I_nfreq, I_all_nfreq);
        ...
        if (cuda_upchannelization_factor > 1 && I_nfreq != Ebar_nfreq)
            FATAL_ERROR("Kernel FRBBeamformer_chord_U4 writes {:d} of the {:d} upchannelized input "
        ...
        if (!I_has_metadata)
            ...
            FATAL_ERROR("Another producer of buffer I set time_downsampling_fpga={:d}, but kernel "
                        "FRBBeamformer_chord_U4 produces {:d}",
        ...
        if (W_nfreq != I_nfreq)
            FATAL_ERROR("Weight buffer W holds {:d} frequencies, but kernel FRBBeamformer_chord_U4 "
                        "processes {:d}",
                        W_nfreq, I_nfreq);
        ...
        for (int freq = 0; freq < W_nfreq; ++freq)
            if (I_coarse_freq.at(Fbar_out_min + freq) != W_coarse_freq.at(freq))
                FATAL_ERROR("Weight buffer W is for coarse frequency {:d} at index {:d}, but "
                            "kernel FRBBeamformer_chord_U4 processes coarse frequency {:d} there",
    ...
    if (!I_buffer.has_metadata())
        FATAL_ERROR("Output buffer I has no metadata; kernel FRBBeamformer_chord_U4 cannot run");
    ...
    if (!(0 <= Fbar_in_min && Fbar_in_min <= Fbar_in_max))
        FATAL_ERROR("Invalid input frequency span for kernel FRBBeamformer_chord_U4: "
    ...
    if (!(0 <= Fbar_out_min && Fbar_out_min <= Fbar_out_max))
        FATAL_ERROR("Invalid output frequency span for kernel FRBBeamformer_chord_U4: "
                    "Fbar_out_min={:d}, Fbar_out_max={:d} "
                    "(require 0 <= Fbar_out_min <= Fbar_out_max)",
                    Fbar_out_min, Fbar_out_max);
    if (Fbar_out_max - Fbar_out_min != Fbar_in_max - Fbar_in_min)
        FATAL_ERROR("Input and output frequency spans have different lengths for kernel "
                    "FRBBeamformer_chord_U4: input [{:d},{:d}) holds {:d} frequencies, "
                    "output [{:d},{:d}) holds {:d}",
    ...
    if (!(0 <= blocks && blocks <= max_blocks))
        FATAL_ERROR("Kernel FRBBeamformer_chord_U4 would launch {:d} blocks, but the `info` buffer "
    ...
    if (!did_init_host_S_buffer) {
        ...
        if (array_dish_count + surplus_dishes.size()
            ...
            FATAL_ERROR(
                "Dish bookkeeping is inconsistent for kernel FRBBeamformer_chord_U4: {:d} array "
        ...
        if (surplus_idx != surplus_dishes.size())
            FATAL_ERROR(
                "Kernel FRBBeamformer_chord_U4 placed {:d} of {:d} surplus dishes on the dish grid",
        ...
        for (std::size_t s = 0; s < host_S_buffer.size(); ++s)
            if (host_S_buffer.at(s) < 0)
                FATAL_ERROR(
                    "Kernel FRBBeamformer_chord_U4 left entry {:d} of the dish location map `S` "
                    "unassigned (value {:d})",
    ...
    const std::string symname = "FRBBeamformer_chord_U4_" + std::string(kernel_symbol);
    ...
    if (poison_buffers) {
        ...
        if (error_code != 0)
            ERROR("CUDA kernel FRBBeamformer_chord_U4 returned error code: {}", error_code);
            ...
            for (int block = 0; block < blocks; ++block) {
                for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                    for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                        ...
                        if (val != 0)
                            ERROR("CUDA kernel FRBBeamformer_chord_U4 returned 'info' value {:d} "
...
void cudaFRBBeamformer_chord_U4::finalize_frame() {

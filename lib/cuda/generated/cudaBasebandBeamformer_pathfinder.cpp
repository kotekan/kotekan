    ...
    // Update metadata
    {
        ...
        if (E_meta->get_time_downsampling_fpga() != 1)
            FATAL_ERROR(
                ...
                "generated for kernel BasebandBeamformer_pathfinder assume an input sampled at the "
                "FPGA rate",
        ...
        if (A_buffer.get_metadata()->get_fpga_seq_num() != E_meta->get_fpga_seq_num())
            FATAL_ERROR(
                ...
                "voltage buffer E begins at {:d}; kernel BasebandBeamformer_pathfinder requires "
                "them to be aligned",

/**
 * @file
 * @brief CUDA BasebandBeamformer_smallfinder kernel
 *
...
#include "kotekanLogging.hpp" // for ERROR, WARN
...
/**
 * @class cudaBasebandBeamformer_smallfinder
 * @brief cudaCommand for BasebandBeamformer_smallfinder
 */
class cudaBasebandBeamformer_smallfinder : public cudaCommand {
public:
    cudaBasebandBeamformer_smallfinder(Config& config, const std::string& unique_name,
                                       bufferContainer& host_buffers, cudaDeviceInterface& device,
                                       const int instance_num);
    virtual ~cudaBasebandBeamformer_smallfinder();
...
private:
    ...
    static constexpr int blocks = 48;
    ...
    static constexpr kotekan::DataType J_type = kotekan::int4x2_swapped_withoffset;
    enum J_indices {
        J_index_T,
        J_index_P,
        J_index_F,
...
REGISTER_CUDA_COMMAND(cudaBasebandBeamformer_smallfinder);

cudaBasebandBeamformer_smallfinder::cudaBasebandBeamformer_smallfinder(
    ...
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "BasebandBeamformer_smallfinder", "BasebandBeamformer_smallfinder.ptx"),
    ...
    dummy() // avoid trailing comma
{
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx("lib/cuda/generated/BasebandBeamformer_smallfinder.ptx", {kernel_symbol},
                         opts, "BasebandBeamformer_smallfinder_");
    }
}

cudaBasebandBeamformer_smallfinder::~cudaBasebandBeamformer_smallfinder() {}

int cudaBasebandBeamformer_smallfinder::wait_on_precondition() {
    {
...
cudaEvent_t
cudaBasebandBeamformer_smallfinder::execute(cudaPipelineState& /*pipestate*/,
    ...
    const std::string symname = "BasebandBeamformer_smallfinder_" + std::string(kernel_symbol);
    ...
    for (int block = 0; block < info_lengths[info_index_block]; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel BasebandBeamformer_smallfinder returned 'info' value {:d} "
    ...
    if (log_code != 0)
        WARN("CUDA kernel BasebandBeamformer_smallfinder returned log code cuLaunchKernel: {}",
             log_code);
    ...
    for (std::size_t i = 0; i < host_log_buffer.size(); ++i) {
        ...
        if (val != 0)
            WARN("CUDA kernel BasebandBeamformer_smallfinder returned 'log' value {:d} at index "
                 "{:d} (zero "
                 "indicates success)",
                 val, i);
    }
...
void cudaBasebandBeamformer_smallfinder::finalize_frame() {

...
cudaEvent_t cudaFRBBeamformer_hirax_U16::execute(cudaPipelineState& /*pipestate*/,
    ...
    if (instance_num == 0 && !did_set_metadata) {
        ...
        if (!(Telescope::instance().get_grid_size_x() <= std::uint64_t(cuda_dish_layout_M)
            ...
            FATAL_ERROR("Telescope dish grid {:d}x{:d} does not fit the dish layout {:d}x{:d} "
                        "for which kernel FRBBeamformer_hirax_U16 was compiled",
        ...
        if (Fbar_out_max - Fbar_out_min != Fbar_in_max - Fbar_in_min)
            FATAL_ERROR("Input and output frequency spans have different lengths for kernel "
                        "FRBBeamformer_hirax_U16: input [{:d},{:d}), output [{:d},{:d})",
        ...
        if (!(I_nfreq >= 0 && I_nfreq <= I_all_nfreq))
            FATAL_ERROR(
                "Kernel FRBBeamformer_hirax_U16 would write {:d} frequencies, but its output "
                "buffer I holds {:d}",
                I_nfreq, I_all_nfreq);
        ...
        if (cuda_upchannelization_factor > 1 && I_nfreq != Ebar_nfreq)
            FATAL_ERROR(
                "Kernel FRBBeamformer_hirax_U16 writes {:d} of the {:d} upchannelized input "
        ...
        if (!I_has_metadata)
            ...
            FATAL_ERROR("Another producer of buffer I set time_downsampling_fpga={:d}, but kernel "
                        "FRBBeamformer_hirax_U16 produces {:d}",
        ...
        if (W_nfreq != I_nfreq)
            FATAL_ERROR(
                "Weight buffer W holds {:d} frequencies, but kernel FRBBeamformer_hirax_U16 "
                "processes {:d}",
                W_nfreq, I_nfreq);
        ...
        for (int freq = 0; freq < W_nfreq; ++freq)
            if (I_coarse_freq.at(Fbar_out_min + freq) != W_coarse_freq.at(freq))
                FATAL_ERROR("Weight buffer W is for coarse frequency {:d} at index {:d}, but "
                            "kernel FRBBeamformer_hirax_U16 processes coarse frequency {:d} there",
    ...
    if (!I_buffer.has_metadata())
        FATAL_ERROR("Output buffer I has no metadata; kernel FRBBeamformer_hirax_U16 cannot run");
    ...
    if (!(0 <= Fbar_in_min && Fbar_in_min <= Fbar_in_max))
        FATAL_ERROR("Invalid input frequency span for kernel FRBBeamformer_hirax_U16: "
    ...
    if (!(0 <= Fbar_out_min && Fbar_out_min <= Fbar_out_max))
        FATAL_ERROR("Invalid output frequency span for kernel FRBBeamformer_hirax_U16: "
                    "Fbar_out_min={:d}, Fbar_out_max={:d} "
                    "(require 0 <= Fbar_out_min <= Fbar_out_max)",
                    Fbar_out_min, Fbar_out_max);
    if (Fbar_out_max - Fbar_out_min != Fbar_in_max - Fbar_in_min)
        FATAL_ERROR("Input and output frequency spans have different lengths for kernel "
                    "FRBBeamformer_hirax_U16: input [{:d},{:d}) holds {:d} frequencies, "
                    "output [{:d},{:d}) holds {:d}",
    ...
    if (!(0 <= blocks && blocks <= max_blocks))
        FATAL_ERROR(
            "Kernel FRBBeamformer_hirax_U16 would launch {:d} blocks, but the `info` buffer "
    ...
    if (!did_init_host_S_buffer) {
        ...
        if (array_dish_count + surplus_dishes.size()
            ...
            FATAL_ERROR(
                "Dish bookkeeping is inconsistent for kernel FRBBeamformer_hirax_U16: {:d} array "
        ...
        if (surplus_idx != surplus_dishes.size())
            FATAL_ERROR("Kernel FRBBeamformer_hirax_U16 placed {:d} of {:d} surplus dishes on the "
                        "dish grid",
        ...
        for (std::size_t s = 0; s < host_S_buffer.size(); ++s)
            if (host_S_buffer.at(s) < 0)
                FATAL_ERROR(
                    "Kernel FRBBeamformer_hirax_U16 left entry {:d} of the dish location map `S` "
                    "unassigned (value {:d})",
    ...
    if (poison_buffers) {
        ...
        if (error_code != 0)
            ERROR("CUDA kernel FRBBeamformer_hirax_U16 returned error code: {}", error_code);
            ...
            for (int block = 0; block < blocks; ++block) {
                for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                    for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                        ...
                        if (val != 0)
                            ERROR("CUDA kernel FRBBeamformer_hirax_U16 returned 'info' value {:d} "

/**
 * @file
 * @brief CUDA FRBBeamformer_smallfinder_U1 kernel
 *
...
#include <string>                  // for basic_string, operator==, string, operator+, operator<
...
/**
 * @class cudaFRBBeamformer_smallfinder_U1
 * @brief cudaCommand for FRBBeamformer_smallfinder_U1
 */
class cudaFRBBeamformer_smallfinder_U1 : public cudaCommand {
public:
    cudaFRBBeamformer_smallfinder_U1(Config& config, const std::string& unique_name,
                                     bufferContainer& host_buffers, cudaDeviceInterface& device,
                                     const int instance_num);
    virtual ~cudaFRBBeamformer_smallfinder_U1();
...
REGISTER_CUDA_COMMAND(cudaFRBBeamformer_smallfinder_U1);

cudaFRBBeamformer_smallfinder_U1::cudaFRBBeamformer_smallfinder_U1(Config& config,
                                                                   const std::string& unique_name,
                                                                   bufferContainer& host_buffers,
                                                                   cudaDeviceInterface& device,
                                                                   const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "FRBBeamformer_smallfinder_U1", "FRBBeamformer_smallfinder_U1.ptx"),
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx("lib/cuda/generated/FRBBeamformer_smallfinder_U1.ptx", {kernel_symbol},
                         opts, "FRBBeamformer_smallfinder_U1_");
    }
}

cudaFRBBeamformer_smallfinder_U1::~cudaFRBBeamformer_smallfinder_U1() {}

std::int64_t
cudaFRBBeamformer_smallfinder_U1::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_produced_elements(num_available_elements) * cuda_downsampling_factor;
}
std::int64_t
cudaFRBBeamformer_smallfinder_U1::num_produced_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) / cuda_downsampling_factor;
}

std::int64_t cudaFRBBeamformer_smallfinder_U1::num_processed_elements(
    std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaFRBBeamformer_smallfinder_U1::wait_on_precondition() {
    {
...
cudaEvent_t
cudaFRBBeamformer_smallfinder_U1::execute(cudaPipelineState& /*pipestate*/,
    ...
    const std::string symname = "FRBBeamformer_smallfinder_U1_" + std::string(kernel_symbol);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel FRBBeamformer_smallfinder_U1 returned error code: {}", error_code);
        ...
        for (int block = 0; block < blocks; ++block) {
            for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                    ...
                    if (val != 0)
                        ERROR("CUDA kernel FRBBeamformer_smallfinder_U1 returned 'info' value {:d} "
...
void cudaFRBBeamformer_smallfinder_U1::finalize_frame() {

/**
 * @file
 * @brief CUDA BasebandBeamformer_charts kernel
 *
...
/**
 * @class cudaBasebandBeamformer_charts
 * @brief cudaCommand for BasebandBeamformer_charts
 */
class cudaBasebandBeamformer_charts : public cudaCommand {
public:
    cudaBasebandBeamformer_charts(Config& config, const std::string& unique_name,
                                  bufferContainer& host_buffers, cudaDeviceInterface& device,
                                  const int instance_num);
    virtual ~cudaBasebandBeamformer_charts();
...
private:
    ...
    static constexpr int blocks = 336;
...
REGISTER_CUDA_COMMAND(cudaBasebandBeamformer_charts);

cudaBasebandBeamformer_charts::cudaBasebandBeamformer_charts(Config& config,
                                                             const std::string& unique_name,
                                                             bufferContainer& host_buffers,
                                                             cudaDeviceInterface& device,
                                                             const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "BasebandBeamformer_charts", "BasebandBeamformer_charts.ptx"),
...
int cudaBasebandBeamformer_charts::wait_on_precondition() {
    {
...
cudaEvent_t cudaBasebandBeamformer_charts::execute(cudaPipelineState& /*pipestate*/,
    ...
    // Update metadata
    {
        ...
        if (E_meta->get_time_downsampling_fpga() != 1)
            FATAL_ERROR(
                ...
                "generated for kernel BasebandBeamformer_charts assume an input sampled at the "
                "FPGA rate",
        ...
        if (A_buffer.get_metadata()->get_fpga_seq_num() != E_meta->get_fpga_seq_num())
            FATAL_ERROR(
                "Buffer A begins at FPGA sequence number {:d}, but the "
                "voltage buffer E begins at {:d}; kernel BasebandBeamformer_charts requires "
                "them to be aligned",
    ...
    const std::string symname = "BasebandBeamformer_charts_" + std::string(kernel_symbol);
    ...
    if (poison_buffers) {
        ...
        for (int block = 0; block < info_lengths[info_index_block]; ++block) {
            for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                    ...
                    if (val != 0)
                        ERROR("CUDA kernel BasebandBeamformer_charts returned 'info' value {:d} "
        ...
        if (log_code != 0)
            WARN("CUDA kernel BasebandBeamformer_charts returned log code cuLaunchKernel: {}",
                 log_code);
        ...
        for (std::size_t i = 0; i < host_log_buffer.size(); ++i) {
            ...
            if (val != 0)
                WARN("CUDA kernel BasebandBeamformer_charts returned 'log' value {:d} at index "
                     "{:d} (zero "
                     "indicates success)",
                     val, i);
        }

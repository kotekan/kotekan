/**
 * @file
 * @brief CUDA Upchannelizer_smallfinder_U64 kernel
 *
...
/**
 * @class cudaUpchannelizer_smallfinder_U64
 * @brief cudaCommand for Upchannelizer_smallfinder_U64
 */
class cudaUpchannelizer_smallfinder_U64 : public cudaCommand {
public:
    cudaUpchannelizer_smallfinder_U64(Config& config, const std::string& unique_name,
                                      bufferContainer& host_buffers, cudaDeviceInterface& device,
                                      const int instance_num);
    virtual ~cudaUpchannelizer_smallfinder_U64();
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_smallfinder_U64);

cudaUpchannelizer_smallfinder_U64::cudaUpchannelizer_smallfinder_U64(Config& config,
                                                                     const std::string& unique_name,
                                                                     bufferContainer& host_buffers,
                                                                     cudaDeviceInterface& device,
                                                                     const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_smallfinder_U64", "Upchannelizer_smallfinder_U64.ptx"),
    ...
    dummy() // avoid trailing comma
{
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx("lib/cuda/generated/Upchannelizer_smallfinder_U64.ptx", {kernel_symbol},
                         opts, "Upchannelizer_smallfinder_U64_");
    }
}

cudaUpchannelizer_smallfinder_U64::~cudaUpchannelizer_smallfinder_U64() {}

std::int64_t cudaUpchannelizer_smallfinder_U64::num_consumed_elements(
...
std::int64_t cudaUpchannelizer_smallfinder_U64::num_produced_elements(
    std::int64_t num_available_elements) const {
...
std::int64_t cudaUpchannelizer_smallfinder_U64::num_processed_elements(
    std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_smallfinder_U64::wait_on_precondition() {
    {
...
cudaEvent_t
cudaUpchannelizer_smallfinder_U64::execute(cudaPipelineState& /*pipestate*/,
    ...
    const std::string symname = "Upchannelizer_smallfinder_U64_" + std::string(kernel_symbol);
    ...
    if (poison_buffers) {
        ...
        if (error_code != 0)
            ERROR("CUDA kernel Upchannelizer_smallfinder_U64 returned error code: {}", error_code);
            ...
            for (int block = 0; block < blocks; ++block) {
                for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                    for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                        ...
                        if (val != 0)
                            ERROR("CUDA kernel Upchannelizer_smallfinder_U64 returned 'info' value "
                                  "{:d} "
...
void cudaUpchannelizer_smallfinder_U64::finalize_frame() {

...
@inbounds begin #= /home/eschnett/src/kotekan/julia/kernels/bb.jl:1015 =#
    ...
    s_polr0 = s_memory[(((IndexSpaces.assume_inrange(IndexSpaces.cuda_threadidx()::Int32, 0, 32) ÷ 8) % 4 + (IndexSpaces.assume_inrange(IndexSpaces.cuda_warpidx()::Int32, 0, 4) % 4) * 4) % 16 + ((IndexSpaces.assume_inrange(IndexSpaces.cuda_blockidx()::Int32, 0, 384) % 384) % 384) * 32 + ((0::Int32 % 2) % 2) * 16) + 0x01]
    s_polr1 = s_memory[(((IndexSpaces.assume_inrange(IndexSpaces.cuda_threadidx()::Int32, 0, 32) ÷ 8) % 4 + (IndexSpaces.assume_inrange(IndexSpaces.cuda_warpidx()::Int32, 0, 4) % 4) * 4) % 16 + ((IndexSpaces.assume_inrange(IndexSpaces.cuda_blockidx()::Int32, 0, 384) % 384) % 384) * 32 + ((1::Int32 % 2) % 2) * 16) + 0x01]
    s_polr0 = s_polr0 - 2
    s_polr1 = s_polr1 - 2

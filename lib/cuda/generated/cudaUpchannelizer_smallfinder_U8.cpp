/**
 * @file
 * @brief CUDA Upchannelizer_smallfinder_U8 kernel
 *
...
/**
 * @class cudaUpchannelizer_smallfinder_U8
 * @brief cudaCommand for Upchannelizer_smallfinder_U8
 */
class cudaUpchannelizer_smallfinder_U8 : public cudaCommand {
public:
    cudaUpchannelizer_smallfinder_U8(Config& config, const std::string& unique_name,
                                     bufferContainer& host_buffers, cudaDeviceInterface& device,
                                     const int instance_num);
    virtual ~cudaUpchannelizer_smallfinder_U8();
...
private:
    ...
    // Kernel arguments:
    enum class args { T_min, T_max, Tbar_min, Tbar_max, Fmin, Fmax, G_U8, E, Ebar, info, count };
    ...
    static constexpr std::ptrdiff_t G_U8_length = G_U8_strides[G_U8_rank];
    static constexpr std::ptrdiff_t G_U8_length_in_bytes =
        type_total_bytes(G_U8_type) * G_U8_length;
    static_assert(G_U8_length_in_bytes <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    //
    // E: voltage_name
    ...
    // Buffers
    NDArrayBuffer<kotekan::GetType_t<G_U8_type>, G_U8_rank> G_U8_buffer;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_smallfinder_U8);

cudaUpchannelizer_smallfinder_U8::cudaUpchannelizer_smallfinder_U8(Config& config,
                                                                   const std::string& unique_name,
                                                                   bufferContainer& host_buffers,
                                                                   cudaDeviceInterface& device,
                                                                   const int instance_num) :
    cudaCommand(config, unique_name, host_buffers, device, instance_num, no_cuda_command_state,
                "Upchannelizer_smallfinder_U8", "Upchannelizer_smallfinder_U8.ptx"),
    ...
    G_U8_buffer(G_U8_name, G_U8_quantity, reverse(G_U8_lengths), reverse(G_U8_labels), *this,
    ...
    dummy() // avoid trailing comma
{
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx("lib/cuda/generated/Upchannelizer_smallfinder_U8.ptx", {kernel_symbol},
                         opts, "Upchannelizer_smallfinder_U8_");
    }
}

cudaUpchannelizer_smallfinder_U8::~cudaUpchannelizer_smallfinder_U8() {}

std::int64_t
cudaUpchannelizer_smallfinder_U8::num_consumed_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_smallfinder_U8::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t cudaUpchannelizer_smallfinder_U8::num_processed_elements(
    std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_smallfinder_U8::wait_on_precondition() {
    {
...
cudaEvent_t
cudaUpchannelizer_smallfinder_U8::execute(cudaPipelineState& /*pipestate*/,
                                          const std::vector<cudaEvent_t>& /*pre_events*/) {
    pre_execute();
    record_start_event();

    void* const G_U8_memory = G_U8_buffer.get_ndarray().data();
    ...
    array_desc G_U8_arg(G_U8_memory, G_U8_length_in_bytes);
    ...
    const std::string symname = "Upchannelizer_smallfinder_U8_" + std::string(kernel_symbol);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_smallfinder_U8 returned error code: {}", error_code);
        ...
        for (int block = 0; block < blocks; ++block) {
            for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
                for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                    ...
                    if (val != 0)
                        ERROR("CUDA kernel Upchannelizer_smallfinder_U8 returned 'info' value {:d} "
...
void cudaUpchannelizer_smallfinder_U8::finalize_frame() {

...
private:
    ...
    static constexpr std::ptrdiff_t G_U8_length = type_total_bytes(G_U8_type) * 64;
    ...
    static constexpr std::ptrdiff_t Ebar_length = type_total_bytes(Ebar_type) * 512 * 2 * 64 * 4096;
...
cudaEvent_t cudaUpchannelizer_chord_U8::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_chord_U8 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_chord_U8 returned 'info' value {:d} "

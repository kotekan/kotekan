...
#include <driver_types.h> // for CUevent_st, CUstream_st, cudaError, cudaEvent_t, cudaMemcp...
...
cudaEvent_t cudaCopyFromRingbuffer::execute(cudaPipelineState& pipestate,
    ...
    if (out_buffer) {
        ...
        out_buffer->allocate_new_frame_desc(out_id, meta->type, meta->get_name(), extents,
                                            dimnames);

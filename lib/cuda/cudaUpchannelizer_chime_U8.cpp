...
private:
    ...
    static constexpr std::ptrdiff_t G_U8_length = chord_datatype_bytes(G_U8_type) * 8;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 1024 * 2 * 8 * 8192;
...
cudaEvent_t cudaUpchannelizer_chime_U8::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaUpchannelizer_chime_U8 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif

...
private:
    ...
    static constexpr std::ptrdiff_t G_U2_length = chord_datatype_bytes(G_U2_type) * 32;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 512 * 2 * 32 * 16384;
...
cudaEvent_t cudaUpchannelizer_chord_U2::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaUpchannelizer_chord_U2 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif

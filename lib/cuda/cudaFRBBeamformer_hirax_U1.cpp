...
private:
    ...
    static constexpr std::size_t W_U1_length =
        chord_datatype_bytes(W_U1_type) * 2 * 16 * 16 * 2 * 64;
    ...
    static constexpr std::size_t Ebar_U1_length =
        chord_datatype_bytes(Ebar_U1_type) * 256 * 2 * 64 * 32768;
    ...
    static constexpr std::size_t I_U1_length =
        chord_datatype_bytes(I_U1_type) * 32 * 32 * 64 * 4096;
    ...
    static constexpr std::array<std::ptrdiff_t, I_U1_rank + 1> I_U1_strides = {
        ...
        I_U1_calc_stride(I_U1_index_Fbar_U1), I_U1_calc_stride(I_U1_index_Ttilde_U1_Tds25),
    ...
    static constexpr std::size_t I_U1_Ttilde_U1_Tds25_sample_bytes =
...
cudaFRBBeamformer_hirax_U1::cudaFRBBeamformer_hirax_U1(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "FRBBeamformer_hirax_U1_");
    }
...
int cudaFRBBeamformer_hirax_U1::wait_on_precondition() {
    ...
    // to bytes
    const std::size_t output_bytes = Ttildelength * I_U1_Ttilde_U1_Tds25_sample_bytes;
    ...
    assert(mod(output_cursor, I_U1_Ttilde_U1_Tds25_sample_bytes) == 0);
    Ttildemin = output_cursor / I_U1_Ttilde_U1_Tds25_sample_bytes;
...
cudaEvent_t cudaFRBBeamformer_hirax_U1::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_U1_meta = *Ebar_U1_meta;
    ...
    const std::size_t Ttilde_ringbuf =
        output_ringbuf_signal->size / I_U1_Ttilde_U1_Tds25_sample_bytes;
    ...
    // Update metadata
    I_U1_meta->dim[I_U1_rank - 1 - I_U1_index_Ttilde_U1_Tds25] = Ttildelength;
    assert(I_U1_meta->dim[I_U1_rank - 1 - I_U1_index_Ttilde_U1_Tds25]
           <= int(I_U1_lengths[I_U1_index_Ttilde_U1_Tds25]));
    ...
    DEBUG("Running CUDA FRBBeamformer_hirax_U1 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaFRBBeamformer_hirax_U1 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
...
void cudaFRBBeamformer_hirax_U1::finalize_frame() {
    ...
    DEBUG("    Produced bytes:   {:d}", Ttilde_produced * I_U1_Ttilde_U1_Tds25_sample_bytes);
    output_ringbuf_signal->finish_write(unique_name, instance_num,
                                        Ttilde_produced * I_U1_Ttilde_U1_Tds25_sample_bytes);

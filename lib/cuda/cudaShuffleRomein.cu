...
REGISTER_CUDA_COMMAND(cudaShuffleRomein);

cudaShuffleRomein::cudaShuffleRomein(Config& config, const std::string& unique_name,
...
cudaShuffleRomein::~cudaShuffleRomein() {}

__global__ void shuffle_romein(int *input, int *output, int ne, int nt, int nf) {
...
cudaEvent_t cudaShuffleRomein::execute(int gpu_frame_id, cudaEvent_t pre_event) {
    ...
    void *output_memory = device.get_gpu_memory("in_romein", input_frame_len);
    ...
    shuffle_romein<<<grd,blk,0,device.getStream(CUDA_COMPUTE_STREAM)>>>

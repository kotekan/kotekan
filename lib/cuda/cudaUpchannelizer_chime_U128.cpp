...
private:
    ...
    static constexpr std::ptrdiff_t G_U128_length = type_total_bytes(G_U128_type) * 2048;
    ...
    static_assert(G_U128_length == type_total_bytes(G_U128_type) * G_U128_strides[G_U128_rank]);
    //
    // E: voltage_name
...
cudaUpchannelizer_chime_U128::cudaUpchannelizer_chime_U128(Config& config,
    ...
    output_ringbuf_signal(dynamic_cast<RingBuffer*>(host_buffers.get_generic_buffer(
        config.get<std::string>(unique_name, "upchan_U128_voltage_signal_out")))),
    ...
    gpu_buffers_used.push_back(std::make_tuple(G_U128_name, true, true, false));
...
cudaEvent_t cudaUpchannelizer_chime_U128::execute(cudaPipelineState& /*pipestate*/,
    ...
    const std::string G_U128_memname = G_U128_name + "_buffer";
    ...
    assert(std::strncmp(G_U128_meta->name, G_U128_quantity, sizeof G_U128_meta->name) == 0);
    ...
    *Ebar_meta = *E_meta;
    ...
    DEBUG("Running CUDA Upchannelizer_chime_U128 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA Upchannelizer_chime_U128 on GPU frame {:d}", gpu_frame_id);

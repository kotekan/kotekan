...
private:
    ...
    static constexpr std::ptrdiff_t W_length = type_total_bytes(W_type) * 2 * 24 * 24 * 2 * 16;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        type_total_bytes(Ebar_type) * 512 * 2 * 48 * 32768;
    ...
    static constexpr std::ptrdiff_t info_length = type_total_bytes(info_type) * 32 * 24 * 48;
...
cudaEvent_t cudaFRBBeamformer_chord_U1::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA kernel FRBBeamformer_chord_U1 on GPU frame {:d}", gpu_frame_id);
    ...
    // Check outputs for poison
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            if (I_found_error)
                WARN("CUDA kernel FRBBeamformer_chord_U1 returned produced non-finite results");
        } // for chunk

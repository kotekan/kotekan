/**
 * @file
 * @brief CUDA TransposeKernel_hirax kernel
 *
...
/**
 * @class cudaTransposeKernel_hirax
 * @brief cudaCommand for TransposeKernel_hirax
 */
class cudaTransposeKernel_hirax : public cudaCommand {
public:
    cudaTransposeKernel_hirax(Config& config, const std::string& unique_name,
                              bufferContainer& host_buffers, cudaDeviceInterface& device,
                              const int inst);
    virtual ~cudaTransposeKernel_hirax();
...
private:
    ...
    static constexpr std::size_t Ein_length =
        chord_datatype_bytes(Ein_type) * 8 * 16 * 32 * 2 * 16 * 2048;
    ...
    static constexpr std::size_t E_length = chord_datatype_bytes(E_type) * 256 * 2 * 16 * 32768;
...
REGISTER_CUDA_COMMAND(cudaTransposeKernel_hirax);

cudaTransposeKernel_hirax::cudaTransposeKernel_hirax(Config& config, const std::string& unique_name,
    ...
    cudaCommand(config, unique_name, host_buffers, device, inst, no_cuda_command_state,
                "TransposeKernel_hirax", "TransposeKernel_hirax.ptx"),
...
cudaTransposeKernel_hirax::~cudaTransposeKernel_hirax() {}

cudaEvent_t cudaTransposeKernel_hirax::execute(cudaPipelineState& /*pipestate*/,
    ...
    DEBUG("Running CUDA TransposeKernel_hirax on GPU frame {:d}", gpu_frame_id);
    const CUresult err =
    ...
    for (std::size_t i = 0; i < info_host.at(gpu_frame_index).size(); ++i)
        if (info_host.at(gpu_frame_index)[i] != 0)
            ERROR("cudaTransposeKernel_hirax returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
...
void cudaTransposeKernel_hirax::finalize_frame() {

...
private:
    ...
    static constexpr int cuda_number_of_frequencies = 4096;
    ...
    static constexpr std::size_t W_U64_length =
        chord_datatype_bytes(W_U64_type) * 2 * 16 * 16 * 2 * 4096;
    ...
    static constexpr std::size_t Ebar_U64_length =
        chord_datatype_bytes(Ebar_U64_type) * 256 * 2 * 4096 * 512;
    ...
    static constexpr std::size_t I_U64_length =
        chord_datatype_bytes(I_U64_type) * 32 * 32 * 4096 * 64;
    ...
    static constexpr std::array<std::ptrdiff_t, I_U64_rank + 1> I_U64_strides = {
        ...
        I_U64_calc_stride(I_U64_index_Fbar_U64), I_U64_calc_stride(I_U64_index_Ttilde_U64_Tds25),
    ...
    static constexpr std::size_t info_length = chord_datatype_bytes(info_type) * 32 * 16 * 4096;
    ...
    static constexpr std::size_t I_U64_Ttilde_U64_Tds25_sample_bytes =
...
int cudaFRBBeamformer_hirax_U64::wait_on_precondition() {
    ...
    // to bytes
    const std::size_t output_bytes = Ttildelength * I_U64_Ttilde_U64_Tds25_sample_bytes;
    ...
    assert(mod(output_cursor, I_U64_Ttilde_U64_Tds25_sample_bytes) == 0);
    Ttildemin = output_cursor / I_U64_Ttilde_U64_Tds25_sample_bytes;
...
cudaEvent_t cudaFRBBeamformer_hirax_U64::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_U64_meta = *Ebar_U64_meta;
    ...
    const std::size_t Ttilde_ringbuf =
        output_ringbuf_signal->size / I_U64_Ttilde_U64_Tds25_sample_bytes;
    ...
    // Update metadata
    I_U64_meta->dim[I_U64_rank - 1 - I_U64_index_Ttilde_U64_Tds25] = Ttildelength;
    assert(I_U64_meta->dim[I_U64_rank - 1 - I_U64_index_Ttilde_U64_Tds25]
           <= int(I_U64_lengths[I_U64_index_Ttilde_U64_Tds25]));
    ...
    DEBUG("Running CUDA FRBBeamformer_hirax_U64 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaFRBBeamformer_hirax_U64 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
...
void cudaFRBBeamformer_hirax_U64::finalize_frame() {
    ...
    DEBUG("    Produced bytes:   {:d}", Ttilde_produced * I_U64_Ttilde_U64_Tds25_sample_bytes);
    output_ringbuf_signal->finish_write(unique_name, instance_num,
                                        Ttilde_produced * I_U64_Ttilde_U64_Tds25_sample_bytes);

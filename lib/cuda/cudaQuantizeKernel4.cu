...
void cpu_quantize4_chunks(const __half* __restrict__ const input,
    ...
    for (int chunk = 0; chunk < chunks_per_frame; ++chunk) {
        ...
        // The factor 2.3 is "tribal knowledge". ChatGPT suggests a factor of 2.83 instead.
        const float offset = mean;
        const float scale = fmax(1.0f, stddev / 2.3f);

        // Store offset and scale for this chunk in the output array
        const __half offseth = __float2half(offset);
        const __half scaleh = __float2half(scale);
        outputf[outputf_offset + 0] = offseth;
        outputf[outputf_offset + 1] = scaleh;
        ...
        // value.
        const __half out_offseth = __float2half(8.0f);
        const __half minh = __float2half(1.0f);
        const __half maxh = __float2half(15.0f);
        for (int i = 0; i < chunk_size; i += 2) {
            const __half x0 = input[i + input_offset + 0];
            const __half x1 = input[i + input_offset + 1];
            const __half y0 = (x0 - offseth) / scaleh;

...
cudaEvent_t cudaRFIS012bar::execute(cudaPipelineState& /*pipestate*/,
    ...
    auto time_downsampling_fpga_per_frequency =
        rfi_S012bar_meta->get_time_downsampling_fpga_per_frequency();
    ...
    assert(time_downsampling_fpga_per_frequency.size()
           == static_cast<size_t>(rfi_S012bar_meta->get_nfreq()));
    ...
    for (int freq = 0; freq < rfi_S012bar_meta->get_nfreq(); ++freq) {
        ...
        time_downsampling_fpga_per_frequency[freq] *= rfi_second_downsampling_factor;
    }
    rfi_S012bar_meta->set_freq_upchan_factor(freq_upchan_factor);
    rfi_S012bar_meta->set_time_downsampling_fpga_per_frequency(

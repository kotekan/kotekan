...
cudaCopyToRingbuffer::cudaCopyToRingbuffer(Config& config, const std::string& unique_name,
    ...
    if (_gpu_mem_input.size() == 0) {
        ...
        if (!in_buffer)
            throw std::runtime_error("In cudaCopyToRingbuffer " + unique_name + ", must set either gpu_mem_input or in_buf");
        if (instance_num == 0)
        ...
        if (in_buffer->frame_size) {
            ...
            if (cudaErrorInvalidValue == cudaHostGetFlags(&flags, in_buffer->frames[instance_num])) {
                CHECK_CUDA_ERROR(cudaHostRegister(in_buffer->frames[instance_num], in_buffer->frame_size, 0));
            }
        }
    } else {
        in_buffer = nullptr;
...
int cudaCopyToRingbuffer::wait_on_precondition() {
    ...
    if (in_buffer) {
        ...
        uint8_t* frame = in_buffer->wait_for_full_frame(unique_name, gpu_frame_id % in_buffer->num_frames);
        if (frame == nullptr)
            return -1;
    }
    return 0;

/**
 * @file
 * @brief CUDA Upchannelizer_hirax_U64 kernel
 *
...
/**
 * @class cudaUpchannelizer_hirax_U64
 * @brief cudaCommand for Upchannelizer_hirax_U64
 */
class cudaUpchannelizer_hirax_U64 : public cudaCommand {
public:
    cudaUpchannelizer_hirax_U64(Config& config, const std::string& unique_name,
                                bufferContainer& host_buffers, cudaDeviceInterface& device,
                                const int instance_num);
    virtual ~cudaUpchannelizer_hirax_U64();
...
private:
    ...
    static constexpr std::ptrdiff_t G_U64_length = chord_datatype_bytes(G_U64_type) * 4096;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_hirax_U64);

cudaUpchannelizer_hirax_U64::cudaUpchannelizer_hirax_U64(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_hirax_U64_");
    }
...
std::int64_t
cudaUpchannelizer_hirax_U64::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) - cuda_algorithm_overlap;
}
std::int64_t
cudaUpchannelizer_hirax_U64::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_hirax_U64::num_processed_elements(std::int64_t num_available_elements) const {
    return round_down(num_available_elements, cuda_granularity_number_of_timesamples);
}

int cudaUpchannelizer_hirax_U64::wait_on_precondition() {
...
cudaEvent_t cudaUpchannelizer_hirax_U64::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_U64_meta = *E_meta;
    ...
    const std::string symname = "Upchannelizer_hirax_U64_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA Upchannelizer_hirax_U64 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaUpchannelizer_hirax_U64 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
...
void cudaUpchannelizer_hirax_U64::finalize_frame() {
    const std::ptrdiff_t Tlength = Tmax - Tmin;
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;

/**
 * @file
 * @brief CUDA Upchannelizer_chime_U64 kernel
 *
...
/**
 * @class cudaUpchannelizer_chime_U64
 * @brief cudaCommand for Upchannelizer_chime_U64
 */
class cudaUpchannelizer_chime_U64 : public cudaCommand {
public:
    cudaUpchannelizer_chime_U64(Config& config, const std::string& unique_name,
                                bufferContainer& host_buffers, cudaDeviceInterface& device,
                                const int instance_num);
    virtual ~cudaUpchannelizer_chime_U64();
...
private:
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 1024 * 2 * 64 * 1024;
...
REGISTER_CUDA_COMMAND(cudaUpchannelizer_chime_U64);

cudaUpchannelizer_chime_U64::cudaUpchannelizer_chime_U64(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "Upchannelizer_chime_U64_");
    }
...
std::int64_t
cudaUpchannelizer_chime_U64::num_consumed_elements(std::int64_t num_available_elements) const {
    return num_processed_elements(num_available_elements) - cuda_algorithm_overlap;
}
std::int64_t
cudaUpchannelizer_chime_U64::num_produced_elements(std::int64_t num_available_elements) const {
...
std::int64_t
cudaUpchannelizer_chime_U64::num_processed_elements(std::int64_t num_available_elements) const {
...
cudaEvent_t cudaUpchannelizer_chime_U64::execute(cudaPipelineState& /*pipestate*/,
    ...
    *Ebar_meta = *E_meta;
    ...
    const std::string symname = "Upchannelizer_chime_U64_" + std::string(kernel_symbol);
    CHECK_CU_ERROR(cuFuncSetAttribute(device.runtime_kernels[symname],
                                      CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                                      shmem_bytes));

    DEBUG("Running CUDA Upchannelizer_chime_U64 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA Upchannelizer_chime_U64 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaUpchannelizer_chime_U64 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif
...
void cudaUpchannelizer_chime_U64::finalize_frame() {
    const std::ptrdiff_t Tlength = Tmax - Tmin;
    const std::ptrdiff_t Tbarlength = Tbarmax - Tbarmin;

...
private:
    ...
    static constexpr std::ptrdiff_t W_length = chord_datatype_bytes(W_type) * 2 * 24 * 24 * 2 * 64;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 24 * 64;
...
cudaEvent_t cudaFRBBeamformer_chord_U8::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA FRBBeamformer_chord_U8 on GPU frame {:d}", gpu_frame_id);

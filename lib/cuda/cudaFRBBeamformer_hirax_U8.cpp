...
cudaEvent_t cudaFRBBeamformer_hirax_U8::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA kernel FRBBeamformer_hirax_U8 on GPU frame {:d}", gpu_frame_id);
    ...
    // Check outputs for poison
    {
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            if (I_found_error)
                WARN("CUDA kernel FRBBeamformer_hirax_U8 returned produced non-finite results");
        } // for chunk

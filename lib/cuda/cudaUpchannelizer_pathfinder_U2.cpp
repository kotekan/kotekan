...
private:
    ...
    // Kernel arguments:
    enum class args { Tmin, Tmax, Tbarmin, Tbarmax, Fmin, Fmax, G_U2, E, Ebar_U2, info, count };
    ...
    //
    // G_U2: gpu_mem_gain
    static constexpr const char* G_U2_name = "G_U2";
    static constexpr chordDataType G_U2_type = float16;
    enum G_U2_indices {
        G_U2_index_Fbar_U2,
        G_U2_rank,
    };
    static constexpr std::array<const char*, G_U2_rank> G_U2_labels = {
        "Fbar_U2",
    };
    static constexpr std::array<std::ptrdiff_t, G_U2_rank> G_U2_lengths = {
        256,
    };
    static constexpr std::ptrdiff_t G_U2_length = chord_datatype_bytes(G_U2_type) * 256;
    static_assert(G_U2_length <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    static constexpr auto G_U2_calc_stride = [](int dim) {
        std::ptrdiff_t str = 1;
        for (int d = 0; d < dim; ++d)
            str *= G_U2_lengths[d];
        return str;
    };
    static constexpr std::array<std::ptrdiff_t, G_U2_rank + 1> G_U2_strides = {
        G_U2_calc_stride(G_U2_index_Fbar_U2),
        G_U2_calc_stride(G_U2_rank),
    };
    static_assert(G_U2_length == chord_datatype_bytes(G_U2_type) * G_U2_strides[G_U2_rank]);
    //
    ...
    //
    // Ebar_U2: gpu_mem_output_voltage
    ...
    static constexpr std::array<std::ptrdiff_t, Ebar_U2_rank> Ebar_U2_lengths = {
        64,
        2,
        256,
        16384,
    };
    static constexpr std::ptrdiff_t Ebar_U2_length =
        chord_datatype_bytes(Ebar_U2_type) * 64 * 2 * 256 * 16384;
    static_assert(Ebar_U2_length <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 2 * 384;
    ...
    static constexpr std::ptrdiff_t E_T_sample_bytes = chord_datatype_bytes(E_type)
                                                       ...
                                                       * E_lengths[E_index_F];
    static constexpr std::ptrdiff_t Ebar_U2_Tbar_U2_sample_bytes =
...
cudaUpchannelizer_pathfinder_U2::cudaUpchannelizer_pathfinder_U2(Config& config,
    ...
    G_U2_memname(config.get<std::string>(unique_name, "gpu_mem_gain")),
    E_memname(config.get<std::string>(unique_name, "gpu_mem_input_voltage")),
    Ebar_U2_memname(config.get<std::string>(unique_name, "gpu_mem_output_voltage")),
    ...
    assert(output_ringbuf_signal->size == Ebar_U2_length);
    ...
    gpu_buffers_used.push_back(std::make_tuple(G_U2_memname, true, true, false));
...
int cudaUpchannelizer_pathfinder_U2::wait_on_precondition() {
    ...
    // to bytes
    const std::ptrdiff_t output_bytes = Tbarlength * Ebar_U2_Tbar_U2_sample_bytes;
    ...
    assert(mod(output_cursor, Ebar_U2_Tbar_U2_sample_bytes) == 0);
    Tbarmin = output_cursor / Ebar_U2_Tbar_U2_sample_bytes;
...
cudaUpchannelizer_pathfinder_U2::execute(cudaPipelineState& /*pipestate*/,
    ...
    void* const G_U2_memory =
        args::G_U2 == args::E ? device.get_gpu_memory(G_U2_memname, input_ringbuf_signal->size)
        : args::G_U2 == args::Ebar_U2
            ? device.get_gpu_memory(G_U2_memname, output_ringbuf_signal->size)
        : args::G_U2 == args::G_U2 ? device.get_gpu_memory(G_U2_memname, G_U2_length)
                                   : device.get_gpu_memory_array(G_U2_memname, gpu_frame_id,
                                                                 _gpu_buffer_depth, G_U2_length);
    void* const E_memory =
        args::E == args::E         ? device.get_gpu_memory(E_memname, input_ringbuf_signal->size)
        : args::E == args::Ebar_U2 ? device.get_gpu_memory(E_memname, output_ringbuf_signal->size)
        : args::E == args::G_U2
    ...
    // G_U2 is an input buffer: check metadata
    const std::shared_ptr<metadataObject> G_U2_mc =
        args::G_U2 == args::E ? input_ringbuf_signal->get_metadata(0)
                              : device.get_gpu_memory_array_metadata(G_U2_memname, gpu_frame_id);
    assert(G_U2_mc);
    assert(metadata_is_chord(G_U2_mc));
    const std::shared_ptr<chordMetadata> G_U2_meta = get_chord_metadata(G_U2_mc);
    DEBUG("input G_U2 array: {:s} {:s}", G_U2_meta->get_type_string(),
          G_U2_meta->get_dimensions_string());
    assert(std::strncmp(G_U2_meta->name, G_U2_name, sizeof G_U2_meta->name) == 0);
    assert(G_U2_meta->type == G_U2_type);
    assert(G_U2_meta->dims == G_U2_rank);
    for (std::ptrdiff_t dim = 0; dim < G_U2_rank; ++dim) {
        assert(std::strncmp(G_U2_meta->dim_name[G_U2_rank - 1 - dim], G_U2_labels[dim],
                            sizeof G_U2_meta->dim_name[G_U2_rank - 1 - dim])
               == 0);
        if (args::G_U2 == args::E && dim == E_index_T) {
            assert(G_U2_meta->dim[G_U2_rank - 1 - dim] <= int(G_U2_lengths[dim]));
            assert(G_U2_meta->stride[G_U2_rank - 1 - dim] <= G_U2_strides[dim]);
        } else {
            assert(G_U2_meta->dim[G_U2_rank - 1 - dim] == int(G_U2_lengths[dim]));
            assert(G_U2_meta->stride[G_U2_rank - 1 - dim] == G_U2_strides[dim]);
        }
    ...
    //
    // Ebar_U2 is an output buffer: set metadata
    std::shared_ptr<metadataObject> const Ebar_U2_mc =
        args::Ebar_U2 == args::Ebar_U2 ? output_ringbuf_signal->get_metadata(0)
                                       : device.create_gpu_memory_array_metadata(
                                           Ebar_U2_memname, gpu_frame_id, E_mc->parent_pool);
    std::shared_ptr<chordMetadata> const Ebar_U2_meta = get_chord_metadata(Ebar_U2_mc);
    *Ebar_U2_meta = *E_meta;
    std::strncpy(Ebar_U2_meta->name, Ebar_U2_name, sizeof Ebar_U2_meta->name);
    Ebar_U2_meta->type = Ebar_U2_type;
    Ebar_U2_meta->dims = Ebar_U2_rank;
    for (std::ptrdiff_t dim = 0; dim < Ebar_U2_rank; ++dim) {
        std::strncpy(Ebar_U2_meta->dim_name[Ebar_U2_rank - 1 - dim], Ebar_U2_labels[dim],
                     sizeof Ebar_U2_meta->dim_name[Ebar_U2_rank - 1 - dim]);
        Ebar_U2_meta->dim[Ebar_U2_rank - 1 - dim] = Ebar_U2_lengths[dim];
        Ebar_U2_meta->stride[Ebar_U2_rank - 1 - dim] = Ebar_U2_strides[dim];
    }
    DEBUG("output Ebar_U2 array: {:s} {:s}", Ebar_U2_meta->get_type_string(),
    ...
    array_desc G_U2_arg(G_U2_memory, G_U2_length);
    ...
    void* args[] = {
        ...
        &Fmax_arg, &G_U2_arg, &E_arg,    &Ebar_U2_arg, &info_arg,
    };
    ...
    const std::ptrdiff_t Tbar_ringbuf = output_ringbuf_signal->size / Ebar_U2_Tbar_U2_sample_bytes;
    ...
    // Update metadata
    Ebar_U2_meta->dim[Ebar_U2_rank - 1 - Ebar_U2_index_Tbar_U2] = Tbarlength;
    assert(Ebar_U2_meta->dim[Ebar_U2_rank - 1 - Ebar_U2_index_Tbar_U2]
           <= int(Ebar_U2_lengths[Ebar_U2_index_Tbar_U2]));
    // Since we use a ring buffer we do not need to update `meta->sample0_offset`

    assert(Ebar_U2_meta->nfreq >= 0);
    assert(Ebar_U2_meta->nfreq == E_meta->nfreq);
    for (int freq = 0; freq < Ebar_U2_meta->nfreq; ++freq) {
        Ebar_U2_meta->freq_upchan_factor[freq] =
            cuda_upchannelization_factor * E_meta->freq_upchan_factor[freq];
        Ebar_U2_meta->half_fpga_sample0[freq] =
            E_meta->half_fpga_sample0[freq] + cuda_number_of_taps - 1;
        Ebar_U2_meta->time_downsampling_fpga[freq] =
...
void cudaUpchannelizer_pathfinder_U2::finalize_frame() {
    ...
    DEBUG("    Produced bytes:   {:d}", Tbar_produced * Ebar_U2_Tbar_U2_sample_bytes);
    output_ringbuf_signal->finish_write(unique_name, instance_num,
                                        Tbar_produced * Ebar_U2_Tbar_U2_sample_bytes);

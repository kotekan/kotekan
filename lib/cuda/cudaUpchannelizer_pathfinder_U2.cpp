...
private:
    ...
    // Kernel arguments:
    enum class args { Tmin, Tmax, Tbarmin, Tbarmax, G2, E, Ebar2, info, count };
    ...
    //
    // G2: gpu_mem_gain
    static constexpr const char* G2_name = "G2";
    static constexpr chordDataType G2_type = float16;
    enum G2_indices {
        G2_index_Fbar,
        G2_rank,
    };
    static constexpr std::array<const char*, G2_rank> G2_labels = {
        "Fbar",
    };
    static constexpr std::array<std::ptrdiff_t, G2_rank> G2_lengths = {
        768,
    };
    static constexpr std::ptrdiff_t G2_length = chord_datatype_bytes(G2_type) * 768;
    static_assert(G2_length <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    //
    ...
    static constexpr std::array<std::ptrdiff_t, Ebar2_rank> Ebar2_lengths = {
        64,
        2,
        768,
        16384,
    };
    static constexpr std::ptrdiff_t Ebar2_length =
        chord_datatype_bytes(Ebar2_type) * 64 * 2 * 768 * 16384;
    static_assert(Ebar2_length <= std::ptrdiff_t(std::numeric_limits<int>::max()) + 1);
    //
    // info: gpu_mem_info
    ...
    static constexpr std::ptrdiff_t E_T_sample_bytes = chord_datatype_bytes(E_type)
                                                       ...
                                                       * E_lengths[E_index_F];
    static constexpr std::ptrdiff_t Ebar2_Tbar_sample_bytes =
...
cudaUpchannelizer_pathfinder_U2::cudaUpchannelizer_pathfinder_U2(Config& config,
    ...
    G2_memname(config.get<std::string>(unique_name, "gpu_mem_gain")),
    E_memname(config.get<std::string>(unique_name, "gpu_mem_input_voltage")),
    Ebar2_memname(config.get<std::string>(unique_name, "gpu_mem_output_voltage")),
    ...
    assert(output_ringbuf_signal->size == Ebar2_length);
    ...
    gpu_buffers_used.push_back(std::make_tuple(G2_memname, true, true, false));
...
int cudaUpchannelizer_pathfinder_U2::wait_on_precondition() {
    ...
    // to bytes
    const std::ptrdiff_t output_bytes = Tbarlength * Ebar2_Tbar_sample_bytes;
    ...
    assert(mod(output_cursor, Ebar2_Tbar_sample_bytes) == 0);
    Tbarmin = output_cursor / Ebar2_Tbar_sample_bytes;
...
cudaUpchannelizer_pathfinder_U2::execute(cudaPipelineState& /*pipestate*/,
    ...
    void* const G2_memory =
        args::G2 == args::E       ? device.get_gpu_memory(G2_memname, input_ringbuf_signal->size)
        : args::G2 == args::Ebar2 ? device.get_gpu_memory(G2_memname, output_ringbuf_signal->size)
        : args::G2 == args::G2
            ? device.get_gpu_memory(G2_memname, G2_length)
            : device.get_gpu_memory_array(G2_memname, gpu_frame_id, _gpu_buffer_depth, G2_length);
    void* const E_memory =
        args::E == args::E       ? device.get_gpu_memory(E_memname, input_ringbuf_signal->size)
        : args::E == args::Ebar2 ? device.get_gpu_memory(E_memname, output_ringbuf_signal->size)
        : args::E == args::G2
    ...
    void* const Ebar2_memory =
        args::Ebar2 == args::E ? device.get_gpu_memory(Ebar2_memname, input_ringbuf_signal->size)
        : args::Ebar2 == args::Ebar2
            ? device.get_gpu_memory(Ebar2_memname, output_ringbuf_signal->size)
        : args::Ebar2 == args::G2 ? device.get_gpu_memory(Ebar2_memname, Ebar2_length)
                                  : device.get_gpu_memory_array(Ebar2_memname, gpu_frame_id,
                                                                _gpu_buffer_depth, Ebar2_length);
    void* const info_memory = device.get_gpu_memory(info_memname, info_length);

    // G2 is an input buffer: check metadata
    const std::shared_ptr<metadataObject> G2_mc =
        args::G2 == args::E ? input_ringbuf_signal->get_metadata(0)
                            : device.get_gpu_memory_array_metadata(G2_memname, gpu_frame_id);
    assert(G2_mc);
    assert(metadata_is_chord(G2_mc));
    const std::shared_ptr<chordMetadata> G2_meta = get_chord_metadata(G2_mc);
    DEBUG("input G2 array: {:s} {:s}", G2_meta->get_type_string(),
          G2_meta->get_dimensions_string());
    assert(std::strncmp(G2_meta->name, G2_name, sizeof G2_meta->name) == 0);
    assert(G2_meta->type == G2_type);
    assert(G2_meta->dims == G2_rank);
    for (std::ptrdiff_t dim = 0; dim < G2_rank; ++dim) {
        assert(std::strncmp(G2_meta->dim_name[G2_rank - 1 - dim], G2_labels[dim],
                            sizeof G2_meta->dim_name[G2_rank - 1 - dim])
               == 0);
        if (args::G2 == args::E && dim == E_index_T)
            assert(G2_meta->dim[G2_rank - 1 - dim] <= int(G2_lengths[dim]));
        else
            assert(G2_meta->dim[G2_rank - 1 - dim] == int(G2_lengths[dim]));
    }
    //
    ...
    //
    // Ebar2 is an output buffer: set metadata
    std::shared_ptr<metadataObject> const Ebar2_mc =
        args::Ebar2 == args::Ebar2 ? output_ringbuf_signal->get_metadata(0)
                                   : device.create_gpu_memory_array_metadata(
                                       Ebar2_memname, gpu_frame_id, E_mc->parent_pool);
    std::shared_ptr<chordMetadata> const Ebar2_meta = get_chord_metadata(Ebar2_mc);
    *Ebar2_meta = *E_meta;
    std::strncpy(Ebar2_meta->name, Ebar2_name, sizeof Ebar2_meta->name);
    Ebar2_meta->type = Ebar2_type;
    Ebar2_meta->dims = Ebar2_rank;
    for (std::ptrdiff_t dim = 0; dim < Ebar2_rank; ++dim) {
        std::strncpy(Ebar2_meta->dim_name[Ebar2_rank - 1 - dim], Ebar2_labels[dim],
                     sizeof Ebar2_meta->dim_name[Ebar2_rank - 1 - dim]);
        Ebar2_meta->dim[Ebar2_rank - 1 - dim] = Ebar2_lengths[dim];
    }
    DEBUG("output Ebar2 array: {:s} {:s}", Ebar2_meta->get_type_string(),
    ...
    array_desc G2_arg(G2_memory, G2_length);
    ...
    void* args[] = {
        ...
        &G2_arg,  &E_arg,    &Ebar2_arg, &info_arg,
    };
    ...
    const std::ptrdiff_t Tbar_ringbuf = output_ringbuf_signal->size / Ebar2_Tbar_sample_bytes;
    ...
    // Update metadata
    Ebar2_meta->dim[Ebar2_rank - 1 - Ebar2_index_Tbar] = Tbarlength;
    assert(Ebar2_meta->dim[Ebar2_rank - 1 - Ebar2_index_Tbar]
           <= int(Ebar2_lengths[Ebar2_index_Tbar]));
    // Since we use a ring buffer we do not need to update `meta->sample0_offset`

    assert(Ebar2_meta->nfreq >= 0);
    assert(Ebar2_meta->nfreq == E_meta->nfreq);
    for (int freq = 0; freq < Ebar2_meta->nfreq; ++freq) {
        Ebar2_meta->freq_upchan_factor[freq] =
            cuda_upchannelization_factor * E_meta->freq_upchan_factor[freq];
        Ebar2_meta->half_fpga_sample0[freq] =
            E_meta->half_fpga_sample0[freq] + cuda_number_of_taps - 1;
        Ebar2_meta->time_downsampling_fpga[freq] =
...
void cudaUpchannelizer_pathfinder_U2::finalize_frame() {
    ...
    DEBUG("    Produced bytes:   {:d}", Tbar_produced * Ebar2_Tbar_sample_bytes);
    output_ringbuf_signal->finish_write(unique_name, instance_num,
                                        Tbar_produced * Ebar2_Tbar_sample_bytes);

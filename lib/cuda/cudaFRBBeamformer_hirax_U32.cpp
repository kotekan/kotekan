...
private:
    ...
    static constexpr std::ptrdiff_t W_length =
        chord_datatype_bytes(W_type) * 2 * 16 * 16 * 2 * 2048;
    ...
    static constexpr std::ptrdiff_t I_length = chord_datatype_bytes(I_type) * 32 * 32 * 2048 * 128;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 16 * 2048;
...
cudaEvent_t cudaFRBBeamformer_hirax_U32::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Finished CUDA FRBBeamformer_hirax_U32 on GPU frame {:d}", gpu_frame_id);

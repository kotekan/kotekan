...
private:
    ...
    // Kernel arguments:
    enum class args { Tmin, Tmax, Tbarmin, Tbarmax, Fmin, Fmax, G_U4, E, Ebar, info, count };
    ...
    static constexpr std::ptrdiff_t G_U4_length = type_total_bytes(G_U4_type) * 512;
    ...
    static constexpr std::ptrdiff_t Ebar_length = type_total_bytes(Ebar_type) * 64 * 2 * 512 * 8192;
...
cudaUpchannelizer_pathfinder_U4::cudaUpchannelizer_pathfinder_U4(Config& config,
    ...
    output_ringbuf_signal(dynamic_cast<RingBuffer*>(host_buffers.get_generic_buffer(
        config.get<std::string>(unique_name, "upchan_U4_voltage_signal_out")))),
    ...
    gpu_buffers_used.push_back(std::make_tuple(G_U4_name, true, true, false));
...
cudaUpchannelizer_pathfinder_U4::execute(cudaPipelineState& /*pipestate*/,
    ...
    const std::string G_U4_memname = G_U4_name + "_buffer";
    ...
    void* const Ebar_memory =
        ...
        : args::Ebar == args::G_U4 ? device.get_gpu_memory(Ebar_memname, Ebar_length)
    ...
    assert(std::strncmp(G_U4_meta->name, G_U4_quantity, sizeof G_U4_meta->name) == 0);
    ...
    *Ebar_meta = *E_meta;
    ...
    DEBUG("Finished CUDA Upchannelizer_pathfinder_U4 on GPU frame {:d}", gpu_frame_id);
    ...
    if (error_code != 0)
        ERROR("CUDA kernel Upchannelizer_pathfinder_U4 returned error code: {}", error_code);
    ...
    for (int block = 0; block < blocks; ++block) {
        for (int warp = 0; warp < info_lengths[info_index_warp]; ++warp) {
            for (int thread = 0; thread < info_lengths[info_index_thread]; ++thread) {
                ...
                if (val != 0)
                    ERROR("CUDA kernel Upchannelizer_pathfinder_U4 returned 'info' value {:d} "

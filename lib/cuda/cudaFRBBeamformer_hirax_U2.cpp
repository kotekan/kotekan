...
private:
    ...
    static constexpr std::ptrdiff_t W_length = chord_datatype_bytes(W_type) * 2 * 16 * 16 * 2 * 2;
    ...
    static constexpr std::ptrdiff_t Ebar_length =
        chord_datatype_bytes(Ebar_type) * 256 * 2 * 2 * 32768;
    ...
    static constexpr std::ptrdiff_t info_length = chord_datatype_bytes(info_type) * 32 * 16 * 2;
...
cudaFRBBeamformer_hirax_U2::cudaFRBBeamformer_hirax_U2(Config& config,
    ...
    if (instance_num == 0) {
        ...
        device.build_ptx(kernel_file_name, {kernel_symbol}, opts, "FRBBeamformer_hirax_U2_");
    }
...
cudaEvent_t cudaFRBBeamformer_hirax_U2::execute(cudaPipelineState& /*pipestate*/,
    ...
    *I_meta = *Ebar_meta;
    ...
    DEBUG("Running CUDA FRBBeamformer_hirax_U2 on GPU frame {:d}", gpu_frame_id);
    ...
    DEBUG("Finished CUDA FRBBeamformer_hirax_U2 on GPU frame {:d}", gpu_frame_id);
    ...
    for (std::size_t i = 0; i < info_host.size() * blocks / max_blocks; ++i)
        if (info_host[i] != 0)
            ERROR("cudaFRBBeamformer_hirax_U2 returned 'info' value {:d} at index {:d} (zero "
                  "indicates no error)",
                  info_host[i], i);
#endif

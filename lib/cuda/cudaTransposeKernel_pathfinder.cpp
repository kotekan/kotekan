...
cudaTransposeKernel_pathfinder::execute(cudaPipelineState& pipestate,
    ...
    const metadataContainer* const mc_Ein =
        device.get_gpu_memory_array_metadata(Ein_memname, pipestate.gpu_frame_id);
    assert(mc_Ein && metadata_container_is_chord(mc_Ein));
    const chordMetadata* const meta_Ein = get_chord_metadata(mc_Ein);
    INFO("input Ein array: {:s} {:s}", meta_Ein->get_type_string(),
         meta_Ein->get_dimensions_string());
    assert(meta_Ein->type == Ein_type);
    assert(meta_Ein->dims == Ein_rank);
    for (std::size_t dim = 0; dim < Ein_rank; ++dim) {
        assert(std::strncmp(meta_Ein->dim_name[dim], Ein_labels[Ein_rank - 1 - dim],
                            sizeof meta_Ein->dim_name[dim])
               == 0);
        assert(meta_Ein->dim[dim] == int(Ein_lengths[Ein_rank - 1 - dim]));
    }
    //
    /// E is an output buffer: set metadata
    metadataContainer* const mc_E = device.create_gpu_memory_array_metadata(
        E_memname, pipestate.gpu_frame_id, mc_Ein->parent_pool);
    chordMetadata* const meta_E = get_chord_metadata(mc_E);
    chord_metadata_copy(meta_E, meta_E);
    meta_E->type = E_type;
    meta_E->dims = E_rank;
    for (std::size_t dim = 0; dim < E_rank; ++dim) {
        std::strncpy(meta_E->dim_name[dim], E_labels[E_rank - 1 - dim],
                     sizeof meta_E->dim_name[dim]);
        meta_E->dim[dim] = E_lengths[E_rank - 1 - dim];
    }
    INFO("output E array: {:s} {:s}", meta_E->get_type_string(), meta_E->get_dimensions_string());
    //

              ...
              __local  uint *localDataY)
{
    ...
    uint base_addr_x = ( (BLOCK_DIM*block_x)
                       ...
                       + get_group_id(0)*NUM_ELEMENTS*NUM_FREQUENCIES )*2u; //times 2 because there are pairs of numbers for the complex values
    uint base_addr_y = ( (BLOCK_DIM*block_y)
                       + FREQUENCY_BAND*NUM_ELEMENTS
                       + get_group_id(0)*NUM_ELEMENTS*NUM_FREQUENCIES )*2u;
    ...
    uint addr_o = ((BLOCK_ID * 2048u) + (LOCAL_Y * 256u) + (LOCAL_X * 8u)) + (FREQUENCY_BAND * NUM_BLOCKS_x_2048) + (get_group_id(0) * NUM_FREQUENCIES * NUM_BLOCKS_x_2048);
    //row 0

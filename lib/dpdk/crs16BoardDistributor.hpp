...
inline crs16BoardDistributor::crs16BoardDistributor(kotekan::Config& config, const std::string& unique_name,
                                      kotekan::bufferContainer& buffer_container, int port,
                                      const std::vector<rte_ring*>& worker_rings) :
    ...
    if(worker_ring_ids.size() == 0) {
        throw std::runtime_error(
            fmt::format(fmt("The crs16BoardDistributor handler must have at least one worker ring.")));
    }

    if (worker_ring_ids.size() % 2 != 0) {
        throw std::runtime_error(
            fmt::format(fmt("The crs16BoardDistributor handler must have an even number of worker rings.")));
    }
    ...
    if (num_crs_boards != 16) {
        throw std::runtime_error(
            fmt::format(fmt("num_crs_boards' parameter must be 16. Other configurations are not supported.")));
    }
}

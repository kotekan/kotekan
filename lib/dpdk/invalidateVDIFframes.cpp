...
void invalidateVDIFframes::main_thread() {
    ...
    while (!stop_thread) {
        ...
        for (int32_t i = 0; i < lost_samples_buf->frame_size; ++i) {
            ...
            // Check array bounds
            assert((frame_location + vdif_frame_size * num_elements) <= (uint32_t)out_buf->frame_size);
            if (flag_frame[i] == 1) {
                // There is one VDIF frame generated for each element extracted from a sample.  So losing one sample
                // results in losing one or more (default 2) VDIF frames.

...
#include "chimeMetadata.hpp"      // for chimeMetadata, get_chime_metadata, get_dataset_id, get_...
...
#include "visUtil.hpp"            // for frameID, prod_ctype, modulo, input_ctype, operator+
...
visAccumulate::internalState::internalState(Buffer* out_buf, std::unique_ptr<gateSpec> gate_spec,
                                            size_t nprod) :
    buf(out_buf), frame_id(buf), spec(std::move(gate_spec)), changed(true), vis1(2 * nprod),
    vis2(nprod) {}

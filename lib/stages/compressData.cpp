...
compressData::compressData(Config& config_, const string& unique_name,
    ...
    Stage(config_, unique_name, buffer_container,
          std::bind(&compressData::main_thread, this)) {
...
void compressData::main_thread() {
    ...
    while (!stop_thread) {
      ...
      uint32_t* compressed_data = (uint32_t*) malloc(((max_dst_size/sizeof(uint32_t)) +1)*sizeof(uint32_t));
      ...
      INFO("compression rate (MB/s): %f", float(num_elements * sizeof(char)) * 1e-6 / elapsed0.count());
      INFO("decompression ratio (to original): %f", float(recovered_samples)/float(num_elements));
      INFO("decompression rate (MB/s): %f", float(recovered_samples * sizeof(uint8_t)) * 1e-6 / elapsed1.count());

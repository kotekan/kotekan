...
DotProduct::DotProduct(Config& config, const std::string& unique_name,
    ...
    Stage(config, unique_name, buffer_container, 
            std::bind(&DotProduct::main_thread, this)) {
...
void DotProduct::main_thread() {
    ...
    while (!stop_thread) {
        ...
        uint8_t* frame_a_ptr = wait_for_full_frame(in_a_buf,
                                            unique_name.c_str(),
                                            in_a_frame_id);
        uint8_t* frame_b_ptr = wait_for_full_frame(in_b_buf,
                                            unique_name.c_str(),
                                            in_b_frame_id);
        ...
        INFO("Output buffer {:s}[{:d}]: {:f}, ..., {:f}, ..., {:f}", out_buf->buffer_name, out_frame_id, output[0], output[_num_elements / 2], output[_num_elements - 1]);

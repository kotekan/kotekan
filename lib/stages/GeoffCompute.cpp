...
REGISTER_KOTEKAN_STAGE(GeoffCompute);

STAGE_CONSTRUCTOR(GeoffCompute) {
...
GeoffCompute::~GeoffCompute() {}

void GeoffCompute::main_thread() {
    ...
    while (!stop_thread) {
        ...
        for (uint32_t i = 0; i < frame_length; i++) {
            ...
            for(uint32_t j = 0; j < frame_length; j++) {
                uint32_t k = i >= j ? i - j : i + frame_length - j;
                output[i] += a[j]*b[k];
            }
            output[i] /= frame_length;
        }

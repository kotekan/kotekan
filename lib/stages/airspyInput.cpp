...
struct airspy_device* airspyInput::init_device() {
    ...
    if (result != AIRSPY_SUCCESS) {
        ERROR("airspy_set_vga_gain() failed: %s (%d)", airspy_error_name((enum airspy_error)result),
              result);
        return NULL;
    }
    ...
    if (result != AIRSPY_SUCCESS) {
        ERROR("airspy_set_lna_gain() failed: %s (%d)", airspy_error_name((enum airspy_error)result),
              result);
        return NULL;
    }
    ...
    if (result != AIRSPY_SUCCESS) {
        ERROR("airspy_set_rf_bias() failed: %s (%d)", airspy_error_name((enum airspy_error)result),
              result);
        return NULL;
    }
    ...
    INFO("Part ID Number: 0x%08X 0x%08X", read_partid_serialno.part_id[0],
         read_partid_serialno.part_id[1]);
    INFO("Serial Number: 0x%08X%08X", read_partid_serialno.serial_no[2],

...
void BeamBufferSort::main_thread(){
	...
	//outputs.push_back(std::vector<uint8_t*>(wait_nframe, nullptr));
    }
	...
	if (time_offset >= wait_nframes){ // Frame comes beyond the waiting frames, dump old frames.
		...
		for (uint32_t j = 0; j < queue_status[i].size(); j++){
			...
			fill_pos = j * sub_frame_size;
		        fake_fpga_seq_start = fpga_seq_start0 + i;
		        add_nsec(fake_ctime, (long)(i * subframe_time_nsec)); 	
			fake_freq_bin = j;
			make_freq_meta((FreqIDBeamMetadata*) &sort_queue[i][fill_pos], fake_freq_bin, fake_fpga_seq_start,
					fake_ctime, 0, dset_id_t::null, beam_number, ra, dec, scaling);
			memset(&sort_queue[i][fill_pos + FreqIDBeamMeta_size], 0, in_buf->frame_size);
	                //FreqIDBeamMetadata* debug_meta = (FreqIDBeamMetadata*) &sort_queue[i][fill_pos];
                        //INFO("Fill Debug RA {:f} Dec: {:f}, scaling: {:d}, beam_num: {:d}, freq_id {:d}, seq_start {:d}\n",

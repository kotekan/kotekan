...
void BeamBufferSort::main_thread(){
	...
	if (time_offset >= wait_nframes){ // Frame comes beyond the waiting frames, dump old frames.
	    ...
	    // dump offset is the position where data has been dump in the frame.
            ...
            for(uint32_t i = 0; i < advance_nframe + 1; i++){
                for (uint32_t j = 0; j < queue_status.at(i).size(); j++){
                    if (queue_status.at(i).at(j) == 0){
	    ...
	    //INFO("Start dumping advance {:d}", advance_nframe);
            ...
            for (uint32_t i = 0; i < total_freq_chan; i++){
                curr_queue0_metadata.at(i) = (FreqIDBeamMetadata*)&sort_queue.at(0).at(i)[0];
	    }
		...
		if (in_frame_dump_offset >= queue_frame_size){
		    ...
		    dump_frame_count++;
	            // Rotate the queue
                    // Reset queue status
                    std::fill(queue_status.at(0).begin(), queue_status.at(0).end(), 0);
		...
		// Dump out data to a new buffer. 
			...
			// Get a new output buffer frame
		        outputs.at(k) = wait_for_empty_frame(out_bufs.at(k), unique_name.c_str(), output_frame_ids.at(k));
                        if (outputs.at(k) == nullptr)
                            break;
		        ...
		        // Fill the merge metadata
                        out_metadata -> sub_frame_pre_frame = out_buf_nchan.at(k);
			...
			out_metadata -> n_pol = num_pol;
                        out_metadata -> freq_start = out_buf_start_chan.at(k);
                        out_metadata -> nchan = out_buf_nchan.at(k);
			// Start fpga seq in the top of frequency band
                        out_metadata -> fpga_seq_start = curr_queue0_metadata.at(out_buf_start_chan.at(k)) -> fpga_seq_start + in_frame_dump_offset;
                        out_metadata -> ctime = curr_queue0_metadata.at(out_buf_start_chan.at(k)) -> ctime;
		    ...
		    src_start = FreqIDBeamMeta_size + in_frame_dump_offset * num_pol;
                    ...
                    for (uint32_t ch_id=out_buf_start_chan.at(k); ch_id < out_buf_nchan.at(k) + out_buf_start_chan.at(k); ch_id++){
			// Get the metadata for each single input frame
			dest_start = (ch_id - out_buf_start_chan.at(k)) * (dump_size * num_pol + FreqIDBeamMeta_size);
			DEBUG2("Buffer {:d} freq_id {:d} dest {:d} copy_size {:d}", k, ch_id, dest_start, copy_size); 
		        if (output_buff_offset == 0){
                            FreqIDBeamMetadata* dump_frame_metadata = (FreqIDBeamMetadata*)&sort_queue.at(0).at(ch_id)[0];
		            // Write output meta	
                            FreqIDBeamMetadata* sub_frame_metadata = (FreqIDBeamMetadata*)&outputs.at(k)[dest_start];
			...
			DEBUG2("Buffer {:d} freq_id {:d} dest {:d} copy_size {:d}", k, ch_id, dest_start + FreqIDBeamMeta_size + output_buff_offset * num_pol, copy_size);
                        memcpy(&outputs.at(k)[dest_start + FreqIDBeamMeta_size + output_buff_offset * num_pol], \
			       &sort_queue.at(0).at(ch_id)[src_start], copy_size * num_pol);	
	      		//INFO("Buff {:d} freq {:d}, rec offset {:d} ptr {} framesize {:d} Output0 {:d} + {:d}i output1 {:d} + {:d}i", k, ch_id, output_buff_offset, \
			     fmt::ptr(outputs.at(k)), out_bufs.at(k) -> frame_size,
		    ...
		    if (!dump_started.at(k))
		        dump_started.at(k) = true;
	...
	FreqIDBeamMetadata* sub_frame_metadata = (FreqIDBeamMetadata*)&sort_queue.at(time_offset).at(freq_bin)[0];
	memcpy(sub_frame_metadata, in_metadata, FreqIDBeamMeta_size);
	// copy data
	uint8_t* sub_frame_data = &sort_queue.at(time_offset).at(freq_bin)[FreqIDBeamMeta_size];
	memcpy(sub_frame_data, input, in_buf->frame_size);
         
	// mark fill status is done
	queue_status.at(time_offset).at(freq_bin) = 1;
	// Mark in frame empty
	mark_frame_empty(in_buf, unique_name.c_str(), input_frame_id);
        input_frame_id++;
    }

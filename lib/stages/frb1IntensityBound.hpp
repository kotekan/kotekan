 ...
 *
 * @param W                 Weights of one frequency, laid out as [P][dishN][dishM][C]
 * @param num_polarizations Number of polarizations P
 * @param num_dishes_M      Dish grid size M
 * @param num_dishes_N      Dish grid size N
 * @param num_components    Number of components C of each weight: 2 for complex (re/im) weights,
 *                          1 for real ones
...
inline double frb1_intensity_bound(const float16_t* const W, const int num_polarizations,
    ...
    for (int polr = 0; polr < num_polarizations; ++polr) {
        const float16_t* const W_polr = W + num_components * num_dishes * polr;
        double sum_dish = 0;
        for (std::ptrdiff_t dish = 0; dish < num_dishes; ++dish) {
            // |W| over all components
            double abs2 = 0;
            for (int c = 0; c < num_components; ++c) {
                const double w = double(W_polr[num_components * dish + c]);
                abs2 += w * w;
            }
            sum_dish += std::sqrt(abs2);
        }

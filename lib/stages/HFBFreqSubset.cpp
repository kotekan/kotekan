...
HFBFreqSubset::HFBFreqSubset(Config& config, const std::string& unique_name,
    ...
    Stage(config, unique_name, buffer_container, std::bind(&HFBFreqSubset::main_thread, this)) {
...
void HFBFreqSubset::change_dataset_state(dset_id_t input_dset_id) {
...
void HFBFreqSubset::main_thread() {
    ...
    while (!stop_thread) {
        ...
        if (dset_id_map.count(input_frame.dataset_id) == 0) {
            change_dset_fut =
                std::async(&HFBFreqSubset::change_dataset_state, this, input_frame.dataset_id);
        }

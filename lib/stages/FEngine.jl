...
function make_dishes(ndishes_i::Int, ndishes_j::Int)
    ...
    locations = NTuple{2,Float}[let
                                ...
                                for i in 1:ndishes_i for j in 1:ndishes_j]
...
function plot(frame::FFrame{T}) where {T<:Real}
    ...
    fig = Figure(; resolution=(1280, 960))
    ax = Axis(fig[1, 1]; title="F-engine field frame", xlabel="dish, polr", ylabel="freq")
...
function quantize(::Type{I}, inframes::AbstractVector{<:FFrame}) where {I<:Integer}
    ...
    quantize1(x::Real) = clamp(round(I, x / maxabs * (2 * outrange + 1) / 2), (-I(outrange)):(+I(outrange)))
...
struct XFrame{T}
    ...
    f₀::Float
    Δf::Float
    data::Array{Complex{T},4} # [dish, polr, time, freq]
end
...
function plot(frame::XFrame{T}) where {T<:Real}
    ...
    fig = Figure(; resolution=(1280, 960))
    ax = Axis(fig[1, 1]; title="X-engine field frame", xlabel="time", ylabel="dish, polr")
    obj = heatmap!(ax, times, dishes, data; colormap=:plasma)
...
function corner_turn(inframes::AbstractVector{FFrame{T}}, ntimes::Int) where {T<:Real}
    ...
    outframes = Vector{XFrame{T}}(undef, size(inframes′, 2))
    @threads for n in 1:size(inframes′, 2)
        outframes[n] = let
            ...
            data = Complex{T}[
                              ...
                              for dish in 1:ndishes, polr in 1:npolrs, time in 1:ntimes,
                                  freq in 1:nfreqs
                              ]
...
function bb(::Type{T}, A::AbstractArray{<:Complex,4}, E::AbstractArray{<:Complex,4}) where {T<:Real}
    ...
    # E: [dish, time, polr, freq]
    # J: [beam, time, polr, freq]
    ndishes, nbeams, npolrs, nfreqs = size(A)
    ndishes′, ntimes, npolrs′, nfreqs′ = size(E)
    @assert (ndishes, npolrs, nfreqs) == (ndishes′, npolrs′, nfreqs′)
    J = Array{Complex{T},4}(undef, nbeams, ntimes, npolrs, nfreqs)
    @threads for freq in 1:nfreqs
        for beam in 1:nbeams, time in 1:ntimes, polr in 1:npolrs
            J[beam, time, polr, freq] = let
                J0 = zero(Complex{T})
                for dish in 1:ndishes
                    J0 += A[dish, beam, polr, freq] * E[dish, time, polr, freq]
                end
                J0
            end
        end
...
function make_baseband_beams(nbeamsi::Int, nbeamsj::Int, Δθi::T, Δθj::T) where {T<:Real}
    sinxys = NTuple{2,T}[let
                         ...
                         for beami in 1:nbeamsi for beamj in 1:nbeamsj]
...
struct BBeams{T}
    ...
    data::Array{Complex{T},4}   # [beam, polr, time, freq]
end
...
function plot(beams::BBeams{T}) where {T<:Real}
    data = (sum(abs2, beams.data; dims=(2, 3, 4)) / size(beams.data, 3))::AbstractArray{T,4}
    data = reshape(data, size(data, 1))::AbstractArray{T,1}
...
function bb(::Type{T}, frames::AbstractVector{<:XFrame}, dishes::Dishes, sinxys::Vector{NTuple{2,Float}}) where {T<:Real}
    ndishes, npolrs, ntimes, nfreqs = size(frames[begin].data)
    ...
    beamss = BBeams{T}[]
    for frame in frames
        # frame: [dish, polr, time, freq]
        # E: [dish, time, polr, freq]
        E = PermutedDimsArray(frame.data, (1, 3, 2, 4))
        J = bb(T, A, E)
        # J: [beam, time, polr, freq]
        # frame: [beam, polr, time, freq]
        phases = A
        data = Array(PermutedDimsArray(J, (1, 3, 2, 4)))
        beams = BBeams(frame.t₀, frame.Δt, frame.f₀, frame.Δf, sinxys, phases, data)
        push!(beamss, beams)
    end
...
function quantize(::Type{I}, inbeams::AbstractVector{<:BBeams}) where {I<:Integer}
    ...
    quantize1_phases(x::Real) = I(clamp(round(Int, x / maxabs_phases * (2 * outrange + 1) / Float(2)), (-outrange):(+outrange)))
    ...
    quantize1_beams(x::Real) = I(clamp(round(Int, x / maxabs_beams * (2 * outrange + 1) / Float(2)), (-outrange):(+outrange)))
...
function run(ndishes_i::Int64, ndishes_j::Int64, ntaps::Int64, nfreq::Int64, ntimes::Int64, do_plot::Bool)
    source = let
        A = Complex{Float}(1)
        f₀ = Float(0.3e+9)
        sinx = Float(0.02)
    ...
    dishes = make_dishes(ndishes_i, ndishes_j)
    ...
    println("Corner turn output: $(length(xframes)) frames of size (ndishes, npolrs, ntimes, nfreqs)=$(size(xframes[begin].data)) t₀=$(xframes[begin].t₀) Δt=$(xframes[begin].Δt) f₀=$(xframes[begin].f₀) Δf=$(xframes[begin].Δf)")
    if do_plot
...
function setup(ndishes_i=8, ndishes_j=8, ntaps=4, nfreq=64, ntimes=64)
    return run(Int64(ndishes_i), Int64(ndishes_j), Int64(ntaps), Int64(nfreq), Int64(ntimes), false)
end

stored_xframes = nothing
function set_E(ptr::Ptr{UInt8}, sz::Int64, frame_index::Int64)
    println("set_E.0")
    println("set_E.1 sz=$sz frame_index=$frame_index")
    xframes = stored_xframes::AbstractVector{<:XFrame}
    println("set_E.2 length(xframes)=$(length(xframes))")
    xframe = xframes[frame_index]::XFrame
    println("set_E.3 length(xframe.data)=$(length(xframe.data)) size(xframe.data)=$(size(xframe.data))")
...
function set_A(ptr::Ptr{UInt8}, sz::Int64, frame_index::Int64)
    println("set_A.0")
    flush(stdout)
    println("set_A.1 sz=$sz")
    flush(stdout)
    beamss = stored_beamss::AbstractVector{<:BBeams}
    println("set_A.2 length(beamss)=$(length(beamss))")
    beams = beamss[frame_index]::BBeams
    println("set_A.3 length(beams.phases)=$(length(beams.phases)) size(beams.phases)=$(size(beams.phases))")
    @assert length(beams.phases) * sizeof(Complex{Int8}) == sz
    @assert sz % sizeof(Complex{Int8}) == 0
...
function set_J(ptr::Ptr{UInt8}, sz::Int64, frame_index::Int64)
    println("set_J.0")
    println("set_J.1 sz=$sz frame_index=$frame_index")
    beamss = stored_beamss::AbstractVector{<:BBeams}
    println("set_J.2 length(beamss)=$(length(beamss))")
    beams = beamss[frame_index]::BBeams
    println("set_J.3 length(beams.data)=$(length(beams.data)) size(beams.data)=$(size(beams.data))")

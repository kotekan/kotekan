...
processGains::processGains(Config& config, const std::string& unique_name,
    ...
    gain_frb_buf->set_frame_desc(
        kotekan::NDArray<kotekan::GetType_t<kotekan::float32>, 4>::describe(
            "gain_frb",
    ...
    gain_tracking_buf->set_frame_desc(
        kotekan::NDArray<kotekan::GetType_t<kotekan::float32>, 4>::describe(
            "gain_tracking",
...
void processGains::main_thread() {
    ...
    while (!stop_thread) {
        ...
        frb_meta->set_from_frame_desc(gain_frb_buf->get_frame_desc<kotekan::GenericNDArray>());
        frb_meta->set_name("gain_frb");
        // Verify that frame desc and metadata match
        frb_meta->check_frame_desc(gain_frb_buf->get_frame_desc<kotekan::GenericNDArray>());
        {
        ...
        tracking_meta->set_from_frame_desc(
            gain_tracking_buf->get_frame_desc<kotekan::GenericNDArray>());
        tracking_meta->set_name("gain_tracking");
        // Verify that frame desc and metadata match
        tracking_meta->check_frame_desc(
            gain_tracking_buf->get_frame_desc<kotekan::GenericNDArray>());
        {

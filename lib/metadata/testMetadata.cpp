...
void readMetadata(const std::shared_ptr<const Metadata>& meta) {
    ...
    assert(meta->has_float("frequency"));
    assert(!meta->has_float("frequency1"));
    ...
    assert(meta->has_float_vector("dish_separations"));
    assert(!meta->has_float_vector("dish_separations1"));
    ...
    assert(meta->float_size() == meta->float_keys().size());
    ...
    assert(meta->float_vector_size() == meta->float_vector_keys().size());
    ...
    assert(isequal(meta->float_keys(), std::vector<std::string>{"frequency"}));
    ...
    assert(isequal(meta->float_vector_keys(), std::vector<std::string>{"dish_separations"}));
    ...
    assert(meta->get_float("frequency") == 3000.0e+6);
    ...
    assert(isequal(meta->get_float_vector("dish_separations"), std::vector<double>{6.3, 8.5}));

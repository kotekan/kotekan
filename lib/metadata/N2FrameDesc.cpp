...
N2FrameDesc N2FrameDesc::_from_config_impl(kotekan::Config& config, const std::string& location) {
    ...
    switch (n2_layout) {
        ...
        case N2Layout::DishInputs: {
            // A compact frame over the telescope's connected elements in the fiducial
            // order. The config's num_elements is the full array count they are selected
            // from; the descriptor's is the selection's size, and the product list is the
            // dense triangle over that axis, generated by the main constructor.
            ...
            num_elements =
                (uint32_t)tel->get_connected_elements(tel->fiducial_element_order()).size();
            if (num_elements == 0)
                FATAL_ERROR_NON_OO("n2_layout DishInputs: dish_inputs has no connected dish");
            break;
        }
...
bool N2FrameDesc::layout_requires_product_list(N2Layout layout) {
    ...
    switch (layout) {
        ...
        // their own element axis, derivable from num_elements.
        case N2Layout::DishInputs:
            return false;
        case N2Layout::InputANDMasked:
        case N2Layout::InputORMasked:

...
#include "CHORDTelescope.hpp" // for CHORDTelescope, DishType, dishInputFields
...
// The connected elements -- those whose dish type is not Fake, array dishes and RFI
// antennas alike -- in the telescope's fiducial element order, for the DishInputs
// layout.
static std::vector<uint16_t> dish_input_list(uint32_t num_elements) {
    const Telescope& tel = Telescope::instance();
    const CHORDTelescope* const chord_tel = dynamic_cast<const CHORDTelescope*>(&tel);
    if (chord_tel == nullptr)
        FATAL_ERROR_NON_OO("n2_layout DishInputs requires a CHORDTelescope");

    dishInputFields dish_inputs;
    chord_tel->fill_input_maps(dish_inputs);
    if (std::count(dish_inputs.type.begin(), dish_inputs.type.end(), DishType::Fake)
        == (std::ptrdiff_t)dish_inputs.type.size())
        FATAL_ERROR_NON_OO("n2_layout DishInputs requires a dish_inputs table with at least "
                           "one non-Fake entry (every dish reads as Fake without one)");

    std::vector<uint16_t> input_list;
    for (uint32_t el = 0; el < num_elements; ++el) {
        const station_id_t st_id =
            tel.element_index_to_station_id(el, tel.fiducial_element_order());
        uint64_t dish;
        uint64_t pol;
        chord_tel->decode_station_id(st_id, dish, pol);
        if (dish_inputs.type.at(dish) != DishType::Fake)
            input_list.push_back((uint16_t)el);
    }
    return input_list;
}
...
N2FrameDesc::N2FrameDesc(uint32_t num_elements, uint32_t num_ev, uint32_t num_products,
    ...
    // Compact layouts carry each element's full-order identity; the others index the
    ...
    } else if (!this->input_list.empty()) {
        FATAL_ERROR_NON_OO("N2FrameDesc: layout {:s} does not take an input_list",
...
N2FrameDesc N2FrameDesc::_from_config_impl(kotekan::Config& config, const std::string& location) {
    ...
    switch (n2_layout) {
        ...
        case N2Layout::DishInputs:
            // A compact frame over the connected (non-Fake) elements: the config's
            // num_elements is the full element count of the fiducial order from which
            // they are selected, and the descriptor's is the selection's size. The
            // elements' identities are kept in input_list; the product list is the
            // dense triangle over the frame's own element axis, generated by the main
            // constructor.
            input_list = dish_input_list(num_elements);
            num_elements = (uint32_t)input_list.size();
            break;
...
nlohmann::json N2FrameDesc::to_json() const {
    ...
    // Compact layouts send their elements' identities, so a receiver deriving its own
    // (e.g. DishInputs from its dish table) fails loudly on a divergent selection.
    if (!input_list.empty())
        j["input_list"] = input_list;
    return j;
}
...
std::shared_ptr<const FrameDesc> N2FrameDesc::from_json(const nlohmann::json& j) {
    ...
    // Compact layouts carry their elements' identities; for the others an input_list
    // arriving anyway is malformed input.
    std::vector<uint16_t> input_list;
    if (n2_layout == N2Layout::DishInputs) {
        input_list = j.at("input_list").get<std::vector<uint16_t>>();
    } else if (j.contains("input_list")) {
        FATAL_ERROR_NON_OO("N2FrameDesc::from_json: unexpected input_list for layout {:s}",
                           N2Layout_to_string(n2_layout));
    }
    ...
    return std::make_shared<N2FrameDesc>(num_elements, num_ev, num_products, n2_layout,
                                         std::move(product_list), std::move(input_list));
}
...
bool N2FrameDesc::layout_requires_product_list(N2Layout layout) {
    ...
    switch (layout) {
        case N2Layout::FullUpperTri:
        case N2Layout::Autocorrelations:
        // DishInputs frames are compact: their product list is the dense triangle over
        // their own element axis, derivable from num_elements; the element identities
        // travel in input_list instead.
        case N2Layout::DishInputs:
            return false;
        case N2Layout::InputANDMasked:
        case N2Layout::InputORMasked:
...
const char* N2FrameDesc::note_additional_required_config_param(N2Layout layout) {
    switch (layout) {
        ...
        case N2Layout::Autocorrelations:
            return "none";
        case N2Layout::DishInputs:
            return "none (derived from the telescope's dish table)";
        case N2Layout::InputORMasked:
        case N2Layout::InputANDMasked:
            return "input_list";
        case N2Layout::GeneralSubset:
...
bool N2FrameDesc::operator==(const FrameDesc& other) const {
    ...
    // Compact layouts must agree on which elements they carry
    if (input_list != other_ptr->input_list)
        return false;

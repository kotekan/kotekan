...
// The elements whose dish type is ArrayDish, in the telescope's fiducial element
// order, for the DishInputs layout. Fake and RFI-antenna inputs are excluded,
// matching the elements bufferBadInputs can ever unmask.
static std::vector<uint16_t> dish_input_list(uint32_t num_elements) {
    ...
    if (std::count(dish_inputs.type.begin(), dish_inputs.type.end(), DishType::ArrayDish) == 0)
        FATAL_ERROR_NON_OO("n2_layout DishInputs requires a dish_inputs table with at least "
                           "one ArrayDish entry (every dish reads as Fake without one)");
...
N2FrameDesc N2FrameDesc::_from_config_impl(kotekan::Config& config, const std::string& location) {
    ...
    switch (n2_layout) {
        ...
        case N2Layout::DishInputs:
            // A compact frame over the ArrayDish elements: the config's num_elements is
            // the full element count of the fiducial order from which they are
            // selected, and the descriptor's is the selection's size. The elements'
            // identities are kept in input_list; the product list is the dense
            // triangle over the frame's own element axis, generated by the main
            // constructor.

/**
 * @file
 * @brief Function to interact with a Julia interpreter
 *
 */
...
/**
 * @brief Start up the Julia run-time system
 */
void juliaStartup();
/**
 * @brief Shut down the Julia run-time system
 */
void juliaShutdown();

/**
 * @brief Execute some code that may call the Julia run-time.
 *
 * It is NOT possible to call functions that are defined via Julia
 * code directly, but such functions can be called via the Julia
 * run-time system. See [Embedding
...
/**
 * @brief Execute a Julia function
 *

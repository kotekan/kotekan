...
void Stage::start() {
    this_thread = std::thread([&]() {
        ...
        try {
            ...
            // FATAL_ERROR already called exit_kotekan; avoid terminating the whole application abruptly.
            ERROR("Stage {:s} caught FatalError: {:s}, attempting controlled shutdown.", unique_name, e.what());

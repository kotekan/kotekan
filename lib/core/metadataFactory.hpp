...
struct metadataRegistry {
    ...
    void record(const std::string& key, metadataMaker* proc) {
        if ( isMetadataRegistered(key) ) {
            ERROR_NON_OO("Multiple Kotekan Metadata-s registered as '{:s}'!", key);
            throw std::runtime_error("A Metadata was registered twice!");
        }
    ...
    metadataMaker* retrieve(const std::string& key) {
        ...
        if ( proc == registered_metadatas.end()) {
            ...
            throw std::runtime_error("Tried to instantiate a metadata object which is not registered!");
        }
...
struct metadataMakerTemplate : metadataMaker {
    ...
    virtual void* create() const override { return new T(); }
    virtual size_t size() const override { return sizeof(T); }
};

#ifndef REGISTER_KOTEKAN_METADATA
/// special macros for registration of metadata structures
#define REGISTER_KOTEKAN_METADATA(T)
#define REGISTER_KOTEKAN_METADATA_ONCE(T) static ::kotekan::metadataMakerTemplate<T> maker##T(#T);
...
private:
    ...
    metadataPool* new_pool(const std::string& pool_type, const std::string& location)
    {
        ...
        return create_metadata_pool(num_metadata_objects, 
                    metadataRegistry::kotekan_reg_retrieve(pool_type)->size(), location.c_str());
    }

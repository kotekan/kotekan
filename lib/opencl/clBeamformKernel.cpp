...
cl_mem clBeamformKernel::get_freq_map(int frame_index) {
    ...
    std::vector<int> coarse_freq = get_chord_metadata(network_buf, frame_index)->get_coarse_freq();
    assert(num_local_freq <= get_chord_metadata(network_buf, frame_index)->get_nfreq());

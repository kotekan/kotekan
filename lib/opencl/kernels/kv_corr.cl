            ...
            __global int *block_lock)
{
    ...
    uint dest_x = (((yl+1)%8)*8 + xl  )*4;
    uint dest_y = (((xl+1)%8)   + yl*8)*4;
    ...
    for ( ; t<SAMPLES_PER_DATA_SET; t+=256){
        ...
        for (int j=0; j<256; j+=8){
            ...
            for (int i=0; i<8; i++){
                ...
                #pragma unroll
                for (int k=0; k<4; k++){
                    x_re[k] = __builtin_amdgcn_ds_bpermute(dest_x,x_re[k]);
                    x_im[k] = __builtin_amdgcn_ds_bpermute(dest_x,x_im[k]);
                    y_ir[k] = __builtin_amdgcn_ds_bpermute(dest_y,y_ir[k]);
                }

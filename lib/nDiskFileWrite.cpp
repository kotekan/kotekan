...
void nDiskFileWrite::save_meta_data() {
    ...
    int num_freq = config.get_int("/raw_capture/num_freq");
    int num_elements = config.get_int("/processing/num_elements");
    int samples_per_file = config.get_int("/processing/samples_per_data_set");
    const int vdif_header_len = 32;
    const int bit_depth = 4;
    string note = config.get_string("/raw_capture/note");
...
void nDiskFileWrite::main_thread() {
    ...
    string gain_file_dir = config.get_string("/raw_capture/gain_file_dir");

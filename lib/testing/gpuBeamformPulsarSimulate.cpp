...
void gpuBeamformPulsarSimulate::calculate_phase(struct psrCoord psr_coord, timespec time_now, float freq_now, float *gains, double *output) {
    float FREQ = freq_now;
    struct tm * timeinfo;
...
void gpuBeamformPulsarSimulate::cpu_beamform_pulsar(double *input_unpacked, double *phase, float *cpu_output, int _samples_per_data_set, int _num_elements, int _num_pulsar, int _num_pol)
{
    float sum_re, sum_im;
        ...
        for (int b=0;b<_num_pulsar;b++){
            for (int p=0;p<_num_pol;p++){
                ...
                for (int n=0;n<1024;n++){
                    ...
                    sum_re += input_unpacked[(t*_num_elements+p*1024+n)*2]*phase[((b*_num_pol+p)*1024+n)*2] + input_unpacked[(t*_num_elements+p*1024+n)*2+1]*phase[((b*_num_pol+p)*1024+n)*2+1];
                    sum_im += (input_unpacked[(t*_num_elements+p*1024+n)*2+1]*phase[((b*_num_pol+p)*1024+n)*2] - input_unpacked[(t*_num_elements+p*1024+n)*2]*phase[((b*_num_pol+p)*1024+n)*2+1]);
                }
...
void gpuBeamformPulsarSimulate::main_thread() {
    ...
    while(!stop_thread) {
        ...
        INFO("[CPU] freq_now=%d freq+MHz=%.2f metadat_buf_id=%d input_buf_id=%d", freq_now, freq_MHz,metadata_buffer_id,input_buf_id);

        //Load in gains (without dynamic update capability like in GPU, just get set once in the beginning)
        FILE *ptr_myfile = NULL;
        char filename[512];
        for (int b=0;b<_num_pulsar;b++){
            snprintf(filename, sizeof(filename), "%s/quick_gains_%04d_reordered.bin",_gain_dir[b].c_str(), freq_now);
            ...
            if (ptr_myfile == NULL){
                ...
                if (_num_elements != fread(&cpu_gain[b*_num_elements*2],sizeof(float)*2, _num_elements, ptr_myfile)){

...
void fakeGpuBuffer::main_thread() {
    ...
    while (!stop_thread) {
        ...
        (this->*fill)(output, frame_count, (chimeMetadata*)out_buf->metadata[frame_id]->metadata);
...
void fakeGpuBuffer::fill_mode_block(int32_t* data, int frame_number, chimeMetadata* metadata) {
...
void fakeGpuBuffer::fill_mode_accumulate(int32_t* data, int frame_number, chimeMetadata* metadata) {
    ...
    for (int i = 0; i < num_elements; i++) {
        for (int j = i; j < num_elements; j++) {
            ...
            // row.
            data[2 * bi + 1] =
                (i - 4 * ((frame_number + 1) % 4 == 0) + 1) * samples_per_data_set; // Real
        }
    }
}

void fakeGpuBuffer::fill_mode_gaussian(int32_t* data, int frame_number, chimeMetadata* metadata) {
    // Parameters not used in this fill mode, suppress warning.
    (void)frame_number;
    (void)metadata;
...
void fakeGpuBuffer::fill_mode_pulsar(int32_t* data, int frame_number, chimeMetadata* metadata) {

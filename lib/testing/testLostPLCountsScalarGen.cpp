...
/**
 * @class testLostPLCountsScalarGen
 * @brief Generate test PL Lost Counts as a standin for CountLostPLSamplesScalar.
 *
 * @par Buffers
 * @buffer out_buf Buffer to fill with PL counts
 *         @buffer_format int32
 *         @buffer_shape [samples_per_data_set/sub_integration_ntime,
 *              num_local_freq]
 *         @buffer_metadata chordMetadata
 *
 * @conf  name                  String. quantity name for PL Lost counts
 ...
 */
class testLostPLCountsScalarGen : public Stage {
public:
    testLostPLCountsScalarGen(Config& config, const std::string& unique_name,
                              bufferContainer& buffer_container);
    ~testLostPLCountsScalarGen(){};
...
REGISTER_KOTEKAN_STAGE(testLostPLCountsScalarGen);

testLostPLCountsScalarGen::testLostPLCountsScalarGen(Config& config, const std::string& unique_name,
                                                     bufferContainer& buffer_container) :
    Stage(config, unique_name, buffer_container,
          std::bind(&testLostPLCountsScalarGen::main_thread, this)),
    name(config.get_default<std::string>(unique_name, "name", "pl_lost_counts_scalar")),
...
std::shared_ptr<chordMetadata> testLostPLCountsScalarGen::get_new_metadata(Buffer* buf,
                                                                           frameID frame_id) {
...
void testLostPLCountsScalarGen::set_metadata(const std::shared_ptr<chordMetadata>& meta,
                                             uint64_t seq_num) {
...
void testLostPLCountsScalarGen::main_thread() {
    ...
    while (!stop_thread) {
        ...
        // grab frames
        int32_t* framemask = (int32_t*)out_buf->wait_for_empty_frame(unique_name, frame_id);

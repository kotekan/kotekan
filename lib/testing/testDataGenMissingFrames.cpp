...
void testDataGenMissingFrames::main_thread() {
    ...
    while (!stop_thread) {
        float* input = (float*)wait_for_full_frame(input_buf, unique_name.c_str(), input_buf_id);
        if (input == NULL)
            break;

        if (std::find(_missing_frames.begin(), _missing_frames.end(), num_frames)
            == _missing_frames.end()) {
            float* output =
        ...
        num_frames = (num_frames + 1) % input_buf->num_frames;

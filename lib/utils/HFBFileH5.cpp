...
REGISTER_VIS_FILE("HFB_hdf5", HFBFileH5);
REGISTER_VIS_FILE("HFB_hdf5fast", HFBFileH5Fast);


//
// Implementation of standard HDF5 21cm absorber data file
//

HFBFileH5::HFBFileH5(const std::string& name, const kotekan::logLevel log_level,
...
void HFBFileH5::create_axes(const std::vector<freq_ctype>& freqs,
...
template<typename T>
void HFBFileH5::create_axis(std::string name, const std::vector<T>& axis) {
...
void HFBFileH5::create_time_axis() {
...
void HFBFileH5::create_datasets() {

    Group flags = file->createGroup("flags");

    create_dataset("hfb", {"time", "freq", "prod"}, create_datatype<cfloat>());
    create_dataset("flags/hfb_weight", {"time", "freq", "prod"}, create_datatype<float>());
    ...
    dset("hfb_weight").createAttribute<std::string>("type", ds).write(wt);

    file->flush();
}

void HFBFileH5::create_dataset(const std::string& name, const std::vector<std::string>& axes,
                               DataType type) {
...
uint32_t HFBFileH5::extend_time(time_ctype new_time) {
    ...
    dset("hfb").resize({ntime, nfreq, nprod});
    dset("hfb_weight").resize({ntime, nfreq, nprod});
...
void HFBFileH5::write_sample(uint32_t time_ind, uint32_t freq_ind, const FrameView& frame_view) {
...
//
// Implementation of the fast HDF5 21cm absorber data file
//

HFBFileH5Fast::HFBFileH5Fast(const std::string& name, const kotekan::logLevel log_level,
                             const std::map<std::string, std::string>& metadata, dset_id_t dataset,
                             size_t max_time) :
    HFBFileH5(name, log_level, metadata, dataset, max_time) {}

void HFBFileH5Fast::deferred_init() {
    create_time_axis();
    create_datasets();
    setup_raw();
}

HFBFileH5Fast::~HFBFileH5Fast() {
...
void HFBFileH5Fast::create_time_axis() {
...
void HFBFileH5Fast::create_dataset(const std::string& name, const std::vector<std::string>& axes,
...
void HFBFileH5Fast::setup_raw() {
    ...
    hfb_offset = H5Dget_offset(dset("hfb").getId());
    weight_offset = H5Dget_offset(dset("hfb_weight").getId());
...
template<typename T>
bool HFBFileH5Fast::write_raw(off_t dset_base, int ind, size_t n, const std::vector<T>& vec) {
...
template<typename T>
bool HFBFileH5Fast::write_raw(off_t dset_base, int ind, size_t n, const T* data) {
...
void HFBFileH5Fast::flush_raw_async(off_t dset_base, int ind, size_t n) {
#ifdef __linux__
...
void HFBFileH5Fast::flush_raw_sync(off_t dset_base, int ind, size_t n) {
#ifdef __linux__
...
uint32_t HFBFileH5Fast::extend_time(time_ctype new_time) {
    ...
    if (ntime > delta_async) {
        flush_raw_async(hfb_offset, ntime - delta_async, nfreq * nprod * sizeof(cfloat));
    ...
    uint delta_sync = 4;
    if (ntime > delta_sync) {
        flush_raw_sync(hfb_offset, ntime - delta_sync, nfreq * nprod * sizeof(cfloat));
...
void HFBFileH5Fast::deactivate_time(uint32_t time_ind) {
    flush_raw_sync(hfb_offset, time_ind, nfreq * nprod * sizeof(cfloat));
...
void HFBFileH5Fast::write_sample(uint32_t time_ind, uint32_t freq_ind,

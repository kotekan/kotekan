...
std::string format_as(const FRBBeam& b) {
    return fmt::format("FRBBeam(id: {:d}, x_dir_grid: {:f}, y_dir_grid: {:f})", b.id, b.x_dir_grid,
                       b.y_dir_grid);
}

std::string format_as(const FixedBBBeam& b) {
    return fmt::format("FixedBBBeam(id: {:d}, x_dir_grid: {:f}, y_dir_grid: {:f})", b.id,
                       b.x_dir_grid, b.y_dir_grid);
}

std::string format_as(const TrackingBBBeam& b) {
    return fmt::format("TrackingBBBeam(id: {:d}, ra_cirs_deg: {:f}, dec_cirs_deg: {:f})", b.id,
                       b.ra_cirs_deg, b.dec_cirs_deg);
}

...
hfbFileRaw::hfbFileRaw(const std::string& name, const kotekan::logLevel log_level,
    ...
    size_t nhfb = sstate ? sstate->get_num_stack() : pstate->get_prods().size();
    ...
    // Calculate the file structure
    auto layout = hfbFrameView::calculate_buffer_layout(ninput, nhfb);
...
void hfbFileRaw::write_sample(uint32_t time_ind, uint32_t freq_ind, const hfbFrameView& frame) {

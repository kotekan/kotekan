...
#include "factory.hpp"         // for FACTORY, CREATE_FACTORY, Factory, REGISTER_NAMED_TYPE_WITH...
...
public:
    ...
    const T& cast() const {
        try {
            ...
            ERROR("Could not cast Telescope of type {:s} to type {:s}",
                    this->get_name(), FACTORY(Telescope)::label<T>());
            throw;
        }
    }

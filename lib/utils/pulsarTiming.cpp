...
SegmentedPolyco::SegmentedPolyco(double rot_freq, float dm, float seg,
    ...
    if (phase_ref.size() != tmid.size() || tmid.size() != coeff.size()) {
        throw std::runtime_error(
            fmt::format(
                "Number of segments is inconsistent: phase_ref({}), rot_freq({}), "
                "coeff({}).", tmid.size(), phase_ref.size(), coeff.size()
            )
        );
    }

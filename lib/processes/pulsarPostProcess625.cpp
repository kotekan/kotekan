...
# define samples_in_frame 625
# define num_packet 80

#include "buffer.h"
#include "pulsarPostProcess625.hpp"
#include "Config.hpp"
#include "util.h"
#include "errors.h"
...
REGISTER_KOTEKAN_PROCESS(pulsarPostProcess625);

pulsarPostProcess625::pulsarPostProcess625(Config& config_,
        const string& unique_name,
        bufferContainer &buffer_container) :
        KotekanProcess(config_, unique_name, buffer_container,
                       std::bind(&pulsarPostProcess625::main_thread, this)){
...
pulsarPostProcess625::~pulsarPostProcess625() {
    free(in_buf);
}

void pulsarPostProcess625::fill_headers(unsigned char * out_buf,
...
void pulsarPostProcess625::main_thread() {
	...
	INFO("[pulsar PP625] got freq bin %d",freq_id);
		...
		for (uint32_t thread_id = 0; thread_id < _num_gpus; ++thread_id) { //loop the 4 GPUs (input)
		    ...
		    for (uint32_t psr = 0; psr<_num_pulsar; ++psr) { //loop psr
			    ...
			    uint8_t real_part = int((in_buf_data[(i*_num_pulsar*_num_pol + psr*_num_pol + p)*2])/float(psr_coord.scaling[psr]) +0.5)+8;
                            uint8_t imag_part = int((in_buf_data[(i*_num_pulsar*_num_pol + psr*_num_pol + p)*2+1])/float(psr_coord.scaling[psr]) +0.5)+8;

...
{   
    ...
    WITH_RFI = config.get_bool(unique_name,"rfi");
...
void nDiskFileRead::file_read_thread(int disk_id) {
    ...
    float MEAN_SENSITIVITY = 0.01;

    float Mean[num_frequencies*num_elements];
	...
	if(!WITH_RFI){
		...
		while(ptr_counter < sz){

			thread_counter++; //Update number of threads
			char buffer[VDIF_BLOCK_SIZE];
			bool mask[num_frequencies];
			fread(&buffer, sizeof(char), VDIF_BLOCK_SIZE, in_file);
			...
			if(thread_counter == num_elements*SK_STEP){
				...
				for(unsigned int i =0; i < num_elements*SK_STEP; i++){
				...
				buf_ptr += num_elements*SK_STEP*VDIF_BLOCK_SIZE;

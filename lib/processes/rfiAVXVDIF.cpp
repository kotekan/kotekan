...
void rfiAVXVDIF::main_thread() {
    ...
    while(!stop_thread) {
        ...
        for (uint32_t j = 0; j < nthreads; j++){ this_thread[j].join(); }
        //Log/display time results
        INFO("Time used for kurtosis calculation: %fms\n",(e_time()-start_time)*1000);
...
void rfiAVXVDIF::parallelSpectralKurtosis(uint32_t loop_idx, uint32_t loop_length){
    ...
    for (uint32_t i = loop_idx*loop_length; i < (loop_idx+1)*loop_length; i++){
        fastSKVDIF(in_local+(i*_sk_step*PACKET_LEN*_num_elements),
                   temp_buffer, sq_temp_buffer, (float*)(out_local+i*_num_local_freq*sizeof(float)));
    }
}

#ifdef __AVX2__
inline void rfiAVXVDIF::fastSKVDIF(uint8_t * data, uint32_t * temp_buf, uint32_t * sq_temp_buf, float *out) {
    ...
    for (uint32_t i = 0; i < _num_local_freq; ++i) {
        ...
        for (uint32_t p = 0; p < _num_elements; p++) {
            float mean = ((float)temp_buf[i+m32 + p*(_num_local_freq+1)]/integration_count[p]);
            sq_power_across_element += (float)sq_temp_buf[i+m32 + p*(_num_local_freq+1)]/(mean*mean);

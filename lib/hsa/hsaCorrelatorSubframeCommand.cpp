...
hsaCorrelatorSubframeCommand::hsaCorrelatorSubframeCommand(
    ...
    _sub_frame_samples = config.get<uint32_t>(unique_name, "samples_per_data_set") / _num_sub_frames;
    _sub_frame_samples = config.get_default<uint32_t>(unique_name, "sub_frame_samples", _sub_frame_samples);

    INFO("nsf: %i, sfs: %i",_num_sub_frames,_sub_frame_samples);
}
...
int hsaRfiOutput::wait_on_precondition(int gpu_frame_id) {
    ...
    uint8_t* var_frame =
        wait_for_empty_frame(_rfi_output_var_buf, unique_name.c_str(), _rfi_output_var_buf_precondition_id);
    if (var_frame == NULL)
        return -1;
...
hsa_signal_t hsaRfiOutput::execute(int gpu_frame_id, hsa_signal_t precede_signal) {
    ...
    void* gpu_output_var_ptr =
        device.get_gpu_memory_array("rfi_output_var", gpu_frame_id, _rfi_output_var_buf->frame_size);
    ...
    void* host_output_var_ptr = (void*)_rfi_output_var_buf->frames[_rfi_output_var_buf_execute_id];
    device.async_copy_gpu_to_host(host_output_var_ptr, gpu_output_var_ptr, _rfi_output_var_buf->frame_size,
                                  precede_signal, signals[gpu_frame_id]);
    
    // Update execution ID
    _rfi_output_buf_execute_id = (_rfi_output_buf_execute_id + 1) % _rfi_output_buf->num_frames;
    _rfi_output_var_buf_execute_id = (_rfi_output_var_buf_execute_id + 1) % _rfi_output_var_buf->num_frames;
    // Return signal
...
void hsaRfiOutput::finalize_frame(int frame_id) {
    ...
    // Mark the output variance buffer as full, so it can be processed.

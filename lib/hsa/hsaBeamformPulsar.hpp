 ...
 *
 *
 * This is an hsaCommand that launches the kernel (pulsar_beamformer.cl) for 
 * brute-force coherent beamforming and is most applicable to pulsar observations. 
 * An array of phases (shape n_psr x n_elem) is calculated by hsaPulsarUpdatePhase.cpp. 
 * The default number of pulsar beams to be formed is 10. The phases are matrix 
 * multiplied with the input data (shape n_samp x n_elem) and the output is of dimension 
 * (n_samp x n_psr).
 *
 *

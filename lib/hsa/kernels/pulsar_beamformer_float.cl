...
__kernel void pulsarbf_float( __global uint *data, __global float2 *phase,  __global float2 *output){
  ...
  for (int t=0;t<TS;t++){
    ...
    //Reduction of 128, eventually each number comes from the sum of 1024 values
    ...
    if (get_local_id(0) == 0) {
      output[(t*nsamp/TS+get_group_id(2))*20 + get_group_id(1)*2 + get_group_id(0)] = sum[4*get_local_id(0)] + sum[4*get_local_id(0) +1] + sum[4*get_local_id(0) +2] + sum[4*get_local_id(0)+3];
    }

...
hsaSleeper::~hsaSleeper() {

}

hsa_signal_t hsaSleeper::execute(int gpu_frame_id, const uint64_t& fpga_seq, hsa_signal_t precede_signal) {

    hsa_signal_value_t value = hsa_signal_wait_acquire(precede_signal, HSA_SIGNAL_CONDITION_LT, 1, UINT64_MAX, HSA_WAIT_STATE_BLOCKED);
//DEBUG("Sleeping!");
//    usleep(1000000);
    ...
    //INFO("hsaSleeper got write index: %" PRIu64 ", packet_address: %p, precede_signal: %lu", index, barrier_and_packet, precede_signal.handle);
...
void hsaSleeper::finalize_frame(int frame_id) {

...
hsa_signal_t hsaRfiVdif::execute(int gpu_frame_id, hsa_signal_t precede_signal) {
    ...
    args.input =
        device.get_gpu_memory_array("input", gpu_frame_id, _gpu_buffer_depth, input_frame_len); // Grab GPU memory
    args.output = device.get_gpu_memory_array("rfi_output", gpu_frame_id, _gpu_buffer_depth, input_frame_len);

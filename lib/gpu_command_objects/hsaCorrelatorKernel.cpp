...
hsaCorrelatorKernel::hsaCorrelatorKernel(const string& kernel_name, const string& kernel_file_name,
    ...
    uint32_t * host_block_map = (uint32_t *)hsa_host_malloc(block_map_len);
    int block_id = 0;
    ...
    corr_kernel_config_t * host_kernel_args = (corr_kernel_config_t *)hsa_host_malloc(sizeof(corr_kernel_config_t));
    ...
    // Free the host memory (is there any reason we might want to keep this?)

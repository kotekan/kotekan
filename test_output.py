...
def parse_vdif_header(header):
    ...
    class VDIFHeader(ctypes.Structure):
        _fields_ = [
            ...
            ("word4", ctypes.c_uint32),
            ("word5", ctypes.c_uint32),
            ("word6", ctypes.c_uint32),
            ("word7", ctypes.c_uint32),
        ]

    raw_header = VDIFHeader.from_buffer(header)
    parsed_header = dict()
    parsed_header["seconds"] = (
        raw_header.word0 & 0b00111111111111111111111111111111
    ) >> 0
    parsed_header["legacy"] = (
        raw_header.word0 & 0b01000000000000000000000000000000
    ) >> 30
    parsed_header["invalid"] = (
        raw_header.word0 & 0b10000000000000000000000000000000
    ) >> 31
    parsed_header["data_frame"] = (
        raw_header.word1 & 0b00000000111111111111111111111111
    ) >> 0
    parsed_header["ref_epoch"] = (
        raw_header.word1 & 0b00111111000000000000000000000000
    ) >> 24
    parsed_header["unused"] = (
        raw_header.word1 & 0b11000000000000000000000000000000
    ) >> 30
    parsed_header["frame_len"] = (
        raw_header.word2 & 0b00000000111111111111111111111111
    ) >> 0
    parsed_header["log_num_chan"] = (
        raw_header.word2 & 0b00011111000000000000000000000000
    ) >> 24
    parsed_header["vdif_version"] = (
        raw_header.word2 & 0b11100000000000000000000000000000
    ) >> 29
    parsed_header["station_id"] = (
        raw_header.word3 & 0b00000000000000001111111111111111
    ) >> 0
    parsed_header["thread_id"] = (
        raw_header.word3 & 0b00000011111111110000000000000000
    ) >> 16
    parsed_header["bits_depth"] = (
        raw_header.word3 & 0b01111100000000000000000000000000
    ) >> 26
    parsed_header["data_type"] = (
        raw_header.word3 & 0b10000000000000000000000000000000
    ) >> 31
...
for packet in range(0, 5):
    ...
    for psr in range(0, _num_pulsars):
        for frame in range(0, _num_packet_per_stream):
            ...
            header = parse_vdif_header(
                dset[frame_index : frame_index + udp_pulsar_header_size]
            )
                ...
                for p in range(0, _num_pol):
                    for in_frame_location in range(0, _timesamples_per_pulsar_packet):
                        ...
                        i = (
                            ...
                            * _timesamples_per_pulsar_packet
                            + frame * _timesamples_per_pulsar_packet
                            + in_frame_location
                        ) % samples_per_dataset
                        sub_i = i & 0x3
                        i = i & ~0x3
                        ...
                        freq = (
                            thread_id
                            + packet
                            % (num_freqs // num_freqs_per_frame)
                            * num_freqs_per_frame
                        )

                        count = (
                            psr * num_freqs * num_pols * samples_per_dataset // 4

...
void network_thread(void * arg) {
    ...
    if (setsockopt(udp_socket, SOL_SOCKET, SO_RCVBUF, (void *) &n, sizeof(n)) == -1) {
        perror("Error in set socket rcvbuf size.");
    }
    ...
    int32_t last_seq = -1;
    int32_t diff = 0;
    int32_t total_lost = 0;
    int32_t grand_total_lost = 0;
    int32_t lost = 0;
    int32_t total_out_of_order = 0;
    int32_t total_duplicate = 0;
    ...
    for (;;) {
        ...
        for (int i = 0; i < num_events; i++) {
            ...
            if (bytes_read == -1) {
                perror("read");
                printf("bytes_read: %d\nBuffer Id: %d\nBuffer location: %d\n", (int)bytes_read, buffer_id, buffer_location);
                exit(errno);
            }

            if (bytes_read != UDP_PACKETSIZE) {
                printf("Bad packet. Panic, Abort, etc. This should never happen.\n");
                exit(EXIT_FAILURE);
            }
            ...
            seq = ntohs(((uint16_t *) &args->buf->data[buffer_id][buffer_location] )[0]);

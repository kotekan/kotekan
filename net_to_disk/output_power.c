...
void output_power_thread(void * arg)
{
    ...
    const int integration_time = 512;
    ...
    if (args->legacy_output == 1) {
        snprintf(file_name, file_name_len, "/mnt/ram_disk/power_data_legacy.dat");
        ...
        if (fd_legacy == NULL) {
            perror("Cannot open legacy file");
            fprintf(stderr, "File name was: %s", file_name);
            exit(errno);
        }
    }
	...
	// This call is blocking.
        ...
        for (int integration = 0;
             integration < args->num_timesamples/args->integration_samples;
             ++integration) {
            ...
            *((uint32_t *) out_buf + 1) = *(uint32_t *)&data[integration*packet_len*integration_time/args->num_frames + 46];
            ...
            for (int packet = 0; packet < integration_time/args->num_frames; packet++) {
                ...
                for (int frame = 0; frame < args->num_frames; ++frame) {
                    for (int freq = 0; freq < args->num_freq / 16; ++freq) {

                        const int index = integration * packet_len * integration_time/args->num_frames
                                                + packet * packet_len + PACKET_OFFSET
                                                + frame * frame_size + freq*32;
                        ...
                        if (packet != 0) {
                            ymm4 = _mm256_loadu_si256((__m256i const *)&temp_buf[freq * 32 + 0*8]);
                            ymm5 = _mm256_loadu_si256((__m256i const *)&temp_buf[freq * 32 + 1*8]);
                            ymm6 = _mm256_loadu_si256((__m256i const *)&temp_buf[freq * 32 + 2*8]);
                            ymm7 = _mm256_loadu_si256((__m256i const *)&temp_buf[freq * 32 + 3*8]);
                        ...
                        _mm256_storeu_si256((__m256i *)&temp_buf[freq * 32 + 0*8], ymm0);
                        _mm256_storeu_si256((__m256i *)&temp_buf[freq * 32 + 1*8], ymm1);
                        _mm256_storeu_si256((__m256i *)&temp_buf[freq * 32 + 2*8], ymm2);
                        _mm256_storeu_si256((__m256i *)&temp_buf[freq * 32 + 3*8], ymm3);
        ...
        if (args->legacy_output == 1) {
            ...
            if (ints_written != args->num_freq*2) {
                printf("Failed to write power data to legacy ram disk!!!");
                fclose(fd);
            }
        }

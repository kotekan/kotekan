...
int main(int argc, char** argv) {
    int n_job = 11, n_chan = 7, n_chips = 140, iters = 50;
    ...
    for (int b = 0; b < n_job; ++b) {
        CK(cudaMalloc(&dA[b], phi_n * sizeof(float2)));
        CK(cudaMalloc(&dB[b], phi_n * sizeof(float2)));
        CK(cudaMemcpy(dA[b], junk.data(), phi_n * sizeof(float2), cudaMemcpyHostToDevice));
        CK(cudaMemcpy(dB[b], junk.data(), phi_n * sizeof(float2), cudaMemcpyHostToDevice));
    }
